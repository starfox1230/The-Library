...
Use that builder's `MODEL` definition directly. The shared manifest builder loads it from that canonical module; do not add a second handwritten `saCloze++` definition. The model name, model id, fields, templates, and CSS must remain exactly aligned with the user's current `saCloze++` template so Anki does not create a look-alike note type.
...
Write card facts and prompts as direct medical statements or questions. Never qualify them with `according to the source`, `in the source`, `based on the reading`, `the text states`, or similar source-framing language. Treat the supplied material as authoritative for the exercise and reserve provenance for `Extra` and package metadata.
Do not include multiple-choice answer letters in `Text`; package the actual tested answer only.
For pathology image cards, default to either `What is the most likely diagnosis?` or `What is indicated by the [color] arrow(s)?`. When clinical information is needed, write one brief patient/context sentence before the diagnosis question. Color the complete arrow phrase with inline HTML to visually match the arrow. Preserve prompt, `<br><br>`, cloze answer, `<br><br>`, and then the image area. Avoid stilted label fragments.
When a pathology image supports a diagnosis and has one or more meaningful arrows whose targets are established by the caption or source context, always package a complete candidate set: one separate diagnosis card and one separate arrow card for every separately identifiable arrow target. Do not deduplicate or omit these cards based on predicted usefulness; the user will decide which candidates to keep. Multiple arrowheads acting together as one plural annotation of the same target count as one target, while distinct targets require distinct cards.
...
- For every textbook/PDF-derived card, include the correct full source-page screenshot in `Extra`. This includes visual cards, text-only cards, saved-fact cards, and cards derived from auto-generated quizzes. Preserve source-page mapping during quiz and candidate generation; include multiple page screenshots in source order when the card genuinely depends on multiple pages. Omit only when the page cannot be identified, captured, or stored, and record the exception rather than guessing.
