...
QUESTION_START_PAGE = 158
QUESTION_END_PAGE = 203
ANSWER_START_PAGE = 203
ANSWER_END_PAGE = 246
MAX_QUESTIONS = 60
...
def clean_extracted_field(text: str) -> str:
...
def strip_page_noise(text: str) -> str:
    text = re.sub(r"^3 Pediatric Musculoskeletal System Questions\s*", "", text)
...
def split_questions(text: str) -> list[dict]:
    ...
    for i, match in enumerate(matches):
        ...
        if not 1 <= number <= MAX_QUESTIONS:
            continue
        start = match.end()
        end = matches[i + 1].start() if i + 1 < len(matches) else len(text)
        body = text[start:end].strip()
        body = re.sub(r"(?<=[A-Za-z0-9)])([A-E])\.\s*", r" \1. ", body)
        option_matches = list(re.finditer(r"\b([A-E])\.\s*", body))
        if not option_matches:
            continue
        stem = clean_extracted_field(body[: option_matches[0].start()])
        options = []
        for j, opt in enumerate(option_matches):
            opt_start = opt.end()
            opt_end = option_matches[j + 1].start() if j + 1 < len(option_matches) else len(body)
            options.append({"letter": opt.group(1), "text": clean_extracted_field(body[opt_start:opt_end])})
        questions.append({"number": number, "stem": stem, "options": options, "images": []})
    return questions


def split_answers(text: str) -> dict[int, dict]:
    matches = list(re.finditer(r"(?:^|\s)(\d{1,2})\s+Answer\s+([A-E])\.", text))
        ...
        for page_index in range(QUESTION_START_PAGE, QUESTION_END_PAGE):
            ...
            for block in blocks:
                if block["type"] == 0:
                    ...
                    starts = [int(m.group(1)) for m in q_start_re.finditer(text) if 1 <= int(m.group(1)) <= MAX_QUESTIONS]
                    if starts:
...
def extract_explanation_images(questions: list[dict]) -> None:
    ...
    answer_re = re.compile(r"^\s*(\d{1,2})\s+Answer\s+[A-E]\.")
    doc = fitz.open(str(PDF_PATH))
    try:
...
<body>
  <header>
    <div class="bar">
      <div>
        <h1>Pediatric Musculoskeletal System Quiz</h1>
      </div>
  ...
  <dialog id="settingsDialog">
    ...
    <div class="settings-body">
      ...
      <div class="settings-field">
        ...
        <textarea id="stateImport" placeholder='{{"version":1,"appId":"pediatric-msk-imaging-quiz-ch3",...}}'></textarea>
      </div>
      <div class="settings-row">
  ...
  <script>
    ...
    const APP_ID = "pediatric-msk-imaging-quiz-ch3";
