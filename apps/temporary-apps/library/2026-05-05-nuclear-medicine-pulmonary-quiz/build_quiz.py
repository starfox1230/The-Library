...
TITLE = "Nuclear Medicine Pulmonary System Quiz"
APP_ID = "nuclear-medicine-pulmonary-quiz-ch7"

# Zero-based PDF page indexes for chapter 7.
QUESTION_RANGES = [range(245, 263)]
ANSWER_RANGES = [range(262, 280)]
...
def is_noise_line(line: str) -> bool:
    ...
    if re.fullmatch(r"\d{1,2}[A-Z]", line):
        return True
    if line in {"QUESTIONS", "ANSWERS AND EXPLANATIONS", "Endocrine System", "Musculoskeletal System", "Head and Neck", "Nuclear Cardiology", "Vascular and Lymphatics", "Pulmonary System"}:
        return True
    if line in {"2", "3", "4", "5", "6", "7"}:
        return True
    return False
        ...
        for page_index in page_range:
            for line in page_lines(reader, page_index):
                ...
                if qmatch and not re.fullmatch(r"\d{1,2}", qmatch.group(2)):
                    ...
                    if qid == "7" and "Pulmonary System" in stem:
                        continue
                    flush_question()
