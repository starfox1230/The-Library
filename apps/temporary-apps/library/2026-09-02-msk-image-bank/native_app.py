...
PySide6 is the only runtime dependency. Image files live under the user's
    local app-data directory; the repository contains only the 54-item seed list.
"""
...
def parse_seed_data() -> list[dict]:
    ...
    if len(items) != 54:
        raise RuntimeError(f"Expected 54 seed diagnoses, found {len(items)}")
    for item in items:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setStyleSheet("QMainWindow,QWidget { background:#0b1018; color:#e7edf5; } QLineEdit,QTextEdit { background:#0b131f; border:1px solid #26364a; border-radius:6px; padding:7px; color:#e7edf5; } QPushButton { background:#182536; border:1px solid #26364a; border-radius:6px; padding:7px 10px; color:#e7edf5; } QPushButton:hover { border-color:#22d3ee; background:#20354c; } QPushButton#primary { background:#0e7490; border-color:#22d3ee; } QPushButton#favorite-active { color:#facc15; background:#3b2f0a; border-color:#eab308; } QPushButton#favorite-active:hover { color:#fde68a; background:#4a3a0b; border-color:#facc15; } QCheckBox { color:#8ea0b6; } QListWidget { background:#0e1622; border:0; outline:0; } QListWidget::item { padding:7px; border-radius:5px; } QListWidget::item:selected { background:#172131; color:#e7edf5; } QScrollArea { border:0; }")
