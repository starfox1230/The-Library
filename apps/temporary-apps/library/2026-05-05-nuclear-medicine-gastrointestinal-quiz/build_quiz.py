...
TITLE = "Nuclear Medicine Gastrointestinal System Quiz"
APP_ID = "nuclear-medicine-gastrointestinal-quiz-ch8"

# Zero-based PDF page indexes for chapter 8.
QUESTION_RANGES = [range(280, 296), range(313, 318)]
ANSWER_RANGES = [range(295, 313), range(318, 322)]
...
def is_noise_line(line: str) -> bool:
    ...
    if re.fullmatch(r"\d{1,2}[A-Z]", line):
        return True
    if line in {"QUESTIONS", "ANSWERS AND EXPLANATIONS", "Endocrine System", "Musculoskeletal System", "Head and Neck", "Nuclear Cardiology", "Vascular and Lymphatics", "Pulmonary System", "Gastrointestinal System"}:
        return True
    if line in {"2", "3", "4", "5", "6", "7", "8"}:
        return True
    return False
        ...
        for page_index in page_range:
            for line in page_lines(reader, page_index):
                ...
                if qmatch and not re.fullmatch(r"\d{1,2}", qmatch.group(2)):
                    ...
                    if qid == "8" and "Gastrointestinal System" in stem:
                        continue
                    flush_question()
...
def apply_manual_fixes(questions: list[dict]) -> None:
    for q in questions:
        ...
        if q["number"] == "4" and not q["options"]:
            q["options"] = [
                {"letter": "A", "text": "Curve A"},
                {"letter": "B", "text": "Curve B"},
                {"letter": "C", "text": "Curve C"},
                {"letter": "D", "text": "Curve D"},
            ]
