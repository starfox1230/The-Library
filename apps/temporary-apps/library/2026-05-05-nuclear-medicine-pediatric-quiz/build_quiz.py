...
TITLE = "Nuclear Medicine Pediatric Nuclear Medicine Quiz"
APP_ID = "nuclear-medicine-pediatric-quiz-ch10"

# Zero-based PDF page indexes for chapter 10.
QUESTION_RANGES = [range(361, 371), range(380, 384)]
ANSWER_RANGES = [range(370, 380), range(383, 386)]
...
def is_noise_line(line: str) -> bool:
    ...
    if re.fullmatch(r"\d{1,2}[A-Z]", line):
        return True
    if line in {"QUESTIONS", "ANSWERS AND EXPLANATIONS", "Endocrine System", "Musculoskeletal System", "Head and Neck", "Nuclear Cardiology", "Vascular and Lymphatics", "Pulmonary System", "Gastrointestinal System", "Genitourinary System", "Pediatric Nuclear Medicine"}:
        return True
        ...
        for page_index in page_range:
            for line in page_lines(reader, page_index):
                ...
                if qmatch and not re.fullmatch(r"\d{1,2}", qmatch.group(2)):
                    ...
                    if qid == "10" and "Pediatric Nuclear Medicine" in stem:
                        continue
                    flush_question()
