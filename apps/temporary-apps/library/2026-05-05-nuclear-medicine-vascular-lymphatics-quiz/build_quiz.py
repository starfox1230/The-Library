...
TITLE = "Nuclear Medicine Vascular and Lymphatics Quiz"
APP_ID = "nuclear-medicine-vascular-lymphatics-quiz-ch6"

# Zero-based PDF page indexes for chapter 6.
QUESTION_RANGES = [range(226, 236)]
ANSWER_RANGES = [range(235, 245)]
...
def is_noise_line(line: str) -> bool:
    ...
    if re.fullmatch(r"\d{1,2}[A-Z]", line):
        return True
    if line in {"QUESTIONS", "ANSWERS AND EXPLANATIONS", "Endocrine System", "Musculoskeletal System", "Head and Neck", "Nuclear Cardiology", "Vascular and Lymphatics"}:
        return True
    if line in {"2", "3", "4", "5", "6"}:
        return True
    return False
        ...
        for page_index in page_range:
            for line in page_lines(reader, page_index):
                ...
                if qmatch and not re.fullmatch(r"\d{1,2}", qmatch.group(2)):
                    ...
                    if qid == "6" and "Vascular and Lymphatics" in stem:
                        continue
                    flush_question()
