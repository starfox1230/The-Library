...
TITLE = "MSK Normal/Normal Variants Quiz"
APP_ID = "msk-normal-variants-quiz-ch2"

# Zero-based PDF page indexes. Chapter 2's questions 1-8 are followed by their
# answer section, then question 9 and its answer appear as a short continuation.
QUESTION_RANGES = [range(56, 72), range(74, 77)]
ANSWER_RANGES = [range(71, 74), range(77, 78)]
...
def is_noise_line(line: str) -> bool:
    ...
    if line in {"QUESTIONS", "ANSWERS AND EXPLANATIONS"}:
        return True
    if line in {"2", "Normal/Normal Variants"}:
        return True
    return False
        ...
        for page_index in page_range:
            for line in page_lines(reader, page_index):
                ...
                if qmatch and not re.fullmatch(r"\d{1,2}", qmatch.group(2)):
                    ...
                    if int(re.match(r"\d+", qid).group(0)) > 9:
                        if current:
                            current["stem"] += " " + line
                        continue
                    if qid == "2" and "Normal/Normal" in stem:
                        continue
                    flush_question()
