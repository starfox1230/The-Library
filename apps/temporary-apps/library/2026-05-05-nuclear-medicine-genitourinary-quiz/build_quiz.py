...
TITLE = "Nuclear Medicine Genitourinary System Quiz"
APP_ID = "nuclear-medicine-genitourinary-quiz-ch9"

# Zero-based PDF page indexes for chapter 9.
QUESTION_RANGES = [range(322, 341), range(352, 359)]
ANSWER_RANGES = [range(340, 352), range(358, 361)]
...
def is_noise_line(line: str) -> bool:
    ...
    if re.fullmatch(r"\d{1,2}[A-Z]", line):
        return True
    if line in {"QUESTIONS", "ANSWERS AND EXPLANATIONS", "Endocrine System", "Musculoskeletal System", "Head and Neck", "Nuclear Cardiology", "Vascular and Lymphatics", "Pulmonary System", "Gastrointestinal System", "Genitourinary System"}:
        return True
        ...
        for page_index in page_range:
            for line in page_lines(reader, page_index):
                ...
                if is_noise_line(line):
                    continue
                qmatch = re.match(r"^(\d{1,2}(?:[A-Ba-b])?)\s+(.+)$", line)
                if qmatch and not re.fullmatch(r"\d{1,2}", qmatch.group(2)):
                    qid, stem = qmatch.groups()
                    if qid == "9" and "Genitourinary System" in stem:
                        continue
                    flush_question()
    ...
    for page_range in ANSWER_RANGES:
        for page_index in page_range:
            for line in page_lines(reader, page_index):
                if is_noise_line(line):
                    continue
                match = re.match(r"^(\d{1,2}(?:[A-Ba-b])?)(?:s)?\s+Answer\s+([A-J])\.\s*(.*)$", line)
                if match:
...
def apply_manual_fixes(questions: list[dict]) -> None:
    for q in questions:
        ...
        if q["number"] == "2" and not q["options"]:
            q["options"] = [
                {"letter": "A", "text": "Structure A"},
                {"letter": "B", "text": "Structure B"},
                {"letter": "C", "text": "Structure C"},
                {"letter": "D", "text": "Structure D"},
            ]
        if q["number"] == "9" and not q["options"]:
            q["options"] = [
                {"letter": "A", "text": "Curve set A"},
                {"letter": "B", "text": "Curve set B"},
                {"letter": "C", "text": "Curve set C"},
                {"letter": "D", "text": "Curve set D"},
                {"letter": "E", "text": "Curve set E"},
            ]
