...
SOURCE_PDF = Path(r"G:/My Drive/0. Radiology/Core Review Books/IR/6_ Urinary.pdf")
ASSET_DIR = APP_DIR / "assets"

TITLE = "Interventional Radiology Urinary Quiz"
APP_ID = "ir-urinary-quiz-ch6"
DEFAULT_SEED_VERSION = 1
DEFAULT_SELECTED: dict[str, str] = {}
CONTENT_PAGES = range(0, 35)
...
def is_noise_line(line: str) -> bool:
    if line in {"QUESTIONS", "ANSWERS AND EXPLANATIONS", "CHAPTER 6", "Urinary"}:
        return True
...
def looks_like_question_start(qid: str, rest: str, current: dict | None) -> bool:
    ...
    return bool(re.match(
        r"^(A|An|After|Before|Based|Diagnostic|During|Following|Immediately|In|Regarding|The|This|To|When|Which|What|With)\b",
        rest,
    ))
    ...
    for page_index in CONTENT_PAGES:
        for line in page_lines(reader, page_index):
            ...
            if qmatch:
                ...
                if 1 <= question_base(qid) <= 21 and looks_like_question_start(qid, stem, current):
                    flush_question()
...
def render_html(questions: list[dict]) -> str:
    ...
    for old in [
        ...
        "Interventional Radiology Gastrointestinal Quiz",
    ]:
