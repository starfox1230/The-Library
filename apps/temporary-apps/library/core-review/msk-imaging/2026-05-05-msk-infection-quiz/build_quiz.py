...
TITLE = "MSK Infection Quiz"
APP_ID = "msk-infection-quiz-ch4"

# Zero-based PDF page indexes. Chapter 4's questions 1-12 are followed by an
# answer section, then questions 13-14 and their answers appear as a short continuation.
QUESTION_RANGES = [range(111, 138), range(141, 146)]
ANSWER_RANGES = [range(137, 141), range(146, 147)]
        ...
        for page_index in page_range:
            for line in page_lines(reader, page_index):
                ...
                if qmatch and not re.fullmatch(r"\d{1,2}", qmatch.group(2)):
                    ...
                    if qid == "6" and stem.lower().startswith("months prior"):
                        if current:
                            current["stem"] += " " + line
                        continue
                    if int(re.match(r"\d+", qid).group(0)) > 14:
                        if current:
                            current["stem"] += " " + line
                        continue
                    if qid == "4" and "Infection" in stem:
                        continue
                    flush_question()
