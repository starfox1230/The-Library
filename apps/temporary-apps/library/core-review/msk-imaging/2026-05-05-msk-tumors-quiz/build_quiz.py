...
TITLE = "MSK Tumors and Tumor-Like Conditions Quiz"
APP_ID = "msk-tumors-tumor-like-conditions-quiz-ch5"

# Zero-based PDF page indexes. Chapter 5's questions 1-75 are followed by an
# answer section, then questions 76-87 and their answers appear as a short continuation.
QUESTION_RANGES = [range(147, 263), range(282, 287)]
ANSWER_RANGES = [range(262, 282), range(286, 290)]
...
def is_noise_line(line: str) -> bool:
    ...
    if line in {"QUESTIONS", "ANSWERS AND EXPLANATIONS"}:
        return True
    if line in {"5", "Tumors and Tumor-Like Conditions"}:
        return True
    return False
        ...
        for page_index in page_range:
            for line in page_lines(reader, page_index):
                ...
                if qmatch and not re.fullmatch(r"\d{1,2}", qmatch.group(2)):
                    ...
                    if int(re.match(r"\d+", qid).group(0)) > 87:
                        if current:
                            current["stem"] += " " + line
                        continue
                    if qid == "5" and "Tumors and Tumor-Like Conditions" in stem:
                        continue
                    flush_question()
...
def extract_images(reader: PdfReader, questions: list[dict]) -> None:
    # Dynamic map for the long tumor chapter. If a page starts with answer
    # choices/captions and then introduces the next question, its image belongs
    # to the active previous question. If a page starts with a question stem, it
    # belongs to that newly introduced question. This is still validated with the
    # generated contact sheet and the image-cue report after each build.
    question_page_targets: dict[int, list[str]] = {}
    by_number = {q["number"]: q for q in questions}
    active_q: str | None = None
    qid_pattern = re.compile(r"^(\d{1,2}[A-Za-z]?)\s+(.+)$")
    for page_range in QUESTION_RANGES:
        for page_index in page_range:
            lines = [line for line in page_lines(reader, page_index) if not is_noise_line(line)]
            starts: list[str] = []
            for line in lines:
                ...
                if match and not re.fullmatch(r"\d{1,2}", match.group(2)):
                    qid = match.group(1).replace("A", "a").replace("B", "b")
                    if qid in by_number:
                        starts.append(qid)
            if reader.pages[page_index].images:
                first_line = lines[0] if lines else ""
                first_starts_question = bool(qid_pattern.match(first_line)) and starts
                target = starts[0] if first_starts_question else active_q
                if target in by_number:
                    question_page_targets[page_index] = [target]
            if starts:
                active_q = starts[-1]
    question_page_targets.update({
        184: ["29a"],
        200: ["39"],
        201: ["39"],
        237: ["61a"],
    })
    answer_page_targets: dict[int, list[str]] = {}
    ...
    # Follow-up subparts often ask management, histology, or sequelae for the
    # same imaged lesion. Reuse the parent image set when the subpart has none.
    for q in questions:
...
def render_html(questions: list[dict]) -> str:
    ...
    template = template.replace("MSK Congenital and Developmental Anomalies and Dysplasias Quiz", TITLE)
    template = template.replace("MSK Infection Quiz", TITLE)
    ...
    template = template.replace("msk-congenital-developmental-quiz-ch3", APP_ID)
    template = template.replace("msk-infection-quiz-ch4", APP_ID)
