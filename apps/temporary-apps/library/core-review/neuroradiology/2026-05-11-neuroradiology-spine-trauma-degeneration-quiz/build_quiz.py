...
SOURCE_PDF = Path(r"G:/My Drive/0. Radiology/Core Review Books/Neuroradiology/9SpineTraumaandDegene_NeuroradiologyACoreRe.pdf")
ASSET_DIR = APP_DIR / "assets"

TITLE = "Neuroradiology Spine Trauma and Degeneration Quiz"
APP_ID = "neuroradiology-spine-trauma-degeneration-quiz-ch9"
DEFAULT_SEED_VERSION = 1
DEFAULT_SELECTED: dict[str, str] = {}

QUESTION_RANGES = [range(0, 14)]
ANSWER_RANGES = [range(13, 38)]
...
def is_noise_line(line: str) -> bool:
    ...
    if line in {"QUESTIONS", "Questions", "ANSWERS AND EXPLANATIONS", "Answers and Explanations"}:
        return True
    if line in {"Spine Trauma and", "Degeneration"}:
        return True
    if re.fullmatch(r"9\s+Spine\s+Trauma\s+and", line):
        return True
    return False
...
def normalize_options(questions: list[dict]) -> None:
    ...
    parent_stems["11"] = "A 72-year-old female with history of breast cancer presents with neck pain."
    collapsed: list[dict] = []
    for q in questions:
        ...
        for index, option in enumerate(q["options"]):
            if index >= 4:
                break
            if q["number"] == "10" and option["letter"] == "D":
                option["text"] = option["text"].split(" A 72-year-old female", 1)[0]
