...
SOURCE_PDF = Path(r"G:/My Drive/0. Radiology/Core Review Books/Neuroradiology/11SpineNeoplasmsSpine_NeuroradiologyACoreRe.pdf")
ASSET_DIR = APP_DIR / "assets"

TITLE = "Neuroradiology Spine Neoplasms and Vascular Diseases Quiz"
APP_ID = "neuroradiology-spine-neoplasms-vascular-diseases-quiz-ch11"
DEFAULT_SEED_VERSION = 1
DEFAULT_SELECTED: dict[str, str] = {}

QUESTION_RANGES = [range(0, 25)]
ANSWER_RANGES = [range(24, 59)]
...
def is_noise_line(line: str) -> bool:
    ...
    if line in {"QUESTIONS", "Questions", "ANSWERS AND EXPLANATIONS", "Answers and Explanations"}:
        return True
    if line in {"and Neck Vascular Diseases"}:
        return True
    if re.fullmatch(r"11\s+Spine\s+Neoplasms,\s+Spine", line):
        return True
    return False
...
def parse_answers(reader: PdfReader, questions: list[dict]) -> None:
    ...
    for qid, answer in {"13a": "C"}.items():
        if qid in by_id and not by_id[qid].get("answer"):
            by_id[qid]["answer"] = answer
            by_id[qid]["explanation"] = by_id.get("13b", {}).get("explanation", f"Answer {answer}.")
