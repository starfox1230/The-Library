...
SOURCE_PDF = Path(r"G:/My Drive/0. Radiology/Core Review Books/Neuroradiology/12CongenitalandDevelo_NeuroradiologyACoreRe.pdf")
ASSET_DIR = APP_DIR / "assets"

TITLE = "Neuroradiology Congenital and Developmental Spine Quiz"
APP_ID = "neuroradiology-congenital-developmental-spine-quiz-ch12"
DEFAULT_SEED_VERSION = 1
DEFAULT_SELECTED: dict[str, str] = {}

QUESTION_RANGES = [range(0, 11)]
ANSWER_RANGES = [range(10, 25)]
...
def is_noise_line(line: str) -> bool:
    ...
    if line in {"QUESTIONS", "Questions", "ANSWERS AND EXPLANATIONS", "Answers and Explanations"}:
        return True
    if line in {"Developmental Abnormalities of the", "Spine"}:
        return True
    if re.fullmatch(r"12\s+Congenital\s+and", line):
        return True
    return False
        ...
        for page_index in page_range:
            for line in page_lines(reader, page_index):
                ...
                if is_noise_line(line) or line.startswith("Images courtesy"):
                    continue
                qmatch = re.match(r"^(\d{1,2}[a-c]?)\s+(.+)$", line, flags=re.I)
                if qmatch and not re.fullmatch(r"\d{1,3}", qmatch.group(2)):
                    ...
                    continue
                bare_qmatch = re.fullmatch(r"(\d{1,2}[a-c])", line, flags=re.I)
                if bare_qmatch:
                    flush_question()
...
def parse_answers(reader: PdfReader, questions: list[dict]) -> None:
    ...
    start_re = re.compile(r"^(\d{1,2}[a-c]?)(?:\s+Answers?|\s+Answer)\s+([A-D])\.\s*(.*)$", re.I)
...
def normalize_options(questions: list[dict]) -> None:
    ...
    for q in questions:
        if re.fullmatch(r"\d{1,2}", q["number"]) and not q["options"]:
            continue
        subpart = re.fullmatch(r"(\d{1,2})[a-c]", q["number"])
...
def render_html(questions: list[dict]) -> str:
    ...
    template = template.replace("Neuroradiology Spine Infection, Inflammation, and Demyelination Quiz", TITLE)
    template = template.replace("Gastrointestinal Imaging Small Bowel Quiz", TITLE)
    template = template.replace("Neuroradiology Spine Neoplasms and Vascular Diseases Quiz", TITLE)
    ...
    template = template.replace("neuroradiology-spine-infection-inflammation-demyelination-quiz-ch10", APP_ID)
    template = template.replace("neuroradiology-spine-neoplasms-vascular-diseases-quiz-ch11", APP_ID)
