# Pediatric MSK Imaging Quiz Build Notes

This temporary app turns chapter 3 of `Core Review Pediatric Imaging.pdf` into a self-contained HTML quiz.
...
```text
temporary-app-state:pediatric-msk-imaging-quiz-ch3
```
...
- Questions are PDF page indexes `158..202` in zero-based Python indexing, shown as pages 159-203 in a PDF viewer.
- Answers and explanations are PDF page indexes `203..245`, shown as pages 204-246.
- Chapter 4 starts on PDF page 247.
...
```powershell
& 'C:\Users\sterl\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' 'apps\temporary-apps\library\2026-04-24-pediatric-msk-imaging-quiz\build_quiz.py'
...
& 'C:\Users\sterl\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe' 'apps\temporary-apps\library\2026-04-24-pediatric-msk-imaging-quiz\build_quiz.py'
python scripts\verify-temporary-apps-index.py
& 'C:\Users\sterl\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe' -e "const fs=require('fs'), vm=require('vm'); const html=fs.readFileSync('apps/temporary-apps/library/2026-04-24-pediatric-msk-imaging-quiz/index.html','utf8'); new vm.Script(html.match(/<script>([\s\S]*)<\/script>/)[1]); console.log('script compiles');"
```
