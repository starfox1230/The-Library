...
TITLE = "Absolute Breast Imaging Review Screening Mammogram Quiz"
APP_ID = "absolute-breast-imaging-review-screening-mammogram-quiz-ch3"
DEFAULT_SEED_VERSION = 1
DEFAULT_SELECTED: dict[str, str] = {}
QUESTION_PAGES = range(84, 106)
ANSWER_PAGES = range(105, 128)
...
def is_noise_line(line: str) -> bool:
    if re.fullmatch(r"\d+\s+N\s*\.\s*Yaghmai et al\.", line):
        return True
    if re.fullmatch(r"3 Screening Mammogram\s+\d+", line):
        return True
    if re.fullmatch(r"\d+\s+\d+\s+N\s*\.\s*Yaghmai et al\.", line):
        return True
    if re.fullmatch(r"3 Screening Mammogram\s+\d+\s+\d+", line):
        return True
...
def parse_questions(reader: PdfReader) -> list[dict]:
    ...
    for page_index in QUESTION_PAGES:
        for line in page_lines(reader, page_index):
            ...
            if qmatch and 1 <= int(re.match(r"\d+", qmatch.group(1)).group(0)) <= 41:
                flush_question()
...
def extract_images(reader: PdfReader, questions: list[dict]) -> None:
    ...
    explanation_image_targets = {
        ...
        (112, 1): ["20"],
        (113, 1): ["21", "22"],
        (114, 1): ["23a"], (114, 2): ["23a"], (114, 3): ["23a"],
        (115, 1): ["24a"],
        (117, 1): ["24c"], (117, 2): ["24c"], (117, 3): ["24c"],
        (118, 1): ["25b"],
        (120, 1): ["26"],
        (121, 1): ["29"], (121, 2): ["29"],
        (122, 1): ["30"],
