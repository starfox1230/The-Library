(function () {
  ...
  const AUTO_LOW_RESOURCE_SATELLITE_THRESHOLD = 130;
  ...
  function getSatelliteCount(data) {
    const colors = Array.isArray(data?.satelliteColors) ? data.satelliteColors : [];
    return colors.length;
  }

  function shouldAutoUseUltraLowResource(data) {
    return getVisualMode(data) === "sphere"
      && getRenderMode(data) !== "ultra_low_resource"
      && getSatelliteCount(data) >= AUTO_LOW_RESOURCE_SATELLITE_THRESHOLD;
  }

  function getActiveRenderMode(data) {
    return shouldAutoUseUltraLowResource(data) ? "ultra_low_resource" : getRenderMode(data);
  }
  ...
  function isReducedRenderMode(data) {
    return isLightweightRowsMode(data) || getActiveRenderMode(data) !== "classic";
  }
  ...
  function getTimerStepMs(data) {
    ...
    return getActiveRenderMode(data) === "ultra_low_resource" ? 500 : 100;
  }
  ...
  function syncQuickControls(data) {
    ...
    const renderMode = getActiveRenderMode(data);
    const autoLowResource = shouldAutoUseUltraLowResource(data);
    ...
    syncQuickControl(sphereButton, visualMode === "sphere", autoLowResource ? "Satellite view auto-throttled for performance" : sphereResourceActive ? "Satellite view with reduced resources" : "Satellite view");
    ...
    syncQuickControl(
      sphereUltraToggle,
      sphereUltraActive,
      autoLowResource ? "Ultra low resource auto-throttle active" : sphereUltraActive ? "Ultra low resource on" : "Ultra low resource"
    );
    syncQuickControl(
      $("acgHapticsToggle"),
      hapticsEnabled,
  ...
  function handleStateEffects(data) {
    ...
    const sphereMode = getSphereMode(data);
    const ultraLowResource = getActiveRenderMode(data) === "ultra_low_resource";
  ...
  function render(data) {
    ...
    sidebar.classList.toggle("auto-low-resource", enabled && visualsEnabled && visualMode === "sphere" && autoLowResource);
