...
VISUAL_COLOR_FIELDS = [
    ("core", "Sphere: Central Orb", "Only the Sphere's central orb, rings, and related glow."),
...
class SettingsDialog(QDialog):
    ...
    def _build_appearance_section(self, parent: QWidget) -> QWidget:
        ...
        preview_layout.setSpacing(8)
        preview_title = QLabel("Color Preview", preview)
        preview_title.setObjectName("speedStreakPreviewTitle")
        self.preview_swatches: dict[str, QWidget] = {}
        preview_layout.addWidget(preview_title)
        for group_title, color_items in (
            ("Visuals", (("core", "Sphere"), ("crystal", "Crystal"))),
            ("Ratings (shared)", (("red", "Again"), ("yellow", "Hard"), ("green", "Good"), ("blue", "Easy"))),
        ):
            group_label = QLabel(group_title, preview)
            group_label.setProperty("class", "fieldLabel")
            preview_layout.addWidget(group_label)
            swatch_grid = QGridLayout()
            swatch_grid.setContentsMargins(0, 0, 0, 0)
            swatch_grid.setHorizontalSpacing(12)
            swatch_grid.setVerticalSpacing(7)
            for index, (color_key, swatch_label) in enumerate(color_items):
                chip = QWidget(preview)
                chip_layout = QHBoxLayout(chip)
                chip_layout.setContentsMargins(0, 0, 0, 0)
                chip_layout.setSpacing(6)
                if color_key in {"core", "crystal"}:
                    swatch = OrbPreviewButton(color_key, preview)
                    swatch.setFixedSize(34, 28)
                    swatch.setCursor(Qt.CursorShape.PointingHandCursor)
                    swatch.setToolTip("Open Visual Colors")
                    swatch.clicked.connect(lambda _checked=False: self.open_color_picker())
                else:
                    swatch = QFrame(preview)
                    swatch.setProperty("swatch", "true")
                    swatch.setFixedSize(24, 16)
                label_widget = QLabel(swatch_label, preview)
                label_widget.setProperty("class", "helpText")
                chip_layout.addWidget(swatch)
                chip_layout.addWidget(label_widget)
                chip_layout.addStretch(1)
                self.preview_swatches[color_key] = swatch
                swatch_grid.addWidget(chip, index // 2, index % 2)
            preview_layout.addLayout(swatch_grid)
