...
DEFAULT_AUDIO_EVENT_FILES = {
    "sync": "kenney_impact-sounds/Audio/footstep_grass_004.wav",
    "reveal": "kenney_rpg-audio/Audio/drawKnife2.wav",
    "again": "kenney_impact-sounds/Audio/impactSoft_heavy_000.wav",
    "hard": "kenney_impact-sounds/Audio/impactPlank_medium_002.wav",
    "good": "kenney_casino-audio/Audio/chips-handle-3.wav",
    "easy": "kenney_impact-sounds/Audio/impactMining_002.wav",
    "skip": "kenney_rpg-audio/Audio/bookFlip2.wav",
    "reset": "kenney_ui-audio/Audio/switch9.wav",
    "timeout": "kenney_impact-sounds/Audio/impactBell_heavy_000.wav",
}

DEFAULT_AUDIO_VOLUME_PERCENT = 100
MIN_AUDIO_VOLUME_PERCENT = 0
MAX_AUDIO_VOLUME_PERCENT = 200
...
def normalize_audio_volume_percent(value: Any) -> int:
    try:
        numeric = int(round(float(value)))
    except (TypeError, ValueError):
        numeric = DEFAULT_AUDIO_VOLUME_PERCENT
    return max(MIN_AUDIO_VOLUME_PERCENT, min(MAX_AUDIO_VOLUME_PERCENT, numeric))


def default_audio_event_volumes() -> dict[str, int]:
    return {
        item["event"]: DEFAULT_AUDIO_VOLUME_PERCENT
        for item in HAPTIC_EVENT_OPTIONS
    }


def normalize_audio_event_volumes(value: Any) -> dict[str, int]:
    source = value if isinstance(value, dict) else {}
    return {
        item["event"]: normalize_audio_volume_percent(
            source.get(item["event"], DEFAULT_AUDIO_VOLUME_PERCENT)
        )
        for item in HAPTIC_EVENT_OPTIONS
    }
