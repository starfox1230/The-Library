...
class MacHapticsDiagnosticsTests(unittest.TestCase):
    ...
    def test_browser_fallback_is_healthy_and_manifest_reports_v204(self) -> None:
        module = _load_module()
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "1237336370"
            root.mkdir()
            (root / "manifest.json").write_text(
                json.dumps({"package": "speed_streak_v2_04"}),
                encoding="utf-8",
            )
        ...
        summary, report = module.render_mac_haptics_report(payload)
        self.assertEqual(payload["addonVersion"], "2.04")
