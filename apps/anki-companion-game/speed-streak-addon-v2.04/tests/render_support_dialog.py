...
def _load_support_dialog() -> ModuleType:
    package_name = "speed_streak_v2_04_support_render"
    package = ModuleType(package_name)
    package.__path__ = [str(ROOT)]  # type: ignore[attr-defined]
    sys.modules[package_name] = package
    for name in ("settings_components", "support_dialog"):
    ...
    return sys.modules[f"{package_name}.support_dialog"]


def main() -> None:
    _install_aqt_qt_shim()
    module = _load_support_dialog()
    application = QtWidgets.QApplication.instance() or QtWidgets.QApplication([])
    application.setFont(QtGui.QFont("Arial", 10))
    dialog = module.SupportDialog()
    dialog.show()
    application.processEvents()
    output = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / "tests" / "support-dialog.png"
