...
class ReviewerOverlayController:
    ...
    def on_js_message(self, handled: tuple[bool, Any], message: str, context: Any) -> tuple[bool, Any]:
        ...
        if message.startswith("speed-streak:open-settings:visual-colors:"):
            ...
            if visual not in {"sphere", "satellite", "crystal", "crystal_reactor", "brick", "lightweight_rows", "singularity"}:
                visual = "sphere"
        ...
        if message.startswith("speed-streak:update-settings:"):
            ...
            try:
                ...
                elif visual_mode == VISUAL_MODE_SINGULARITY:
                    reduced_motion_enabled = False
            except Exception:
                return (True, None)
    ...
    def apply_settings_from_dialog(
        ...
        *,
        ...
        elif self.visual_mode == VISUAL_MODE_SINGULARITY:
            self.engine.state.reduced_motion_enabled = False
        self.haptics.set_enabled(self.engine.state.haptics_enabled)
        if self.engine.state.no_undo_mode and not previous_no_undo_mode:
            self._clear_anki_undo_history()
