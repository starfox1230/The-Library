...
class SettingsDialog(QDialog):
    ...
    def _build_feedback_section(self, parent: QWidget) -> QWidget:
        ...
        haptics_group, haptics_layout = self._build_feedback_panel(frame, "Haptics", expanded=True)
        ...
        lab_group = QFrame(haptics_group)
        lab_group.setProperty("class", "buttonRowGroup")
        lab_layout = QVBoxLayout(lab_group)
        lab_layout.setContentsMargins(12, 12, 12, 12)
        lab_layout.setSpacing(10)
        lab_title = QLabel("Haptics Lab", lab_group)
        lab_title.setProperty("class", "fieldLabel")
        lab_copy = QLabel(
            "Use these one-off tests to feel patterns before assigning them to review events.",
            lab_group,
        )
        lab_copy.setWordWrap(True)
        lab_copy.setProperty("class", "helpText")
        lab_layout.addWidget(lab_title)
        lab_layout.addWidget(lab_copy)
        for group_name, pattern_keys in HAPTIC_LAB_GROUPS:
            group_label = QLabel(group_name, lab_group)
            group_label.setProperty("class", "fieldLabel")
            lab_layout.addWidget(group_label)
            grid = QGridLayout()
            ...
            for index, pattern_key in enumerate(pattern_keys):
                ...
                button = QPushButton(label, lab_group)
                button.setProperty("class", "secondaryAction")
            ...
            lab_layout.addLayout(grid)
        haptics_layout.addWidget(lab_group)
