param(
    [string]$OutputName = "speed_streak_v1_22.ankiaddon"
)
...
if (-not (Test-Path $zipPath)) {
    ...
    if (-not $tarCommand) {
        Remove-Item -Path $staging -Recurse -Force
        Write-Error "Failed to create package with Compress-Archive, and tar.exe was not available for fallback packaging."
        exit 1
    }
    & $tarCommand.Source -a -cf $zipPath -C $staging .
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path $zipPath)) {
        Remove-Item -Path $staging -Recurse -Force
        Write-Error "Failed to create package with Compress-Archive or tar.exe."
        exit 1
    }
}
