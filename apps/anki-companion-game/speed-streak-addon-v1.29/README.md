...
This folder contains the Speed Streak v1.29 version of the project packaged as an Anki add-on.
...
<ul>
  <li>Optimized the <strong>WebGL satellite renderer</strong> so the GPU animates static streak data and all background rendering suspends when it cannot be seen</li>
...
- Moves satellite orbit calculation into the vertex shader, replacing per-frame JavaScript position calculation and full GPU buffer uploads with one static upload whenever the streak changes.
- Uses adaptive frame pacing without adding settings: 60 FPS briefly after a streak change, 30 FPS while the scene is active, and 15 FPS during idle ambience.
- Suspends WebGL, live side-panel timer work, and looping CSS scene animations while the pane is hidden or collapsed, the run is paused, Settings is open, review has ended, the document is hidden, or Anki is not the active application.
- Caps the WebGL side-panel timer animation at 30 FPS while preserving its continuous countdown appearance.
- Keeps the existing Sphere, Consolidate, Brick, theme, timer, streak, score, and milestone behavior unchanged.
...
3. Close Anki.
4. In the folder that opens, create a new folder named `speed_streak_v1_29`.
...
`v1.29` keeps the `Brick Layout` visual mode, the optional deck-page Review Later count button, the WebGL sphere renderer, the improved Windows haptics path, and the Haptics Lab while reducing background rendering work.
...
The Windows installer removes previous `speed_streak_v1_28`, `speed_streak_v1_27`, `speed_streak_v1_26`, `speed_streak_v1_25`, `speed_streak_v1_24`, `speed_streak_v1_23`, `speed_streak_v1_22`, `speed_streak_v1_21`, `speed_streak_v1_20`, historical `speed_streak_v2_0`, and AnkiWeb `1237336370` folders if present so v1.29 replaces them on the next Anki restart. Speed Streak keeps its mutable data in the current Anki profile's `addons-data/speed_streak` folder. The installer still preserves a legacy `user_files` folder so older installs can migrate forward safely.
