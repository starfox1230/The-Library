(function () {
  ...
  const WEBGL_BURST_DURATION_MS = 1200;
  const WEBGL_ACTIVE_DURATION_MS = 5000;
  const WEBGL_BURST_FRAME_INTERVAL_MS = 1000 / 60;
  const WEBGL_ACTIVE_FRAME_INTERVAL_MS = 1000 / 30;
  const WEBGL_IDLE_FRAME_INTERVAL_MS = 1000 / 15;
  const TIMER_FRAME_INTERVAL_MS = 1000 / 30;
  ...
  function ensureMounted() {
    ...
    document.addEventListener("visibilitychange", syncRenderingActivity);
  ...
  function isBaseSurfaceActive(data = state.data) {
    if (!state.surfaceVisible || document.hidden || Number(data?.appActive ?? 1) <= 0) {
      return false;
    }
  ...
  function isSidebarCollapsed(data = state.data) {
    return String(data?.displayMode || "inline") !== "compatibility" && Boolean(data?.sidebarCollapsed);
  }

  function isSceneAnimationActive(data = state.data) {
    return Boolean(
      data
      && isBaseSurfaceActive(data)
      && data.enabled
      && data.visualsEnabled
      && data.orbitAnimationEnabled
      && !data.paused
      && !state.settingsOpen
  ...
  function isWebglOrbitAnimationActive(data = state.data) {
    return isSceneAnimationActive(data) && getRenderMode(data) === "webgl";
  }

  function isTimerSurfaceActive(data = state.data) {
    return Boolean(
      data
      && isBaseSurfaceActive(data)
      && data.enabled
  ...
  function syncRenderingActivity() {
    const sidebar = $("speed-streak-sidebar");
    const sceneActive = isSceneAnimationActive();
    sidebar?.classList.toggle("rendering-suspended", !sceneActive);

    if (isWebglOrbitAnimationActive()) {
      resumeWebglOrbit();
    } else {
      suspendWebglOrbit();
    }

    if (isTimerSurfaceActive()) {
      resumeTimerRingAnimation();
    } else {
      suspendTimerRingAnimation();
    }
    syncTimerLoop();
  }
  ...
  function needsLiveTimerLoop(data) {
    if (!data || !isTimerSurfaceActive(data) || !Number(data.enabled || 0) || !Number(data.visualsEnabled || 0)) {
      return false;
    }
    const phase = String(data.phase || "idle");
  ...
  function createWebglProgram(gl) {
    const vertex = createShader(gl, gl.VERTEX_SHADER, `
      ...
      void main() {
        float theta = radians(a_angle) + ((u_time / max(a_duration, 0.001)) * 6.28318530718);
        vec2 position = vec2(cos(theta), sin(theta)) * a_radius;
  ...
  function ensureWebglRenderer() {
    ...
    try {
      ...
      const renderer = {
        ...
        durationLocation: gl.getAttribLocation(program, "a_duration"),
  ...
  function scheduleTimerRingFrame(renderer) {
    if (!renderer.running || renderer.frameId || renderer.frameTimerId) {
      return;
    }
    const elapsed = Math.max(0, performance.now() - Number(renderer.lastDrawAt || 0));
    const delay = Math.max(0, TIMER_FRAME_INTERVAL_MS - elapsed - 4);
    const requestFrame = () => {
      renderer.frameTimerId = 0;
      renderer.frameId = window.requestAnimationFrame((timestamp) => {
        renderer.frameId = 0;
        drawTimerRingFrame(renderer, timestamp);
      });
    };
    if (delay > 1) {
      renderer.frameTimerId = window.setTimeout(requestFrame, delay);
    } else {
      requestFrame();
    }
  }

  function drawTimerRingFrame(renderer, timestamp = performance.now()) {
    const timer = renderer.timer;
    if (!renderer.running || !timer || !isTimerSurfaceActive()) {
      suspendTimerRingAnimation();
      return;
    }
    renderer.lastDrawAt = timestamp;
    ...
    if (timer.active && progress > 0) {
      scheduleTimerRingFrame(renderer);
    } else {
  ...
  function suspendTimerRingAnimation() {
    const renderer = state.timerWebgl;
    if (!renderer) {
      return;
    }
    renderer.running = false;
    ...
    if (renderer.frameTimerId) {
      window.clearTimeout(renderer.frameTimerId);
      renderer.frameTimerId = 0;
    }
  }

  function resumeTimerRingAnimation() {
    const renderer = state.timerWebgl;
    if (!renderer?.timer || !renderer.timer.active || renderer.running || !isTimerSurfaceActive()) {
      return;
    }
    renderer.running = true;
    renderer.lastDrawAt = 0;
  ...
  function syncTimerRingWebgl(timerHero, timer, color, progress, active) {
    ...
    renderer.running = isTimerSurfaceActive();
    renderer.lastDrawAt = 0;
    drawTimerRingFrame(renderer);
  }
  ...
  function uploadWebglSatellites(renderer, satellites, signature) {
    if (renderer.satelliteSignature === signature) {
      return false;
    }
    const values = new Float32Array(satellites.length * 8);
    satellites.forEach((satellite, index) => {
      const offset = index * 8;
      const rgb = satelliteRgb(satellite.color);
      values[offset] = satellite.angle;
      values[offset + 1] = satellite.radius;
      values[offset + 2] = satellite.duration;
      values[offset + 3] = 16;
      values[offset + 4] = rgb[0];
      values[offset + 5] = rgb[1];
      values[offset + 6] = rgb[2];
      values[offset + 7] = rgb[3];
    });
    const gl = renderer.gl;
    gl.bindBuffer(gl.ARRAY_BUFFER, renderer.buffer);
    gl.bufferData(gl.ARRAY_BUFFER, values, gl.STATIC_DRAW);
    renderer.satelliteCount = satellites.length;
    renderer.satelliteSignature = signature;
    const now = performance.now();
    renderer.burstUntil = now + WEBGL_BURST_DURATION_MS;
    renderer.activeUntil = now + WEBGL_ACTIVE_DURATION_MS;
    return true;
  }

  function webglFrameInterval(renderer, now = performance.now()) {
    if (now < renderer.burstUntil) {
      return WEBGL_BURST_FRAME_INTERVAL_MS;
    }
    if (now < renderer.activeUntil) {
      return WEBGL_ACTIVE_FRAME_INTERVAL_MS;
    }
    return WEBGL_IDLE_FRAME_INTERVAL_MS;
  }

  function scheduleWebglFrame(renderer) {
    if (!renderer.running || renderer.frameId || renderer.frameTimerId) {
      return;
    }
    const now = performance.now();
    const interval = webglFrameInterval(renderer, now);
    const elapsed = Math.max(0, now - Number(renderer.lastDrawAt || 0));
    const delay = Math.max(0, interval - elapsed - 4);
    const requestFrame = () => {
      renderer.frameTimerId = 0;
      renderer.frameId = window.requestAnimationFrame((timestamp) => {
        renderer.frameId = 0;
        drawWebglFrame(renderer, timestamp);
      });
    };
    if (delay > 1) {
      renderer.frameTimerId = window.setTimeout(requestFrame, delay);
  ...
  function drawWebglFrame(renderer, timestamp = performance.now()) {
    if (!renderer.running || !renderer.requested || !isWebglOrbitAnimationActive()) {
      suspendWebglOrbit();
      return;
    }
    renderer.lastDrawAt = timestamp;
    ...
    if (renderer.satelliteCount > 0) {
      ...
      gl.bindBuffer(gl.ARRAY_BUFFER, renderer.buffer);
      const stride = 8 * 4;
      gl.enableVertexAttribArray(renderer.angleLocation);
      gl.vertexAttribPointer(renderer.angleLocation, 1, gl.FLOAT, false, stride, 0);
      gl.enableVertexAttribArray(renderer.radiusLocation);
      gl.vertexAttribPointer(renderer.radiusLocation, 1, gl.FLOAT, false, stride, 1 * 4);
      gl.enableVertexAttribArray(renderer.durationLocation);
      gl.vertexAttribPointer(renderer.durationLocation, 1, gl.FLOAT, false, stride, 2 * 4);
      gl.enableVertexAttribArray(renderer.sizeLocation);
      gl.vertexAttribPointer(renderer.sizeLocation, 1, gl.FLOAT, false, stride, 3 * 4);
      ...
      gl.uniform1f(renderer.timeLocation, timestamp / 1000);
      gl.drawArrays(gl.POINTS, 0, renderer.satelliteCount);
    }

    scheduleWebglFrame(renderer);
  }

  function suspendWebglOrbit() {
  ...
  function resumeWebglOrbit() {
    const renderer = state.webgl;
    if (!renderer?.requested || renderer.running || !isWebglOrbitAnimationActive()) {
      return;
    }
    renderer.running = true;
    renderer.lastDrawAt = 0;
  ...
  function stopWebglOrbit({ clear = true } = {}) {
    const renderer = state.webgl;
    if (!renderer) {
      return;
    }
    renderer.requested = false;
    suspendWebglOrbit();
    if (!clear) {
      return;
    }
  ...
  window.SpeedStreak = {
    ...
    setSurfaceVisible(visible) {
      state.surfaceVisible = Boolean(visible);
      syncRenderingActivity();
    },
  };
