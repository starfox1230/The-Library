...
ADDON_DISPLAY_NAME = "Speed Streak v1.29"
...
class ReviewerOverlayController:
    def __init__(self) -> None:
        ...
        self._application_active = self._is_application_active()
    ...
    def install(self) -> None:
        ...
        application_state_changed = getattr(application, "applicationStateChanged", None)
    ...
    @staticmethod
    def _is_application_active(application_state: Any = None) -> bool:
        try:
        ...
        if active_state is None:
            return True
        try:
            current_state = QApplication.applicationState() if application_state is None else application_state
            return current_state == active_state
        except Exception:
            return True

    def _on_application_state_changed(self, application_state: Any) -> None:
        active = self._is_application_active(application_state)
        if active == self._application_active:
            return
        ...
        payload = json.dumps(
            {
                **state,
                ...
                "appActive": int(self._application_active),
    ...
    def _set_sidebar_hidden(self, hidden: bool) -> None:
        ...
        visible_literal = "false" if hidden else "true"
        try:
            self._sidebar_web.eval(
                ...
                (() => {{
                  ...
                  if (window.SpeedStreak && typeof window.SpeedStreak.setSurfaceVisible === "function") {{
                    window.SpeedStreak.setSurfaceVisible({visible_literal});
                  }}
                }})()
                """
            )
    ...
    def _set_floating_sidebar_hidden(self, hidden: bool) -> None:
        ...
        if hidden:
            try:
                self._floating_window.web.eval(
                    "window.SpeedStreak && window.SpeedStreak.setSurfaceVisible(false);"
                )
            except Exception:
                pass
        ...
        try:
            self._floating_window.web.eval(
                "window.SpeedStreak && window.SpeedStreak.setSurfaceVisible(true);"
            )
        except Exception:
            pass
