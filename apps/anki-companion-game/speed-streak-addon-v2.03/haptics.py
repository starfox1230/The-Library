...
from .mac_haptics_bridge import MacHapticsBridge
from .mac_haptics_pyobjc import PyObjCMacHaptics
...
class HapticsController:
    def __init__(self) -> None:
        ...
        self._mac_pyobjc = PyObjCMacHaptics(Path(__file__).resolve().parent)
        self._mac = MacHapticsBridge(Path(__file__).resolve().parent)
        self._load_dll()

    @property
    def available(self) -> bool:
        return (
            self._mac_pyobjc.available
            or self._mac.available
            or self._steam.available
            or bool(self._connected_xinput_indices())
        )

    @property
    def backend_name(self) -> str:
        if sys.platform.startswith("darwin"):
            if self._mac_pyobjc.available:
                return self._mac_pyobjc.backend_name
            if self._mac.available:
                return self._mac.backend_name
            return self._mac_pyobjc.backend_name
        if self._steam.available:
            return "native-steam-input"
        if self._connected_xinput_indices():
            return "native-windows-xinput"
    ...
    def diagnostics(self) -> dict:
        if sys.platform.startswith("darwin"):
            return {
                **self._mac_pyobjc.diagnostics,
                "swiftHelperFallback": self._mac.diagnostics,
            }
        return {
            "backend": self.backend_name,
            "xinputControllerCount": len(self._connected_xinput_indices()),
            "steamInputAvailable": bool(self._steam.available),
        }

    def refresh_mac_diagnostics(self, timeout: float = 1.25) -> dict:
        """Return a fresh direct/native snapshot without changing output selection."""

        if sys.platform.startswith("darwin"):
            self._mac_pyobjc.refresh_controllers()
            self._mac.refresh_status(timeout=timeout)
    ...
    def _play_pattern(self, kind: str, *, ignore_user_enabled: bool) -> bool:
        if not ignore_user_enabled and not self._user_enabled:
            return False
        meta = HAPTIC_PATTERN_LIBRARY.get(str(kind or "").strip())
        raw_steps = list(meta.get("sequence", [])) if meta else []
        if self._mac_pyobjc.available:
            return self._mac_pyobjc.play_sequence(raw_steps)
        if self._mac.available:
            return self._mac.play_pattern(kind)
