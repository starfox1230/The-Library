...
OUTPUT_NAME="${1:-speed_streak_v2_03.ankiaddon}"
...
for item in "$SOURCE_DIR"/* "$SOURCE_DIR"/.[!.]* "$SOURCE_DIR"/..?*; do
  ...
  [ "$name" = ".." ] && continue
  case "$name" in
    .ankiaddon-build|.pytest_cache|__pycache__|tests|user_files|.DS_Store|install_to_anki.ps1|build_ankiaddon.ps1|install_to_anki.sh|build_ankiaddon.sh|trim_audio_to_trimmed.ps1|generate_web_assets.py|*.ankiaddon|*.zip)
      continue
      ;;
  esac
