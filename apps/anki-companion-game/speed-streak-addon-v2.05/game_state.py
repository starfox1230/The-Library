...
GAMEPLAY_MODE_LEGACY = "legacy"
GAMEPLAY_MODE_TIME_BOOST = "time_boost"
GAMEPLAY_MODES = {GAMEPLAY_MODE_LEGACY, GAMEPLAY_MODE_TIME_BOOST}
DEFAULT_BOOST_SECONDS = 10.0
DEFAULT_MAX_BOOST_CHARGES = 5
DEFAULT_STARTING_BOOST_CHARGES = 3
DEFAULT_CARDS_PER_BOOST_CHARGE = 10
...
def review_entry_allowance_after_state_change(
    pending: bool,
    *,
    new_state: str,
    old_state: str,
) -> bool:
    """Arm the next free card only after leaving Review.

    Anki may show the first question before its state_did_change hook announces
    that Review was entered. Re-arming on that late hook would make card two
    free as well.
    """
    if str(old_state or "") == "review" and str(new_state or "") != "review":
        return True
    return bool(pending)
...
class CompanionState:
    ...
    audio_event_volumes: dict[str, int] = field(default_factory=default_audio_event_volumes)
    countdown_audio_enabled: bool = DEFAULT_COUNTDOWN_AUDIO_ENABLED
    countdown_audio_file: str = DEFAULT_COUNTDOWN_AUDIO_FILE
    countdown_audio_volume: int = DEFAULT_AUDIO_VOLUME_PERCENT
    countdown_warning_seconds: int = DEFAULT_COUNTDOWN_WARNING_SECONDS
    countdown_audio_alignment_ms: int = DEFAULT_COUNTDOWN_AUDIO_ALIGNMENT_MS
    ...
    free_first_card_on_review_entry: bool = True
    answer_timeout_breaks_streak: bool = True
    gameplay_mode: str = GAMEPLAY_MODE_TIME_BOOST
    no_pause_mode: bool = False
    no_undo_mode: bool = False
    show_focus_mode_toggles: bool = True
    boost_seconds: float = DEFAULT_BOOST_SECONDS
    max_boost_charges: int = DEFAULT_MAX_BOOST_CHARGES
    starting_boost_charges: int = DEFAULT_STARTING_BOOST_CHARGES
    cards_per_boost_charge: int = DEFAULT_CARDS_PER_BOOST_CHARGE
    ...
    phase_base_limit_ms: int = 0
    phase_boost_remaining_ms: int = 0
    phase_boost_anchor_epoch_ms: int = 0
    ...
    boost_charges: int = DEFAULT_STARTING_BOOST_CHARGES
    boost_charge_progress: int = 0
    boosts_used: int = 0
    paused: bool = False
    ...
    time_drain_auto_pause_enabled: bool = False
    time_drain_auto_paused: bool = False
    time_drain_auto_pause_elapsed_ms: int = 0
...
class CompanionGameEngine:
    ...
    def _capture_user_preferences(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "audio_event_volumes": dict(s.audio_event_volumes),
            "countdown_audio_enabled": bool(s.countdown_audio_enabled),
            "countdown_audio_file": str(s.countdown_audio_file),
            "countdown_audio_volume": int(s.countdown_audio_volume),
            "countdown_warning_seconds": int(s.countdown_warning_seconds),
            "countdown_audio_alignment_ms": int(s.countdown_audio_alignment_ms),
            ...
            "free_first_card_on_review_entry": bool(s.free_first_card_on_review_entry),
            "answer_timeout_breaks_streak": bool(s.answer_timeout_breaks_streak),
            "gameplay_mode": str(s.gameplay_mode),
            "no_pause_mode": bool(s.no_pause_mode),
            "no_undo_mode": bool(s.no_undo_mode),
            "show_focus_mode_toggles": bool(s.show_focus_mode_toggles),
            "boost_seconds": float(s.boost_seconds),
            "max_boost_charges": int(s.max_boost_charges),
            "starting_boost_charges": int(s.starting_boost_charges),
            "cards_per_boost_charge": int(s.cards_per_boost_charge),
            ...
            "time_drain_auto_pause_enabled": bool(s.time_drain_auto_pause_enabled),
    ...
    def _restore_user_preferences(self, preferences: Dict[str, Any]) -> None:
        ...
        s.audio_event_volumes = normalize_audio_event_volumes(
            preferences.get("audio_event_volumes", s.audio_event_volumes)
        )
        s.countdown_audio_enabled = bool(
            preferences.get("countdown_audio_enabled", s.countdown_audio_enabled)
        )
        s.countdown_audio_file = str(
            preferences.get("countdown_audio_file", s.countdown_audio_file) or DEFAULT_COUNTDOWN_AUDIO_FILE
        )
        s.countdown_audio_volume = normalize_audio_volume_percent(
            preferences.get("countdown_audio_volume", s.countdown_audio_volume)
        )
        s.countdown_warning_seconds = normalize_countdown_warning_seconds(
            preferences.get("countdown_warning_seconds", s.countdown_warning_seconds)
        )
        s.countdown_audio_alignment_ms = normalize_countdown_alignment_ms(
            preferences.get("countdown_audio_alignment_ms", s.countdown_audio_alignment_ms)
        )
        ...
        s.free_first_card_on_review_entry = bool(
            preferences.get("free_first_card_on_review_entry", s.free_first_card_on_review_entry)
        )
        s.answer_timeout_breaks_streak = bool(
            preferences.get("answer_timeout_breaks_streak", s.answer_timeout_breaks_streak)
        )
        s.gameplay_mode = self._normalize_gameplay_mode(preferences.get("gameplay_mode", s.gameplay_mode))
        s.no_pause_mode = bool(preferences.get("no_pause_mode", s.no_pause_mode))
        s.no_undo_mode = bool(preferences.get("no_undo_mode", s.no_undo_mode))
        s.show_focus_mode_toggles = bool(
            preferences.get("show_focus_mode_toggles", s.show_focus_mode_toggles)
        )
        s.boost_seconds = max(0.5, min(120.0, float(preferences.get("boost_seconds", s.boost_seconds))))
        s.max_boost_charges = max(1, min(20, int(preferences.get("max_boost_charges", s.max_boost_charges))))
        s.starting_boost_charges = max(
            0,
            min(s.max_boost_charges, int(preferences.get("starting_boost_charges", s.starting_boost_charges))),
        )
        s.cards_per_boost_charge = max(
            1,
            min(100, int(preferences.get("cards_per_boost_charge", s.cards_per_boost_charge))),
        )
        s.boost_charges = max(0, min(s.max_boost_charges, int(s.boost_charges)))
        s.boost_charge_progress = max(0, min(s.cards_per_boost_charge - 1, int(s.boost_charge_progress)))
        ...
        s.custom_colors = self._normalize_custom_colors(
            preferences.get("custom_colors", s.custom_colors),
            migrate_legacy_crystal=True,
        )
        ...
        s.time_drain_auto_pause_enabled = bool(
            preferences.get("time_drain_auto_pause_enabled", s.time_drain_auto_pause_enabled)
        )
        ...
        if s.phase == "question":
            ...
            s.phase_limit_ms = 0
        s.phase_base_limit_ms = s.phase_limit_ms
        s.phase_boost_remaining_ms = 0
        s.phase_boost_anchor_epoch_ms = 0

        if not s.visuals_enabled:
            self._clear_pause_state(accumulate_active=True)
            self._clear_time_drain_auto_pause()
            s.streak = 0
    ...
    def export(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "audioEventVolumes": dict(s.audio_event_volumes),
            "countdownAudioEnabled": int(s.countdown_audio_enabled),
            "countdownAudioFile": str(s.countdown_audio_file),
            "countdownAudioVolume": int(s.countdown_audio_volume),
            "countdownWarningSeconds": int(s.countdown_warning_seconds),
            "countdownAudioAlignmentMs": int(s.countdown_audio_alignment_ms),
            ...
            "freeFirstCardOnReviewEntry": int(s.free_first_card_on_review_entry),
            "answerTimeoutBreaksStreak": int(s.answer_timeout_breaks_streak),
            "gameplayMode": s.gameplay_mode,
            "noPauseMode": int(s.no_pause_mode),
            "noUndoMode": int(s.no_undo_mode),
            "showFocusModeToggles": int(s.show_focus_mode_toggles),
            "boostSeconds": round(s.boost_seconds, 1),
            "maxBoostCharges": s.max_boost_charges,
            "startingBoostCharges": s.starting_boost_charges,
            "cardsPerBoostCharge": s.cards_per_boost_charge,
            ...
            "phaseBaseLimitMs": s.phase_base_limit_ms,
            "phaseBoostRemainingMs": s.phase_boost_remaining_ms,
            "phaseBoostAnchorEpochMs": s.phase_boost_anchor_epoch_ms,
            ...
            "boostCharges": s.boost_charges,
            "boostChargeProgress": s.boost_charge_progress,
            "boostsUsed": s.boosts_used,
            "canUseTimeBoost": int(self.can_use_time_boost()),
            "timeBoostUnavailableReason": self.time_boost_unavailable_reason(),
            "paused": int(s.paused),
            ...
            "timeDrainAutoPauseEnabled": int(s.time_drain_auto_pause_enabled),
            "timeDrainAutoPaused": int(s.time_drain_auto_paused),
            "timeDrainAutoPauseElapsedMs": int(s.time_drain_auto_pause_elapsed_ms),
    ...
    def export_runtime(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "phase_base_limit_ms": int(s.phase_base_limit_ms),
            "phase_boost_remaining_ms": int(s.phase_boost_remaining_ms),
            "phase_boost_anchor_epoch_ms": int(s.phase_boost_anchor_epoch_ms),
            ...
            "boost_charges": int(s.boost_charges),
            "boost_charge_progress": int(s.boost_charge_progress),
            "boosts_used": int(s.boosts_used),
            ...
            "time_drain_auto_paused": bool(s.time_drain_auto_paused),
            "time_drain_auto_pause_elapsed_ms": int(s.time_drain_auto_pause_elapsed_ms),
            "current_timer_policy_source": str(s.current_timer_policy_source),
            "current_timer_policy_mode": str(s.current_timer_policy_mode),
            "current_timer_policy_question_extra_ms": int(s.current_timer_policy_question_extra_ms),
            "current_timer_policy_answer_extra_ms": int(s.current_timer_policy_answer_extra_ms),
    ...
    def restore_runtime(self, runtime: Dict[str, Any]) -> None:
        ...
        s.phase_base_limit_ms = max(
            0,
            min(
                s.phase_limit_ms,
                int(runtime.get("phase_base_limit_ms", s.phase_limit_ms) or 0),
            ),
        )
        s.phase_boost_remaining_ms = max(0, int(runtime.get("phase_boost_remaining_ms", 0) or 0))
        s.phase_boost_anchor_epoch_ms = max(0, int(runtime.get("phase_boost_anchor_epoch_ms", 0) or 0))
        ...
        s.boost_charges = max(
            0,
            min(s.max_boost_charges, int(runtime.get("boost_charges", s.starting_boost_charges) or 0)),
        )
        s.boost_charge_progress = max(
            0,
            min(s.cards_per_boost_charge - 1, int(runtime.get("boost_charge_progress", 0) or 0)),
        )
        s.boosts_used = max(0, int(runtime.get("boosts_used", 0) or 0))
        ...
        s.time_drain_auto_paused = bool(runtime.get("time_drain_auto_paused", False))
        s.time_drain_auto_pause_elapsed_ms = max(
            0, int(runtime.get("time_drain_auto_pause_elapsed_ms", 0) or 0)
        )
        ...
        if s.phase == "idle" or not s.current_card_id:
            ...
            s.phase_limit_ms = 0
            s.phase_base_limit_ms = 0
            s.phase_boost_remaining_ms = 0
            s.phase_boost_anchor_epoch_ms = 0
            s.paused = False
            s.paused_remaining_ms = 0
            s.time_drain_auto_paused = False
            s.time_drain_auto_pause_elapsed_ms = 0
        elif not s.time_drain_auto_pause_enabled or not self._is_time_drain_card():
            self._clear_time_drain_auto_pause()
    ...
    def hard_reset(self) -> None:
        ...
        self.state.boost_charges = self.state.starting_boost_charges
        self.state.boost_charge_progress = 0
        self.state.boosts_used = 0
    ...
    def reset_settings_to_defaults(self) -> str:
        ...
        s.time_drain_auto_pause_enabled = False
        self._clear_time_drain_auto_pause()
        ...
        s.audio_event_volumes = default_audio_event_volumes()
        s.countdown_audio_enabled = DEFAULT_COUNTDOWN_AUDIO_ENABLED
        s.countdown_audio_file = DEFAULT_COUNTDOWN_AUDIO_FILE
        s.countdown_audio_volume = DEFAULT_AUDIO_VOLUME_PERCENT
        s.countdown_warning_seconds = DEFAULT_COUNTDOWN_WARNING_SECONDS
        s.countdown_audio_alignment_ms = DEFAULT_COUNTDOWN_AUDIO_ALIGNMENT_MS
        ...
        s.free_first_card_on_review_entry = True
        s.answer_timeout_breaks_streak = True
        s.gameplay_mode = GAMEPLAY_MODE_TIME_BOOST
        s.no_pause_mode = False
        s.no_undo_mode = False
        s.show_focus_mode_toggles = True
        s.boost_seconds = DEFAULT_BOOST_SECONDS
        s.max_boost_charges = DEFAULT_MAX_BOOST_CHARGES
        s.starting_boost_charges = DEFAULT_STARTING_BOOST_CHARGES
        s.cards_per_boost_charge = DEFAULT_CARDS_PER_BOOST_CHARGE
        s.boost_charges = min(s.boost_charges, s.max_boost_charges)
        s.boost_charge_progress = min(s.boost_charge_progress, s.cards_per_boost_charge - 1)
    ...
    def set_test_streak(self, streak: int) -> str:
        """Set deterministic visual state for developer performance testing."""
        s = self.state
        next_streak = max(0, min(5000, int(streak or 0)))
        palette = ("red", "yellow", "green", "blue")
        s.streak = next_streak
        s.streak_multiplier = (
            1.0 if s.gameplay_mode == GAMEPLAY_MODE_TIME_BOOST else self._multiplier_for_streak(next_streak)
        )
        s.satellite_colors = [palette[index % len(palette)] for index in range(next_streak)]
        s.last_satellite_color = s.satellite_colors[-1] if s.satellite_colors else "green"
        s.failure_visual_active = False
        self._publish("settings", f"Developer test streak set to {next_streak}.")
        return "settings"
    ...
    def sync_on_question(
        ...
        *,
        ...
        s.answer_shown = False
        s.first_card_free = bool(is_free)
        s.failure_visual_active = False
        self.begin_question_phase(is_free=bool(is_free))
        self._sync_time_drain_auto_pause_for_current_card()
        if is_free:
            self._publish("sync", "Review started. First card is free.")
        else:
            self._publish("sync", "Synced on question side.")
    ...
    def sync_visible_question_surface(
        ...
        *,
        ...
        card_flag: int = 0,
        timer_policy: dict[str, Any] | None = None,
        review_entry: bool = False,
    ) -> str:
        s = self.state
        self.set_current_timer_policy(**(timer_policy or {}))
        entry_is_free = bool(review_entry and s.free_first_card_on_review_entry)
        if not s.synced and s.enabled:
            self.sync_on_question(
                card_id=card_id,
                deck_name=deck_name,
                card_flag=card_flag,
                timer_policy=timer_policy,
                is_free=entry_is_free,
            )
            return "sync"
        ...
        same_question_resync = bool(
            previous_card_id == str(card_id)
            and s.phase == "question"
            and not s.answer_shown
        )
        s.previous_card_flag = s.current_card_flag
        s.current_card_id = card_id
        s.deck_name = deck_name
    ...
    def sync_visible_answer_surface(
        ...
        *,
        ...
        previous_card_id = str(s.current_card_id or "")
        current_card_is_free = bool(previous_card_id == str(card_id) and self.card.question_free)
        s.previous_card_flag = s.current_card_flag
        s.current_card_id = card_id
        s.deck_name = deck_name
        s.current_card_flag = int(card_flag or 0)
        s.answer_shown = True
        s.first_card_free = current_card_is_free
        s.failure_visual_active = False
        s.skip_pending = False
        ...
        self.card = CardRuntime(
            question_elapsed_ms=question_elapsed_ms,
            question_free=current_card_is_free,
            answer_elapsed_ms=0,
        )
    ...
    def on_flag_changed(self, *, card_flag: int) -> Optional[str]:
        ...
        if self._is_time_drain_card():
            self._sync_time_drain_auto_pause_for_current_card()
        else:
            self._clear_time_drain_auto_pause(resume_timer=True)
    ...
    def on_answer_shown(self) -> Optional[str]:
        ...
        s.answer_shown = True
        s.first_card_free = bool(self.card.question_free)
        self.begin_answer_phase()
    ...
    def on_rate(self, ease: int) -> Optional[str]:
        ...
        # Rating the answer completes the card. Boost progress deliberately
        # does not depend on whether either side was timed or finished before
        # its deadline; every completed card follows the same earning rule.
        charge_earned = False
        charge_detail = ""
        if s.gameplay_mode == GAMEPLAY_MODE_TIME_BOOST:
            charge_earned = self._advance_boost_charge_progress()
            charge_detail = (
                f"Boost earned ({s.boost_charges}/{s.max_boost_charges})"
                if charge_earned
                else (
                    f"Boosts full ({s.boost_charges}/{s.max_boost_charges})"
                    if s.boost_charges >= s.max_boost_charges
                    else f"Boost {s.boost_charge_progress}/{s.cards_per_boost_charge}"
                )
            )
        ...
        if s.visuals_enabled:
            ...
            if s.gameplay_mode == GAMEPLAY_MODE_TIME_BOOST:
                s.streak_multiplier = 1.0
                event_text = f"{rating_name}: streak {s.streak} | {charge_detail}"
            else:
    ...
    def clear_review_undo_history(self) -> None:
        self._review_undo_stack.clear()
    ...
    def notify_undo_blocked(self) -> str:
        self._publish("undo-blocked", "No Undo mode is active.")
        return "undo-blocked"
    ...
    def begin_answer_phase(self) -> None:
        s = self.state
        self._clear_pause_state(accumulate_active=True)
        if not self._is_time_drain_card():
            self._clear_time_drain_auto_pause()
        elif s.time_drain_auto_paused:
            s.time_drain_auto_pause_elapsed_ms = 0
        s.phase = "answer"
        s.phase_start_epoch_ms = now_epoch_ms()
        s.phase_limit_ms = self._answer_phase_limit_ms()
    ...
    def toggle_pause(self, *, count_in_stats: bool = True) -> Optional[str]:
        ...
        # No Pause is a focus rule for deliberate/manual pauses. The controller
        # passes count_in_stats=False for safety pauses while Anki leaves Review.
        # A safety-paused timer must always be resumable on review re-entry.
        if s.no_pause_mode and count_in_stats and not s.paused:
            self._publish("pause-blocked", "No Pause mode is active.")
            return "pause-blocked"

        if s.paused:
            s.paused = False
            resume_ms = now_epoch_ms()
            if s.paused_remaining_ms > 0:
                s.phase_start_epoch_ms = resume_ms - (s.phase_limit_ms - s.paused_remaining_ms)
            if s.phase_boost_remaining_ms > 0:
                s.phase_boost_anchor_epoch_ms = resume_ms
        ...
        s.phase_boost_remaining_ms = self._current_phase_boost_remaining_ms(now_ms)
        s.phase_boost_anchor_epoch_ms = 0
        s.paused = True
    ...
    def set_time_drain_auto_pause_enabled(self, enabled: bool) -> Optional[str]:
        """Enable the non-blocking Time Drain timer freeze preference."""
        s = self.state
        next_enabled = bool(enabled)
        changed = s.time_drain_auto_pause_enabled != next_enabled
        s.time_drain_auto_pause_enabled = next_enabled
        self._sync_time_drain_auto_pause_for_current_card()
        if not changed:
            return None
        self._publish(
            "time-drain-auto-pause",
            "Time Drain auto-pause turned on." if next_enabled else "Time Drain auto-pause turned off.",
        )
        return "time-drain-auto-pause"

    def _is_time_drain_card(self) -> bool:
        s = self.state
        return bool(
            s.time_drain_flag > 0
            and s.current_card_flag == s.time_drain_flag
            and s.current_timer_policy_source != "time_drain_flag"
            and s.current_timer_policy_mode != TIMER_POLICY_NO_TIMEOUT
        )

    def _sync_time_drain_auto_pause_for_current_card(self) -> None:
        s = self.state
        if (
            not s.time_drain_auto_pause_enabled
            or not s.enabled
            or not s.visuals_enabled
            or s.phase not in {"question", "answer"}
            or s.phase_limit_ms <= 0
            or not self._is_time_drain_card()
        ):
            self._clear_time_drain_auto_pause(resume_timer=True)
            return
        if not s.time_drain_auto_paused:
            s.time_drain_auto_pause_elapsed_ms = min(s.phase_limit_ms, self._phase_elapsed_ms())
            s.time_drain_auto_paused = True

    def _clear_time_drain_auto_pause(self, *, resume_timer: bool = False) -> None:
        s = self.state
        if resume_timer and s.time_drain_auto_paused and s.phase_start_epoch_ms > 0:
            elapsed_ms = max(0, int(s.time_drain_auto_pause_elapsed_ms))
            s.phase_start_epoch_ms = now_epoch_ms() - elapsed_ms
            s.timeout_phase_latch = ""
        s.time_drain_auto_paused = False
        s.time_drain_auto_pause_elapsed_ms = 0

    def update_gameplay_settings(
        self,
        *,
        gameplay_mode: str,
        no_pause_mode: bool,
        ...
        s = self.state
        previous_mode = s.gameplay_mode
        s.gameplay_mode = self._normalize_gameplay_mode(gameplay_mode)
        s.no_pause_mode = bool(no_pause_mode)
        s.no_undo_mode = bool(no_undo_mode)
        s.show_focus_mode_toggles = bool(show_focus_mode_toggles)
        s.boost_seconds = max(0.5, min(120.0, float(boost_seconds)))
        s.max_boost_charges = max(1, min(20, int(max_boost_charges)))
        s.starting_boost_charges = max(0, min(s.max_boost_charges, int(starting_boost_charges)))
        s.cards_per_boost_charge = max(1, min(100, int(cards_per_boost_charge)))
        s.boost_charges = max(0, min(s.max_boost_charges, int(s.boost_charges)))
        s.boost_charge_progress = max(0, min(s.cards_per_boost_charge - 1, int(s.boost_charge_progress)))

        if previous_mode != s.gameplay_mode and s.gameplay_mode == GAMEPLAY_MODE_TIME_BOOST:
            s.boost_charges = s.starting_boost_charges
            s.boost_charge_progress = 0
        if s.gameplay_mode == GAMEPLAY_MODE_TIME_BOOST:
            s.streak_multiplier = 1.0

        self._publish(
            "settings",
            "Gameplay mode updated to Time Boost." if s.gameplay_mode == GAMEPLAY_MODE_TIME_BOOST else "Gameplay mode updated to Legacy Points.",
        )
        return "settings"

    def set_focus_rule(self, rule: str, enabled: bool) -> Optional[str]:
        normalized = str(rule or "").strip().lower().replace("_", "-")
        if normalized == "no-pause":
            self.state.no_pause_mode = bool(enabled)
            label = "No Pause"
        elif normalized == "no-undo":
            self.state.no_undo_mode = bool(enabled)
            label = "No Undo"
        else:
            return None
        status = "on" if enabled else "off"
        self._publish("focus-rule", f"{label} mode {status}.")
        return "focus-rule"

    def time_boost_unavailable_reason(self) -> str:
        s = self.state
        if s.gameplay_mode != GAMEPLAY_MODE_TIME_BOOST:
            return "Time Boost mode is off."
        if not s.enabled:
            return "Speed Streak is off."
        if not s.visuals_enabled:
            return "Time Boost is unavailable while visuals are off."
        if not s.synced or s.phase not in {"question", "answer"}:
            return "Wait for the next timed card."
        if s.paused:
            return "Resume the timer before using a Boost."
        if s.first_card_free:
            return "The first card is free, no Boost needed."
        if s.current_timer_policy_mode == TIMER_POLICY_NO_TIMEOUT:
            return "This card is untimed."
        if s.phase_limit_ms <= 0 or s.phase_start_epoch_ms <= 0:
            return "This card has no active timer."
        if s.boost_charges <= 0:
            return "No Boosts are available."
        if self._phase_elapsed_ms() >= s.phase_limit_ms:
            return "The timer has already expired."
        return ""

    def can_use_time_boost(self) -> bool:
        return not self.time_boost_unavailable_reason()

    def notify_time_boost_unavailable(self) -> str:
        reason = self.time_boost_unavailable_reason() or "Time Boost is unavailable."
        self._publish("time-boost-blocked", reason)
        return "time-boost-blocked"

    def use_time_boost(self) -> Optional[str]:
        s = self.state
        if not self.can_use_time_boost():
            return None
        added_ms = max(500, int(round(s.boost_seconds * 1000)))
        boost_now_ms = now_epoch_ms()
        current_boost_ms = self._current_phase_boost_remaining_ms(boost_now_ms)
        s.boost_charges -= 1
        s.boosts_used += 1
        s.phase_limit_ms += added_ms
        s.phase_boost_remaining_ms = current_boost_ms + added_ms
        s.phase_boost_anchor_epoch_ms = boost_now_ms
        s.timeout_phase_latch = ""
        self._publish(
            "time-boost",
            f"Time Boost: +{s.boost_seconds:g}s | {s.boost_charges} Boost{'s' if s.boost_charges != 1 else ''} left.",
        )
        return "time-boost"

    def _current_phase_boost_remaining_ms(self, at_epoch_ms: int | None = None) -> int:
        s = self.state
        remaining = max(0, int(s.phase_boost_remaining_ms))
        anchor = max(0, int(s.phase_boost_anchor_epoch_ms))
        if s.paused or remaining <= 0 or anchor <= 0:
            return remaining
        current_ms = now_epoch_ms() if at_epoch_ms is None else int(at_epoch_ms)
        return max(0, remaining - max(0, current_ms - anchor))
    ...
    def update_time_limits(
        ...
        *,
        ...
        time_drain_auto_pause_enabled: bool | None = None,
        review_later_flag: int | None = None,
        audio_enabled: bool | None = None,
        selected_audio_file: str | None = None,
        audio_event_files: dict[str, str] | None = None,
        audio_event_volumes: dict[str, int] | None = None,
        countdown_audio_enabled: bool | None = None,
        countdown_audio_file: str | None = None,
        countdown_audio_volume: int | None = None,
        countdown_warning_seconds: int | None = None,
        countdown_audio_alignment_ms: int | None = None,
        ...
        free_first_card_on_review_entry: bool | None = None,
        answer_timeout_breaks_streak: bool | None = None,
        ...
        if time_drain_auto_pause_enabled is not None:
            s.time_drain_auto_pause_enabled = bool(time_drain_auto_pause_enabled)
            self._sync_time_drain_auto_pause_for_current_card()
        ...
        if audio_event_volumes is not None:
            s.audio_event_volumes = normalize_audio_event_volumes(audio_event_volumes)
        if countdown_audio_enabled is not None:
            s.countdown_audio_enabled = bool(countdown_audio_enabled)
        if countdown_audio_file is not None:
            s.countdown_audio_file = str(countdown_audio_file or DEFAULT_COUNTDOWN_AUDIO_FILE).strip()
        if countdown_audio_volume is not None:
            s.countdown_audio_volume = normalize_audio_volume_percent(countdown_audio_volume)
        if countdown_warning_seconds is not None:
            s.countdown_warning_seconds = normalize_countdown_warning_seconds(countdown_warning_seconds)
        if countdown_audio_alignment_ms is not None:
            s.countdown_audio_alignment_ms = normalize_countdown_alignment_ms(countdown_audio_alignment_ms)
        ...
        if free_first_card_on_review_entry is not None:
            s.free_first_card_on_review_entry = bool(free_first_card_on_review_entry)
        if answer_timeout_breaks_streak is not None:
            s.answer_timeout_breaks_streak = bool(answer_timeout_breaks_streak)
        ...
        self._publish(
            "settings",
            f"Timers updated: question {question_seconds:.1f}s, answer {answer_seconds:.1f}s, time drain flag {s.time_drain_flag}, time drain review last {'on' if s.time_drain_review_last else 'off'}, time drain auto-pause {'on' if s.time_drain_auto_pause_enabled else 'off'}, review later flag {s.review_later_flag}, audio {'on' if s.audio_enabled else 'off'}, haptics {'on' if s.haptics_enabled else 'off'}, visuals {'on' if s.visuals_enabled else 'off'}, orbit animation {'on' if s.orbit_animation_enabled else 'off'}, reduced motion {'on' if s.reduced_motion_enabled else 'off'}, appearance {s.appearance_mode}.",
        )
        return "settings"

    def _normalize_custom_colors(
        self,
        custom_colors: dict[str, str],
        *,
        migrate_legacy_crystal: bool = False,
    ) -> dict[str, str]:
        ...
        if migrate_legacy_crystal and "crystal" not in normalized and "core" in normalized:
            # Before the dedicated crystal swatch existed, Single Color
            # crystals inherited the sphere's central-orb color.
            normalized["crystal"] = normalized["core"]
        return normalized
    ...
    def check_timeout(self) -> Optional[str]:
        ...
        if s.current_timer_policy_mode == TIMER_POLICY_NO_TIMEOUT:
            return None
        if s.paused or s.time_drain_auto_paused:
            return None
        ...
        if s.phase == "answer" and not s.answer_timeout_breaks_streak:
            s.failure_visual_active = False
            self._publish("answer-timeout", "Answer timeout: streak preserved.")
    ...
    def _normalize_gameplay_mode(self, value: object) -> str:
        normalized = str(value or GAMEPLAY_MODE_TIME_BOOST).strip().lower()
        if normalized in {"boost", "time", "timeboost", GAMEPLAY_MODE_TIME_BOOST}:
            return GAMEPLAY_MODE_TIME_BOOST
        if normalized in GAMEPLAY_MODES:
            return normalized
        return GAMEPLAY_MODE_TIME_BOOST

    def _advance_boost_charge_progress(self) -> bool:
        s = self.state
        if s.boost_charges >= s.max_boost_charges:
            return False
        s.boost_charge_progress += 1
        if s.boost_charge_progress < s.cards_per_boost_charge:
            return False
        s.boost_charge_progress = 0
        s.boost_charges = min(s.max_boost_charges, s.boost_charges + 1)
        return True
    ...
    def _answer_phase_limit_ms(self) -> int:
        s = self.state
        if s.first_card_free or not s.visuals_enabled:
            return 0
        if s.current_timer_policy_mode == TIMER_POLICY_NO_TIMEOUT:
            return 0
    ...
    def _phase_elapsed_ms(self) -> int:
        ...
        if s.time_drain_auto_paused:
            return max(0, s.time_drain_auto_pause_elapsed_ms)
