param(
    [string]$AddonFolderName = "speed_streak_v2_05",
...
foreach ($folderName in @($AddonFolderName) + $PreviousAddonFolderNames) {
    if ([string]::IsNullOrWhiteSpace($folderName) -or $folderName -in @(".", "..") -or $folderName -match '[\\/:]') {
        throw "Add-on folder names must be simple folder names: $folderName"
    }
}
if (Get-Process -Name anki -ErrorAction SilentlyContinue) {
    throw "Close Anki before replacing Speed Streak."
}
$source = Split-Path -Parent $MyInvocation.MyCommand.Path
$ankiAddonsRoot = Join-Path $env:APPDATA "Anki2\addons21"
$target = Join-Path $ankiAddonsRoot $AddonFolderName
$validatedRoot = [IO.Path]::GetFullPath($ankiAddonsRoot).TrimEnd('\') + '\'
function Assert-AddonPath([string]$Path) {
    $resolved = [IO.Path]::GetFullPath($Path)
    if (-not $resolved.StartsWith($validatedRoot, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Refusing to modify a path outside addons21: $resolved"
    }
    if ((Test-Path -LiteralPath $resolved) -and ((Get-Item -LiteralPath $resolved -Force).Attributes -band [IO.FileAttributes]::ReparsePoint)) {
        throw "Refusing to replace an add-on symlink/junction: $resolved"
    }
}
Assert-AddonPath $target
...
foreach ($previousFolderName in $PreviousAddonFolderNames) {
    ...
    Assert-AddonPath $previousTarget
...
Get-ChildItem -Path $source -Force | Where-Object {
    $_.Name -notin @("install_to_anki.ps1", "apply_event_wav_fade.ps1", "generate_web_assets.py", ".DS_Store") -and
    $_.Name -notin @(".pytest_cache", "__pycache__", ".ankiaddon-build", "tests") -and
...
Get-ChildItem -LiteralPath $target -Directory -Recurse -Force | Where-Object {
    $_.Name -in @("__pycache__", ".pytest_cache", ".build-poc", ".build-release")
} | Sort-Object { $_.FullName.Length } -Descending | ForEach-Object {
    Remove-Item -LiteralPath $_.FullName -Recurse -Force
}
Get-ChildItem -LiteralPath $target -File -Recurse -Force | Where-Object {
    $_.Extension -eq ".pyc"
} | Remove-Item -Force
