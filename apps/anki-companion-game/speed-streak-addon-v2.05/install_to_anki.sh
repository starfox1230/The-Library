...
ADDON_FOLDER_NAME="${1:-speed_streak_v2_05}"
case "$ADDON_FOLDER_NAME" in
  ""|.|..|*/*|*\\*) echo "Add-on folder name must be a simple folder name." >&2; exit 1 ;;
esac
SOURCE_DIR="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
case "$(uname -s)" in
  Darwin) DEFAULT_ANKI_ROOT="$HOME/Library/Application Support/Anki2/addons21" ;;
  Linux) DEFAULT_ANKI_ROOT="${XDG_DATA_HOME:-$HOME/.local/share}/Anki2/addons21" ;;
  *) echo "Use install_to_anki.ps1 on Windows." >&2; exit 1 ;;
esac
ANKI_ADDONS_ROOT="${ANKI_ADDONS_ROOT:-$DEFAULT_ANKI_ROOT}"
...
for item in "$SOURCE_DIR"/* "$SOURCE_DIR"/.[!.]* "$SOURCE_DIR"/..?*; do
  ...
  case "$name" in
    .pytest_cache|tests|*.ankiaddon|*.zip)
      continue
      ;;
  esac
...
find "$TARGET_DIR" -type d \( -name __pycache__ -o -name .pytest_cache -o -name .build-poc -o -name .build-release \) -prune -exec rm -rf {} +
find "$TARGET_DIR" -type f -name '*.pyc' -delete
find "$TARGET_DIR" -type f -name 'SpeedStreakHaptics-diagnostic.json' -delete
