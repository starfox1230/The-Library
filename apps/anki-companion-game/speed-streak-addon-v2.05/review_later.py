...
def review_later_first_flagged_at(
    flag: int,
    card_ids: Iterable[int] | None = None,
) -> dict[int, datetime]:
    """Return the earliest tracked Review Later entry for each requested card.

    This is a small read-only integration API.  It deliberately exposes only
    Speed Streak's own cohort history; callers still decide how to present or
    filter the cards.
    """
    if flag <= 0:
        return {}
    requested = sorted({int(card_id) for card_id in (card_ids or []) if int(card_id) > 0})
    parameters: list[int] = [int(flag)]
    where = "flag = ?"
    if card_ids is not None:
        if not requested:
            return {}
        where += f" AND card_id IN ({','.join('?' for _ in requested)})"
        parameters.extend(requested)
    with _open_connection() as conn:
        rows = conn.execute(
            f"""
            SELECT card_id, MIN(entered_at) AS first_entered_at
            FROM review_later_cohorts
            WHERE {where}
            GROUP BY card_id
            """,
    ...
    return {
        int(row["card_id"]): _cohort_datetime(str(row["first_entered_at"]))
        for row in rows
...
def _collection_media_dir() -> Path | None:
    try:
        media = getattr(getattr(mw, "col", None), "media", None)
        media_dir = getattr(media, "dir", None)
        value = media_dir() if callable(media_dir) else media_dir
        if value:
...
def _local_media_file_url(raw_url: str, media_dir: Path | None) -> str | None:
    raw = unescape(str(raw_url or "").strip())
    if not raw:
        return None
    lowered = raw.lower()
    if raw.startswith("#") or lowered.startswith(
        ("data:", "http:", "https:", "file:", "mailto:", "javascript:", "anki:", "qrc:", "about:")
    ):
        return None

    match = re.match(r"(?P<path>[^?#]*)(?P<tail>[?#].*)?$", raw)
    if not match:
        return None
    path_part = unquote(match.group("path") or "").strip()
    ...
    if not path.is_absolute():
        if media_dir is None:
            return None
        path = media_dir / path_part
    try:
        return path.resolve().as_uri() + tail
    except Exception:
        return None


def rewrite_html_media_to_local_files(html: str) -> str:
    media_dir = _collection_media_dir()

    def replace(match: re.Match[str]) -> str:
        file_url = _local_media_file_url(match.group("url"), media_dir)
        if not file_url:
            return match.group(0)
        return f"{match.group('prefix')}{escape(file_url, quote=True)}{match.group('suffix')}"

    return _LOCAL_MEDIA_ATTR_RE.sub(replace, str(html or ""))


def preview_html(
    entries: list[ReviewLaterEntry],
    pending_card_removals: set[int] | None = None,
    *,
    ...
    if not entries:
        ...
        for entry in entries:
            ...
            pending_remove = interactive and entry.card_id in pending_card_removals
            ...
            actions_html = ""
            pending_copy_html = pending_copy if interactive else ""
            if interactive:
                actions_html = f"""
...
class ReviewLaterManagerDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        header_row.addWidget(self.deck_page_button_toggle)
        ...
        self.download_html_button = QPushButton("Download HTML Review")
        ...
        button_row.addWidget(self.download_html_button)
        ...
        self.download_html_button.clicked.connect(self.download_html_review)
    ...
    def _sync_deck_page_button_toggle(self) -> None:
        ...
        if enabled:
            self.deck_page_button_toggle.setStyleSheet(
                """
                QPushButton {
    ...
    def download_html_review(self) -> None:
        if not self.visible_entries:
            QMessageBox.information(self, "Review Later Manager", "No visible Review Later cards to download.")
            return
        default_name = datetime.now().strftime("Review Later HTML Review %Y-%m-%d %H-%M-%S.html")
        ...
        try:
            html = preview_html(self.visible_entries, interactive=False)
            target.write_text(rewrite_html_media_to_local_files(html), encoding="utf-8")
        except Exception as exc:
            QMessageBox.warning(self, "Review Later Manager", f"Could not download HTML review.\n\n{exc}")
            return
        QMessageBox.information(
            self,
            "Review Later Manager",
            f"Downloaded {len(self.visible_entries)} visible cards to:\n{target}",
        )
