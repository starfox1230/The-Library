...
from aqt.qt import QAction, QApplication, QByteArray, QEvent, QHBoxLayout, QInputDialog, QKeySequence, QMenu, QMessageBox, QObject, QSizePolicy, Qt, QTimer, QVBoxLayout, QWidget
...
from .countdown_audio import (
    ...
    next_countdown_cue,
    normalize_countdown_alignment_ms,
    normalize_countdown_warning_seconds,
)
from .data_paths import speed_streak_data_root
from .crystal_color_mode import CRYSTAL_COLOR_MODE_CORE, normalize_crystal_color_mode
...
from .game_state import (
    ...
    review_entry_allowance_after_state_change,
)
...
from .run_history import (
    ...
    normalize_scoreboard_layout,
    normalize_scoreboard_list_mode,
    normalize_scoreboard_order,
    normalize_scoreboard_purity,
    scoreboard_list_mode_for_layout,
)
from .scoreboard_colors import normalize_scoreboard_colors, resolved_scoreboard_colors
from .render_mode import (
    RENDER_MODE_ULTRA_LOW_RESOURCE,
    RENDER_MODE_WEBGL,
    normalize_render_mode,
)
from .settings_dialog import cleanup_settings_dialog, open_settings_dialog
...
from .sphere_mode import SPHERE_MODE_DEFAULT, normalize_sphere_mode
...
ADDON_DISPLAY_NAME = "Speed Streak"
...
# The reviewer_did_show_answer hook runs before Qt/Chromium is guaranteed to
# have painted the newly revealed answer. Returning from that hook immediately
# prevents panel rendering or audio startup from delaying the card itself.
ANSWER_SHOWN_POST_PAINT_DELAY_MS = 24
ANSWER_SHOWN_PANEL_UPDATE_DELAY_MS = 32
COMPATIBILITY_LAYOUT_VERSION = 3
TIME_DRAIN_FETCH_BATCH_SIZES = (32, 128, 512, 2048, 8192, 32768)
WHATS_NEW_VERSION = "2.05"
...
class PausedAnswerKeyBlocker(QObject):
    def __init__(self, owner: "ReviewerOverlayController") -> None:
        super().__init__(mw)
        self._owner = owner
    ...
    def eventFilter(self, watched: QObject, event: Any) -> bool:
        ...
        try:
            blocked = key in blocked_keys or int(key) in {int(blocked_key) for blocked_key in blocked_keys}
        except Exception:
...
class ReviewerOverlayController:
    def __init__(self) -> None:
        ...
        self.run_history = RunHistoryTracker(self.stats_store)
        self.scoreboard_layout = SCOREBOARD_LAYOUT_BEST_ONLY
        self.scoreboard_list_mode = SCOREBOARD_LIST_BEST
        self.scoreboard_order = SCOREBOARD_ORDER_RANKED
        self.scoreboard_purity = SCOREBOARD_PURITY_ALL
        self.scoreboard_celebration_enabled = True
        self.scoreboard_colors: dict[str, str] = {}
        self._scoreboard_color_preview: dict[str, str] | None = None
        self.display_mode = DISPLAY_MODE_INLINE
        self.inline_side = "left"
        self.side_panel_enabled = True
        self.visual_mode = VISUAL_MODE_SPHERE
        self.sphere_mode = SPHERE_MODE_DEFAULT
        self.render_mode = RENDER_MODE_WEBGL
        self.crystal_rotation_enabled = True
        self.crystal_color_mode = CRYSTAL_COLOR_MODE_CORE
        ...
        self._window_position_default_preset_id = ""
        ...
        self._whats_new_action: Optional[QAction] = None
        self._mac_haptics_diagnostics_action: Optional[QAction] = None
        self._whats_new_seen_version = ""
        self._whats_new_prompt_scheduled = False
        self._whats_new_prompt_open = False
        self._sidebar_web = None
        self._last_inline_sidebar_web = None
        self._last_inline_sidebar_hidden: Optional[bool] = None
        self._review_web = None
        self._auto_paused_for_non_review = False
        self._review_entry_free_card_pending = True
        ...
        self._undo_action_guarded = False
        ...
        self._audio_feedback_bootstrap = _read_web_asset("web", "audio_feedback.js")
        ...
        self.block_answer_keys_while_paused = False
        self.developer_mode_enabled = False
        self._developer_mode_previous_settings: dict[str, Any] = {}
        self._paused_answer_key_blocker = PausedAnswerKeyBlocker(self)
        self._answer_key_blocker_web: Optional[AnkiWebView] = None
        self._answer_key_blocker_app_installed = False
        self._last_timer_display_signature = ""
        self._last_timer_display_remaining_ms = -1
        self._last_timer_display_anchor_ms = 0
        self._boost_shortcut_healthy = False
        self._boost_shortcut_claim_count = 0
        self._boost_shortcut_last_key = ""
        ...
        self.special_timing_anking_mode = TIMER_POLICY_NORMAL
        self.special_timing_anking_question_extra_ms = 0
        self.special_timing_anking_answer_extra_ms = 15_000
        self.special_timing_typed_mode = TIMER_POLICY_NORMAL
        self.special_timing_typed_question_extra_ms = 15_000
        self.special_timing_typed_answer_extra_ms = 0
        self.special_timing_tag = ""
        self.special_timing_tag_mode = TIMER_POLICY_NORMAL
        self.special_timing_tag_question_extra_ms = 15_000
        self.special_timing_tag_answer_extra_ms = 15_000
        self.special_timer_rules: dict[str, Any] = {
            "anking_one_by_one": {},
            "typed_answer": {},
            "time_drain_flag": {},
            "note_types_enabled": False,
            "note_types": [],
            "tags_enabled": False,
            "tags": [],
        }
        ...
        self._countdown_cue_timer = QTimer(mw)
        self._countdown_cue_timer.setSingleShot(True)
        try:
            self._countdown_cue_timer.setTimerType(Qt.TimerType.PreciseTimer)
        except Exception:
            pass
        self._countdown_cue_timer.timeout.connect(self._on_countdown_cue_due)
        self._countdown_cue_pending: tuple[str, CountdownCueSchedule, int] | None = None
        self._countdown_cue_phase_key = ""
        self._countdown_cue_fired_seconds: set[int] = set()
        self._countdown_warning_cycle_active = False
    ...
    def install(self) -> None:
        ...
        self._install_reviewer_answer_guard_wrapper()
        ...
        state_did_change_hook = getattr(gui_hooks, "state_did_change", None)
        if state_did_change_hook is not None:
            state_did_change_hook.append(self.on_state_did_change)
        profile_will_close_hook = getattr(gui_hooks, "profile_will_close", None)
        if profile_will_close_hook is not None:
            profile_will_close_hook.append(self.on_profile_will_close)
            profile_will_close_hook.append(cleanup_settings_dialog)
        profile_did_open_hook = getattr(gui_hooks, "profile_did_open", None)
        if profile_did_open_hook is not None:
            profile_did_open_hook.append(self.on_profile_did_open)
        # Normal add-on loading happens before this hook fires, but calling the
        # idempotent rebind once also covers reload/debug installs performed
        # after a profile is already open.
        self.on_profile_did_open()
        self._start_timer()
        self._install_paused_answer_key_blocker()
        self._sync_paused_answer_key_blocker()
        self._sync_no_undo_guard()
        self._schedule_whats_new_if_needed()
    ...
    def _install_reviewer_answer_guard_wrapper(self) -> None:
        show_answer = getattr(Reviewer, "_showAnswer", None)
        if callable(show_answer) and not getattr(show_answer, "__speed_streak_pause_guard_wrapper__", False):
            def guarded_show_answer(reviewer: Reviewer, *args: Any, **kwargs: Any) -> Any:
                active_controller = controller
                if active_controller is not None and active_controller.should_block_answer_key_while_paused():
                    active_controller._push_bottom_bar_pause_guard()
                    return None
                return show_answer(reviewer, *args, **kwargs)

            setattr(guarded_show_answer, "__speed_streak_pause_guard_wrapper__", True)
            Reviewer._showAnswer = guarded_show_answer

        answer_card = getattr(Reviewer, "_answerCard", None)
        if callable(answer_card) and not getattr(answer_card, "__speed_streak_pause_guard_wrapper__", False):
            def guarded_answer_card(reviewer: Reviewer, *args: Any, **kwargs: Any) -> Any:
                active_controller = controller
                if active_controller is not None and active_controller.should_block_answer_key_while_paused():
                    active_controller._push_bottom_bar_pause_guard()
                    return None
                return answer_card(reviewer, *args, **kwargs)

            setattr(guarded_answer_card, "__speed_streak_pause_guard_wrapper__", True)
            Reviewer._answerCard = guarded_answer_card
    ...
    def _time_drain_review_last_enabled(self) -> bool:
        ...
        return not self._time_drain_timer_override_enabled() and bool(
            getattr(state, "time_drain_review_last", False)
        ) and int(
            getattr(state, "time_drain_flag", 0) or 0
        ) > 0

    def _time_drain_timer_override_enabled(self) -> bool:
        rule = dict(self.special_timer_rules.get("time_drain_flag", {}) or {})
        return bool(rule.get("enabled", False)) and int(
            getattr(self.engine.state, "time_drain_flag", 0) or 0
        ) > 0
    ...
    def audio_feedback_path(self, file_name: str) -> str:
        path = self.audio_feedback.resolve_path(file_name)
    ...
    def preview_audio_feedback(
        ...
        # Qt's Windows MP3 decoder honors per-cue volume, but restarting a
        ...
        if path is not None and (not native_audio_enabled() or path.suffix.lower() != ".wav"):
            if self._play_review_web_audio_feedback(
                ...
                return True
        return self.audio_feedback.play(normalized, volume_percent=volume_percent)

    def preview_native_audio_diagnostic(
        self,
        file_name: str,
        volume_percent: int = DEFAULT_AUDIO_VOLUME_PERCENT,
    ) -> bool:
        ...
        return self.audio_feedback.play(
            self.audio_feedback.normalize_file(file_name),
            volume_percent=volume_percent,
        )

    def preview_compatibility_audio_diagnostic(
        self,
        file_name: str,
        volume_percent: int = DEFAULT_AUDIO_VOLUME_PERCENT,
    ) -> bool:
        return self._play_review_web_audio_feedback(
            self.audio_feedback.normalize_file(file_name),
            interrupt=True,
            volume_percent=volume_percent,
        )

    def audio_diagnostic_report(
        self,
        event_key: str,
        file_name: str,
        volume_percent: int,
        ...
        snapshot = self.audio_feedback.diagnostic_snapshot(file_name)
        try:
            from anki import version as anki_version
        except Exception:
            anki_version = "unavailable"
        try:
        ...
        rows = [
            ...
            f"Anki: {anki_version}",
            f"Qt: {qt_version}",
            f"Anki state: {getattr(mw, 'state', '') or 'unknown'}",
            f"Browser audio available: {bool(self._audio_feedback_web())}",
            f"Event: {event_key}",
            f"Selected file: {snapshot.get('file', '')}",
            f"Extension: {snapshot.get('extension', '')}",
            f"File exists: {snapshot.get('exists', False)}",
            f"File size: {snapshot.get('sizeBytes', 0)} bytes",
            f"Selected volume: {normalize_audio_volume_percent(volume_percent)}%",
            f"Audio device: {snapshot.get('audioDevice', '') or 'unavailable'}",
            f"Native status: {snapshot.get('nativeStatus', 'unavailable')}",
            f"Native loaded: {snapshot.get('nativeLoaded', 'unavailable')}",
            f"Native playing: {snapshot.get('nativePlaying', 'unavailable')}",
            f"Native linear volume: {snapshot.get('nativeLinearVolume', 'unavailable')}",
            f"Last backend: {snapshot.get('lastBackend', 'none')}",
            f"Last result: {snapshot.get('lastResult', 'not-played')}",
            f"User heard: {hearing_result or 'not answered'}",
        ]
        return "\n".join(rows)

    def preview_audio_feedback_from(
        self,
        file_name: str,
        position_ms: int,
        volume_percent: int = DEFAULT_AUDIO_VOLUME_PERCENT,
        ...
        if path is not None and (not native_audio_enabled() or path.suffix.lower() != ".wav"):
            if self._play_review_web_audio_feedback(
                ...
                return True
        return self.audio_feedback.play_from_position(normalized, position_ms, volume_percent)

    def prepare_countdown_audio_preview(
        self,
        file_name: str,
        volume_percent: int = DEFAULT_AUDIO_VOLUME_PERCENT,
    ) -> bool:
    ...
    def preview_countdown_audio(
        ...
        if path is not None and (not native_audio_enabled() or path.suffix.lower() != ".wav"):
            if self._play_review_web_audio_feedback(
                ...
                return True
        return self.audio_feedback.play_timed(normalized, volume_percent)

    def preview_scoreboard_colors(self, colors: dict[str, str]) -> None:
        self._scoreboard_color_preview = normalize_scoreboard_colors(colors)
        self._push_state(only_if_changed=False)

    def save_scoreboard_colors(self, colors: dict[str, str]) -> None:
        self.scoreboard_colors = normalize_scoreboard_colors(colors)
        self._scoreboard_color_preview = None
        self._save_persisted_settings()
        self._push_state(only_if_changed=False)

    def cancel_scoreboard_color_preview(self) -> None:
        if self._scoreboard_color_preview is None:
            return
        self._scoreboard_color_preview = None
        self._push_state(only_if_changed=False)

    def preview_haptic_pattern(self, pattern_key: str) -> bool:
        if self.haptics.preview_pattern(pattern_key):
            return True
        sequence = BROWSER_HAPTIC_PATTERN_SEQUENCES.get(str(pattern_key or ""), [])
        if not sequence or self._sidebar_web is None:
            return False
        try:
            self._sidebar_web.eval(
                "window.SpeedStreak && window.SpeedStreak.previewHaptics"
                f" && window.SpeedStreak.previewHaptics({json.dumps(sequence)});"
            )
            return True
        except Exception:
            return False

    def haptics_diagnostics_summary(self) -> str:
        diagnostics = self.haptics.diagnostics
        backend = str(diagnostics.get("backend") or self.haptics.backend_name)
        if backend == "native-macos-gamecontroller":
            count = int(diagnostics.get("hapticControllerCount", 0) or 0)
            return f"Backend: native macOS GameController ({count} haptics-capable controller{'s' if count != 1 else ''})."
        if backend == "browser-fallback-native-monitoring":
            controller_count = int(diagnostics.get("controllerCount", 0) or 0)
            if controller_count:
                return "Backend: browser fallback. Apple sees a controller, but it did not expose native haptics."
            return "Backend: browser fallback. The native macOS helper is ready and waiting for a compatible controller."
        if backend == "browser-fallback-native-unavailable":
            detail = str(diagnostics.get("lastError") or "No native haptics-capable controller is currently available.")
            return f"Backend: browser fallback. {detail}"
        if backend == "native-windows-xinput":
            return "Backend: native Windows XInput."
        if backend == "native-steam-input":
            return "Backend: native Steam Input."
        return "Backend: embedded-browser gamepad fallback; rumble support depends on this Anki build and controller."
    ...
    def _install_paused_answer_key_blocker(self) -> None:
        if self._answer_key_blocker_app_installed:
            return
        try:
            mw.app.installEventFilter(self._paused_answer_key_blocker)
            self._answer_key_blocker_app_installed = True
        except Exception:
            pass

    def _sync_paused_answer_key_blocker(self) -> None:
        review_web = self._review_web or getattr(getattr(mw, "reviewer", None), "web", None)
        if review_web is self._answer_key_blocker_web:
            return
        if self._answer_key_blocker_web is not None:
            try:
                self._answer_key_blocker_web.removeEventFilter(self._paused_answer_key_blocker)
            except Exception:
                pass
        self._answer_key_blocker_web = review_web
        if review_web is not None:
            try:
                review_web.installEventFilter(self._paused_answer_key_blocker)
            except Exception:
                self._answer_key_blocker_web = None

    def _push_bottom_bar_pause_guard(self) -> None:
        reviewer = getattr(mw, "reviewer", None)
        bottom_web = getattr(getattr(reviewer, "bottom", None), "web", None)
        if bottom_web is None:
            return
        active = bool(self.should_block_answer_key_while_paused())
        try:
            bottom_web.eval(
                f"""
                (() => {{
                  ...
                  if (root) {{
                    root.dataset.speedStreakPauseGuard = active ? "1" : "0";
                  }}
                  if (!document.getElementById("speed-streak-pause-guard-style")) {{
                    const style = document.createElement("style");
                    style.id = "speed-streak-pause-guard-style";
                    style.textContent = `
                      html[data-speed-streak-pause-guard="1"] #ansbut,
                      html[data-speed-streak-pause-guard="1"] button[data-ease] {{
                  ...
                  if (!window.__speedStreakPauseGuardInstalled) {{
                    window.__speedStreakPauseGuardInstalled = true;
                    const blockedSelector = "#ansbut, button[data-ease]";
                    const blockIfNeeded = (event) => {{
                      if (document.documentElement.dataset.speedStreakPauseGuard !== "1") {{
                        return;
                      }}
                      const target = event.target && event.target.closest ? event.target.closest(blockedSelector) : null;
                      const activeElement = document.activeElement && document.activeElement.matches && document.activeElement.matches(blockedSelector)
    ...
    def _normalize_feedback_preferences(self) -> None:
        ...
        state.audio_event_volumes = normalize_audio_event_volumes(state.audio_event_volumes)
        state.selected_audio_file = self._preferred_audio_file(state.audio_event_files)
        state.countdown_audio_file = self.audio_feedback.normalize_file(
            state.countdown_audio_file or DEFAULT_COUNTDOWN_AUDIO_FILE
        )
        selected_event_files = list(state.audio_event_files.values())
        ...
        if state.countdown_audio_file:
            ...
            if countdown_path is not None and (not native_audio_enabled() or countdown_path.suffix.lower() != ".wav"):
                ...
                self.audio_feedback.prepare_timed(
                    state.countdown_audio_file,
                    state.countdown_audio_volume,
                )
        files_to_warm = list(native_event_files)
        if state.audio_enabled or state.countdown_audio_enabled:
            self.audio_feedback.warm_up(files_to_warm)
    ...
    def _play_audio_feedback(self, event_key: str) -> None:
        ...
        if selected:
            volume_percent = normalize_audio_volume_percent(
            ...
            # Qt Multimedia can accept an MP3 play() call and then fail silently
            # on some Windows/codec combinations.  In that case there is no
            # synchronous error that can activate a normal fallback.  Rating
            # sounds have a proven browser player, so use it as the primary
            ...
            if not played:
                played = self.audio_feedback.play(
                    selected,
                    interrupt=(event_key != "sync"),
                    volume_percent=volume_percent,
                    fallback=None if prefer_review_web else web_fallback,
                )
            if played:
                self._last_audio_feedback_started_at = now
                self._last_audio_feedback_event = event_key

    def _play_review_web_audio_feedback(
        self,
        file_name: str,
        *,
        interrupt: bool,
            ...
            options = json.dumps(
                {
                    ...
                    "volumePercent": normalize_audio_volume_percent(volume_percent),
                    "channel": str(channel or "feedback"),
            ...
            web.eval(
                ...
                (() => {{
                  if (!window.SpeedStreakAudio) {{
                    {self._audio_feedback_bootstrap}
                  }}
                  if (window.SpeedStreakAudio) {{
                    window.SpeedStreakAudio.play({json.dumps(audio_url)}, {options});
                  }}
                }})()
                """
            )
            self.audio_feedback.last_playback_backend = "browser-web"
            self.audio_feedback.last_playback_result = "accepted"
            return True
        except Exception:
            self.audio_feedback.last_playback_backend = "browser-web"
            self.audio_feedback.last_playback_result = "failed"
            return False

    def _audio_feedback_web(self) -> Any:
        reviewer_web = getattr(getattr(mw, "reviewer", None), "web", None)
        main_web = getattr(mw, "web", None)
        if getattr(mw, "state", "") == "review":
            return self._review_web or reviewer_web or main_web
        return main_web or self._review_web or reviewer_web

    def _prepare_web_audio_feedback(self, file_name: str) -> bool:
        web = self._audio_feedback_web()
        relative_path = self.audio_feedback.export_relative_path(file_name)
        if web is None or not relative_path:
            return False
        try:
            web.setPlaybackRequiresGesture(False)
        except Exception:
        ...
        try:
            ...
            web.eval(
                ...
                (() => {{
                  ...
                  if (window.SpeedStreakAudio) {{
                    window.SpeedStreakAudio.prepare({json.dumps(audio_url)});
                  }}
                }})()
                """
            )
    ...
    def _stop_web_audio_feedback(self, channel: str) -> None:
        web = self._audio_feedback_web()
        if web is None:
            return
        try:
            web.eval(
                ...
                (() => {{
                  if (window.SpeedStreakAudio) {{
                    window.SpeedStreakAudio.stop({json.dumps(str(channel or 'feedback'))});
                  }}
                }})()
                """
            )
    ...
    def _developer_mode_snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "block_answer_keys_while_paused": bool(self.block_answer_keys_while_paused),
            ...
            "side_panel_enabled": bool(self.side_panel_enabled),
            "gameplay_mode": str(state.gameplay_mode),
            "no_pause_mode": bool(state.no_pause_mode),
            "no_undo_mode": bool(state.no_undo_mode),
            "show_focus_mode_toggles": bool(state.show_focus_mode_toggles),
            "boost_seconds": float(state.boost_seconds),
            "max_boost_charges": int(state.max_boost_charges),
            "starting_boost_charges": int(state.starting_boost_charges),
            "cards_per_boost_charge": int(state.cards_per_boost_charge),
            "boost_charges": int(state.boost_charges),
            "boost_charge_progress": int(state.boost_charge_progress),
            "boosts_used": int(state.boosts_used),
        }

    def enable_developer_mode(self) -> None:
        previous_no_undo_mode = bool(self.engine.state.no_undo_mode)
        ...
        if self.engine.state.no_undo_mode and not previous_no_undo_mode:
            self._clear_anki_undo_history()
        self._sync_no_undo_guard()
        self._save_persisted_settings()
        self._request_deck_browser_refresh()
    ...
    def _apply_developer_mode_settings(self, *, enable_pause_lock: bool = True) -> None:
        state = self.engine.state
        self.shortcut_bindings = normalize_shortcut_bindings({"pause": "9", "unpause": "U", "boost": "`"})
        self.pause_shortcut_mode = PAUSE_SHORTCUT_MODE_SPLIT
        if enable_pause_lock:
            self.block_answer_keys_while_paused = True
        ...
        self.side_panel_enabled = True
        self.engine.update_gameplay_settings(
    ...
    def _restore_developer_mode_snapshot(self, snapshot: dict[str, Any]) -> None:
        ...
        self.block_answer_keys_while_paused = bool(
            snapshot.get("block_answer_keys_while_paused", self.block_answer_keys_while_paused)
        )
        ...
        self.side_panel_enabled = bool(snapshot.get("side_panel_enabled", self.side_panel_enabled))
        self.engine.update_gameplay_settings(
            gameplay_mode=str(snapshot.get("gameplay_mode", state.gameplay_mode)),
            no_pause_mode=bool(snapshot.get("no_pause_mode", state.no_pause_mode)),
            no_undo_mode=bool(snapshot.get("no_undo_mode", state.no_undo_mode)),
            show_focus_mode_toggles=bool(
                snapshot.get("show_focus_mode_toggles", state.show_focus_mode_toggles)
            ),
            boost_seconds=float(snapshot.get("boost_seconds", state.boost_seconds)),
            max_boost_charges=int(snapshot.get("max_boost_charges", state.max_boost_charges)),
            starting_boost_charges=int(
                snapshot.get("starting_boost_charges", state.starting_boost_charges)
            ),
            cards_per_boost_charge=int(
                snapshot.get("cards_per_boost_charge", state.cards_per_boost_charge)
            ),
        )
        state.boost_charges = max(
            0,
            min(state.max_boost_charges, int(snapshot.get("boost_charges", state.boost_charges))),
        )
        state.boost_charge_progress = max(
            0,
            min(
                state.cards_per_boost_charge - 1,
                int(snapshot.get("boost_charge_progress", state.boost_charge_progress)),
            ),
        )
        state.boosts_used = max(0, int(snapshot.get("boosts_used", state.boosts_used)))
        self._normalize_feedback_preferences()
        self.haptics.set_enabled(state.haptics_enabled)
    ...
    def _refresh_review_shortcuts(self) -> None:
        if getattr(mw, "state", "") != "review":
            return
        reviewer = getattr(mw, "reviewer", None)
        shortcut_keys = getattr(reviewer, "_shortcutKeys", None)
        clear_shortcuts = getattr(mw, "clearStateShortcuts", None)
        set_shortcuts = getattr(mw, "setStateShortcuts", None)
        if callable(shortcut_keys) and callable(clear_shortcuts) and callable(set_shortcuts):
            try:
                clear_shortcuts()
                set_shortcuts(list(shortcut_keys()))
                return
            except Exception:
                pass

        # Compatibility fallback for Anki versions without the current
        # state-shortcut lifecycle methods.
    ...
    def on_js_message(self, handled: tuple[bool, Any], message: str, context: Any) -> tuple[bool, Any]:
        if message == "speed-streak:reset":
            self._complete_active_run("manual-reset")
        ...
        if message == "speed-streak:toggle-pause":
            ...
            if event:
                self._track_manual_pause_event(event)
                self._push_state(only_if_changed=False)
            return (True, None)
        if message == "speed-streak:toggle-enabled":
            if self.engine.state.enabled:
                self._complete_active_run("disabled")
        ...
        if message == "speed-streak:open-settings":
            ...
            return (True, None)
        if message.startswith("speed-streak:open-settings:shortcut:"):
            shortcut_key = message.rsplit(":", 1)[-1].strip()
            self._open_settings_dialog(shortcut_key=shortcut_key)
            return (True, None)
        if message.startswith("speed-streak:open-settings:gameplay:time-boost"):
            setting = message.rsplit(":", 1)[-1].strip()
            if setting not in {"capacity", "cards"}:
                setting = "capacity"
            self._open_settings_dialog(time_boost_setting=setting)
            return (True, None)
        if message == "speed-streak:open-settings:gameplay:run-scores":
            self._open_settings_dialog(settings_page="gameplay", run_scores=True)
            return (True, None)
        if message == "speed-streak:open-settings:timers":
            self._open_settings_dialog(settings_page="timers")
            return (True, None)
        if message.startswith("speed-streak:open-settings:visual-colors:"):
            visual = message.rsplit(":", 1)[-1].strip()
            if visual not in {"sphere", "satellite", "crystal", "crystal_reactor", "brick", "lightweight_rows", "singularity", "prism_gate"}:
                visual = "sphere"
            self._open_settings_dialog(settings_page="visuals", color_visual=visual)
            return (True, None)
        if message.startswith("speed-streak:set-gameplay-mode:"):
            mode = message.rsplit(":", 1)[-1].strip()
            if mode not in {GAMEPLAY_MODE_LEGACY, GAMEPLAY_MODE_TIME_BOOST}:
                return (True, None)
            state = self.engine.state
            self.engine.update_gameplay_settings(
                gameplay_mode=mode,
        ...
        if message.startswith("speed-streak:update-settings:"):
            ...
            try:
                ...
                free_first_card_on_review_entry = bool(
                    data.get(
                        ...
                        self.engine.state.free_first_card_on_review_entry,
                    )
                )
                answer_timeout_breaks_streak = bool(
                    data.get(
                        "answerTimeoutBreaksStreak",
                        self.engine.state.answer_timeout_breaks_streak,
                    )
                )
                ...
                time_drain_auto_pause_enabled = bool(
                    data.get(
                        "timeDrainAutoPauseEnabled",
                        self.engine.state.time_drain_auto_pause_enabled,
                    )
                )
                ...
                audio_event_volumes = normalize_audio_event_volumes(
                    data.get("audioEventVolumes", self.engine.state.audio_event_volumes)
                )
                countdown_audio_enabled = bool(
                    data.get("countdownAudioEnabled", self.engine.state.countdown_audio_enabled)
                )
                countdown_audio_file = str(
                    data.get("countdownAudioFile", self.engine.state.countdown_audio_file)
                    or DEFAULT_COUNTDOWN_AUDIO_FILE
                )
                countdown_audio_volume = normalize_audio_volume_percent(
                    data.get("countdownAudioVolume", self.engine.state.countdown_audio_volume)
                )
                countdown_warning_seconds = normalize_countdown_warning_seconds(
                    data.get("countdownWarningSeconds", self.engine.state.countdown_warning_seconds)
                )
                countdown_audio_alignment_ms = normalize_countdown_alignment_ms(
                    data.get("countdownAudioAlignmentMs", self.engine.state.countdown_audio_alignment_ms)
                )
                ...
                side_panel_enabled = bool(data.get("sidePanelEnabled", self.side_panel_enabled))
                ...
                crystal_color_mode = normalize_crystal_color_mode(
                    data.get("crystalColorMode", self.crystal_color_mode)
                )
                if visual_mode == VISUAL_MODE_LIGHTWEIGHT_ROWS:
                    ...
                    render_mode = RENDER_MODE_ULTRA_LOW_RESOURCE
                    reduced_motion_enabled = True
                    orbit_animation_enabled = False
                elif visual_mode == VISUAL_MODE_CRYSTAL_REACTOR:
                    render_mode = RENDER_MODE_WEBGL
                    reduced_motion_enabled = False
                elif visual_mode in {VISUAL_MODE_SINGULARITY, VISUAL_MODE_PRISM_GATE}:
                    reduced_motion_enabled = False
            except Exception:
                return (True, None)
            previous_visuals_enabled = bool(self.engine.state.visuals_enabled)
            if previous_visuals_enabled and not visuals_enabled:
                self._complete_active_run("visuals-disabled")
            self.engine.update_time_limits(
                question_seconds=question_seconds,
                answer_seconds=answer_seconds,
                free_first_card_on_review_entry=free_first_card_on_review_entry,
                answer_timeout_breaks_streak=answer_timeout_breaks_streak,
                time_drain_flag=time_drain_flag,
                time_drain_review_last=time_drain_review_last,
                time_drain_auto_pause_enabled=time_drain_auto_pause_enabled,
            ...
            self._normalize_feedback_preferences()
            side_panel_changed = side_panel_enabled != self.side_panel_enabled
            ...
            self.crystal_color_mode = crystal_color_mode
            ...
            self._save_persisted_settings()
            self._request_deck_browser_refresh()
            if previous_visuals_enabled != self.engine.state.visuals_enabled or side_panel_changed:
                self._reconfigure_side_panel_if_reviewing()
            self._push_state(only_if_changed=False)
            return (True, None)
        if message == "speed-streak:use-time-boost":
            self._use_time_boost()
            return (True, None)
        if message.startswith("speed-streak:set-focus-rule:"):
            parts = message.split(":", 3)
            if len(parts) == 4:
                rule, raw_enabled = parts[2], parts[3]
                self._set_focus_rule(rule, raw_enabled.strip().lower() in {"1", "true", "on", "yes"})
            return (True, None)
        ...
        if message == "speed-streak:toggle-display-mode":
            ...
            self._push_state(only_if_changed=False)
            return (True, None)
        if message == "speed-streak:toggle-inline-side":
            self.inline_side = "right" if self.inline_side == "left" else "left"
            self._apply_inline_side_position()
            self._save_persisted_settings()
            self._push_state(only_if_changed=False)
            return (True, None)
        ...
        if message.startswith("speed-streak:window-preset:"):
            ...
            if action == "apply":
                ...
                self._toggle_default_window_position_preset(preset_id)
            return (True, None)
        if message == "speed-streak:default-settings":
            self._complete_active_run("default-settings-reset")
            ...
            self.special_timing_tag_question_extra_ms = 15_000
            self.special_timing_tag_answer_extra_ms = 15_000
            self.special_timer_rules = self._normalize_special_timer_rules(
                {},
                general_question_seconds=12,
                general_answer_seconds=8,
            )
            self.haptics.set_enabled(self.engine.state.haptics_enabled)
            self._save_persisted_settings()
            self._request_deck_browser_refresh()
        ...
        if message == REVIEW_LATER_TODAY_OPEN_MESSAGE and _is_deck_browser_context(context):
            ...
            return (True, None)
        if message.startswith("speed-streak:open-stats-run:"):
            try:
                run_id = int(message.rsplit(":", 1)[-1])
            except Exception:
                return (True, None)
            open_stats_dialog(
                self.engine,
                ...
                scoreboard_purity=self.scoreboard_purity,
            )
            return (True, None)
        if message.startswith("speed-streak:delete-stats-run:"):
            try:
                run_id = int(message.rsplit(":", 1)[-1])
            except Exception:
                return (True, None)
            if self.stats_store.delete_speed_streak_run(run_id):
                open_stats_dialog(
                    self.engine,
                    self.stats_store,
                    selected_run_id=0,
        ...
        if message == "speed-streak:open-stats":
            ...
            return (True, None)
        if message.startswith("speed-streak:set-scoreboard-purity:"):
            self.set_scoreboard_purity(message.rsplit(":", 1)[-1])
            return (True, None)
        if message.startswith("speed-streak:delete-run:"):
            try:
                run_id = int(message.rsplit(":", 1)[-1])
            except Exception:
                return (True, None)
    ...
    def apply_settings_from_dialog(
        ...
        *,
        ...
        audio_event_volumes: dict[str, int],
        countdown_audio_enabled: bool,
        countdown_audio_file: str,
        countdown_audio_volume: int,
        ...
        visual_color_palettes: dict[str, dict[str, str]] | None = None,
        shortcut_bindings: dict[str, str] | None = None,
        pause_shortcut_mode: str = PAUSE_SHORTCUT_MODE_COMBINED,
        block_answer_keys_while_paused: bool = False,
        special_timing_anking_mode: str = TIMER_POLICY_NORMAL,
        special_timing_anking_question_extra_seconds: float = 0,
        special_timing_anking_answer_extra_seconds: float = 15,
        special_timing_typed_mode: str = TIMER_POLICY_NORMAL,
        special_timing_typed_question_extra_seconds: float = 15,
        special_timing_typed_answer_extra_seconds: float = 0,
        special_timing_tag: str = "",
        special_timing_tag_mode: str = TIMER_POLICY_NORMAL,
        special_timing_tag_question_extra_seconds: float = 15,
        special_timing_tag_answer_extra_seconds: float = 15,
        special_timer_rules: dict[str, Any] | None = None,
    ) -> None:
        previous_visuals_enabled = bool(self.engine.state.visuals_enabled)
        if previous_visuals_enabled and not bool(visuals_enabled):
            self._complete_active_run("visuals-disabled")
        previous_time_drain_flag = int(self.engine.state.time_drain_flag or 0)
        previous_special_timer_rules = dict(self.special_timer_rules)
        self.engine.update_time_limits(
            question_seconds=question_seconds,
            answer_seconds=answer_seconds,
            free_first_card_on_review_entry=free_first_card_on_review_entry,
        ...
        self._normalize_feedback_preferences()
        self._sync_countdown_audio()
        if visual_color_palettes is not None:
            self.visual_color_palettes = normalize_visual_color_palettes(
                visual_color_palettes,
                legacy_colors=custom_colors,
            )
        previous_no_undo_mode = bool(self.engine.state.no_undo_mode)
        self.engine.update_gameplay_settings(
            gameplay_mode=gameplay_mode,
            no_pause_mode=no_pause_mode,
            no_undo_mode=no_undo_mode,
            show_focus_mode_toggles=show_focus_mode_toggles,
            boost_seconds=boost_seconds,
            max_boost_charges=max_boost_charges,
            starting_boost_charges=starting_boost_charges,
            cards_per_boost_charge=cards_per_boost_charge,
        )
        self.resume_run_after_restart_enabled = bool(resume_run_after_restart)
        self.scoreboard_layout = normalize_scoreboard_layout(scoreboard_layout)
        self.scoreboard_list_mode = scoreboard_list_mode_for_layout(
            self.scoreboard_layout,
            scoreboard_list_mode,
        )
        self.scoreboard_order = normalize_scoreboard_order(scoreboard_order)
        self.scoreboard_purity = normalize_scoreboard_purity(scoreboard_purity)
        self.scoreboard_celebration_enabled = bool(scoreboard_celebration_enabled)
        side_panel_changed = bool(side_panel_enabled) != self.side_panel_enabled
        self.side_panel_enabled = bool(side_panel_enabled)
        ...
        self.block_answer_keys_while_paused = bool(block_answer_keys_while_paused)
        if self.developer_mode_enabled and self._developer_mode_previous_settings:
            # Treat an explicit dialog change as the user's new preference so
            # disabling the developer preset does not undo that choice.
            self._developer_mode_previous_settings["block_answer_keys_while_paused"] = (
                self.block_answer_keys_while_paused
            )
        self.special_timing_anking_mode = self._normalize_timer_policy_mode(special_timing_anking_mode)
        self.special_timing_anking_question_extra_ms = max(
            0,
            int(round(float(special_timing_anking_question_extra_seconds or 0) * 1000)),
        )
        self.special_timing_anking_answer_extra_ms = max(
            0,
            int(round(float(special_timing_anking_answer_extra_seconds or 0) * 1000)),
        )
        self.special_timing_typed_mode = self._normalize_timer_policy_mode(special_timing_typed_mode)
        self.special_timing_typed_question_extra_ms = max(
            0,
            int(round(float(special_timing_typed_question_extra_seconds or 0) * 1000)),
        )
        self.special_timing_typed_answer_extra_ms = max(
            0,
            int(round(float(special_timing_typed_answer_extra_seconds or 0) * 1000)),
        )
        self.special_timing_tag = str(special_timing_tag or "").strip()
        self.special_timing_tag_mode = self._normalize_timer_policy_mode(special_timing_tag_mode)
        self.special_timing_tag_question_extra_ms = max(
            0,
            int(round(float(special_timing_tag_question_extra_seconds or 0) * 1000)),
        )
        self.special_timing_tag_answer_extra_ms = max(
            0,
            int(round(float(special_timing_tag_answer_extra_seconds or 0) * 1000)),
        )
        self.special_timer_rules = self._normalize_special_timer_rules(
            special_timer_rules,
            general_question_seconds=question_seconds,
            general_answer_seconds=answer_seconds,
        )
        timer_rules_changed = previous_special_timer_rules != self.special_timer_rules
        time_drain_flag_changed = previous_time_drain_flag != int(self.engine.state.time_drain_flag or 0)
        if self._time_drain_timer_override_enabled():
            self._restore_deferred_time_drain_cards()
        if (timer_rules_changed or time_drain_flag_changed) and getattr(mw, "state", "") == "review":
            reviewer = getattr(mw, "reviewer", None)
            card = getattr(reviewer, "card", None) if reviewer else None
            if card is not None:
                self.engine.set_current_timer_policy(**self._timer_policy_for_card(card))
                self.engine.restart_current_phase_timer()
        self.developer_mode_enabled = False
        self._developer_mode_previous_settings = {}
        self._normalize_feedback_preferences()
        ...
        self.crystal_color_mode = normalize_crystal_color_mode(crystal_color_mode)
        ...
        elif self.visual_mode == VISUAL_MODE_NUMBER_ONLY:
            self.render_mode = RENDER_MODE_ULTRA_LOW_RESOURCE
            self.engine.state.reduced_motion_enabled = True
        elif self.visual_mode == VISUAL_MODE_CRYSTAL_REACTOR:
            self.render_mode = RENDER_MODE_WEBGL
            self.engine.state.reduced_motion_enabled = False
        elif self.visual_mode in {VISUAL_MODE_SINGULARITY, VISUAL_MODE_PRISM_GATE}:
            self.engine.state.reduced_motion_enabled = False
        self.haptics.set_enabled(self.engine.state.haptics_enabled)
        if self.engine.state.no_undo_mode and not previous_no_undo_mode:
            self._clear_anki_undo_history()
    ...
    def set_scoreboard_purity(self, purity: str) -> None:
        normalized = normalize_scoreboard_purity(purity)
        if normalized == self.scoreboard_purity:
            return
        self.scoreboard_purity = normalized
        self._save_persisted_settings()
        self._push_state(only_if_changed=False)

    def _complete_active_run(self, end_reason: str) -> int:
        if self.developer_mode_enabled:
            self.run_history.discard_active()
            return 0
        return self.run_history.complete(end_reason=end_reason, state=self.engine.state)

    def _track_manual_pause_event(self, event: str) -> None:
        if self.developer_mode_enabled:
            return
        if event == "pause":
            self.run_history.begin_manual_pause()
        elif event == "resume":
    ...
    def on_profile_will_close(self, *_args: Any) -> None:
        ...
        if self.resume_run_after_restart_enabled:
            self.run_history.begin_review_exit()
        else:
            self._complete_active_run("anki-closed")
        try:
            self._save_persisted_settings()
        except Exception:
            pass

    def on_profile_did_open(self, *_args: Any) -> None:
        """Move mutable statistics out of the add-on folder once a profile exists.

        Add-ons are imported before Anki has necessarily selected a profile.
        The old initialization therefore fell back to ``user_files`` and never
        switched over, which made v2.04's run records appear disconnected from
        the active profile. Rebinding here keeps every profile's records in its
        normal ``addons-data/speed_streak`` directory.
        """

        next_root = speed_streak_data_root(ADDON_ROOT)
        try:
            same_root = next_root.resolve() == self.data_root.resolve()
        except Exception:
            same_root = str(next_root) == str(self.data_root)
        if same_root:
            # Even when the path is unchanged, refresh against the now-open
            # profile before any later settings save. Missing uploaded keys are
            # preserved, so a transient filesystem problem remains visible
            # instead of silently resetting the selection.
            self._normalize_feedback_preferences()
            return

        active_payload = self.run_history.export_active()
        previous_audio_event_files = dict(self.engine.state.audio_event_files)
        ...
        preferences_migrated = (
            ...
            or self.engine.state.countdown_audio_file != previous_countdown_audio_file
            or self.engine.state.selected_audio_file != previous_selected_audio_file
        )
        self.stats_store = StatsStore(self.data_root)
        self.run_history = RunHistoryTracker(self.stats_store)
        if not self.developer_mode_enabled:
            self.run_history.restore_active(
                active_payload,
                resume_enabled=self.resume_run_after_restart_enabled,
                state=self.engine.state,
            )
        self._last_nonce_sent = -1
        self._push_state(only_if_changed=False)
    ...
    def delete_best_run_from_dialog(self) -> bool:
        deleted = self.stats_store.delete_best_speed_streak_run()
        if deleted:
            self._push_state(only_if_changed=False)
        return deleted

    def reset_run_history_from_dialog(self) -> int:
        deleted = self.stats_store.reset_speed_streak_runs()
        if deleted:
            self._push_state(only_if_changed=False)
        return deleted
    ...
    def set_test_streak_from_dialog(self, streak: int) -> None:
        if not self.developer_mode_enabled:
            return
        self.engine.set_test_streak(streak)
        self._last_nonce_sent = -1
        self._push_state(only_if_changed=False)
    ...
    def _install_menu(self) -> None:
        ...
        menu.addSeparator()
        whats_new_action = menu.addAction("What’s New")
        whats_new_action.setMenuRole(QAction.MenuRole.NoRole)
        whats_new_action.triggered.connect(self._show_whats_new_from_menu)

        action = menu.addAction("Settings")
        action.setMenuRole(QAction.MenuRole.NoRole)
        action.triggered.connect(self._open_settings_dialog)

        help_feedback_action = menu.addAction("Help / Feedback")
        help_feedback_action.setMenuRole(QAction.MenuRole.NoRole)
        help_feedback_action.triggered.connect(self._show_help_feedback_from_menu)

        mac_haptics_diagnostics_action = None
        if sys.platform.startswith("darwin"):
            menu.addSeparator()
            mac_haptics_diagnostics_action = menu.addAction("macOS Haptics Diagnostics…")
            mac_haptics_diagnostics_action.setMenuRole(QAction.MenuRole.NoRole)
            mac_haptics_diagnostics_action.triggered.connect(self._run_mac_haptics_diagnostics)

        self._menu = menu
        self._review_later_action = review_later_action
        self._whats_new_action = whats_new_action
        self._settings_action = action
        self._help_feedback_action = help_feedback_action
        self._mac_haptics_diagnostics_action = mac_haptics_diagnostics_action

    def _show_help_feedback_from_menu(self) -> None:
        if not self.engine.state.paused and self.engine.state.visuals_enabled:
            self.engine.toggle_pause(count_in_stats=False)
            self._push_state(only_if_changed=False)
        try:
            from .support_dialog import show_help_feedback_dialog

            show_help_feedback_dialog(mw)
        except Exception:
            # The help page is optional polish and must never interfere with
            # Anki startup or review if a host Qt build lacks a feature.
            return

    def _run_mac_haptics_diagnostics(self) -> None:
        action = self._mac_haptics_diagnostics_action
        if action is None or not action.isEnabled():
            return
        action.setEnabled(False)
        tooltip("Running controller-free macOS haptics checks…")

        def collect() -> tuple[str, str]:
            from .mac_haptics_diagnostics import build_mac_haptics_report

            diagnostics = self.haptics.refresh_mac_diagnostics(timeout=1.5)
            return build_mac_haptics_report(ADDON_ROOT, diagnostics)

        def worker() -> None:
            try:
                summary, report = collect()
            except Exception as exc:
                summary = "The diagnostic report could not be completed."
                report = f"Unexpected diagnostic error: {type(exc).__name__}: {exc}"

            def present() -> None:
                action.setEnabled(True)
                self._present_mac_haptics_diagnostics(summary, report)

            mw.taskman.run_on_main(present)

        threading.Thread(target=worker, daemon=True).start()

    def _present_mac_haptics_diagnostics(self, summary: str, report: str) -> None:
        dialog = QMessageBox(mw)
        dialog.setWindowTitle("Speed Streak macOS Haptics Diagnostics")
        dialog.setIcon(QMessageBox.Icon.Information)
        dialog.setText(summary)
        dialog.setInformativeText(
            "This check does not require a controller and does not change Speed Streak settings or visuals."
        )
        dialog.setDetailedText(report)
        copy_button = dialog.addButton("Copy Report", QMessageBox.ButtonRole.ActionRole)
        close_button = dialog.addButton("Close", QMessageBox.ButtonRole.AcceptRole)
        dialog.setDefaultButton(close_button)
        dialog.exec()
        if dialog.clickedButton() is copy_button:
            QApplication.clipboard().setText(report)
            tooltip("macOS haptics diagnostic report copied.")

    def _schedule_whats_new_if_needed(self) -> None:
        if self._whats_new_seen_version == WHATS_NEW_VERSION or self._whats_new_prompt_scheduled:
            return
        self._whats_new_prompt_scheduled = True
        QTimer.singleShot(900, self._show_whats_new_if_needed)

    def _show_whats_new_if_needed(self) -> None:
        self._whats_new_prompt_scheduled = False
        if self._whats_new_seen_version == WHATS_NEW_VERSION:
            return
        self._show_whats_new()

    def _show_whats_new_from_menu(self) -> None:
        self._show_whats_new(force=True)

    def _show_whats_new(self, *, force: bool = False) -> None:
        if self._whats_new_prompt_open:
            return
        if not force and self._whats_new_seen_version == WHATS_NEW_VERSION:
            return
        self._whats_new_prompt_open = True
        destination = ""
        shown = False
        try:
            if not self.engine.state.paused and self.engine.state.visuals_enabled:
                self.engine.toggle_pause(count_in_stats=False)
                self._push_state(only_if_changed=False)
            # Import lazily so an already-seen update adds no dialog construction
            # or custom-paint setup to normal Anki startup.
            from .whats_new_dialog import show_whats_new_dialog

            destination = show_whats_new_dialog(mw)
            shown = True
        except Exception:
            # A welcome window should never keep the add-on or Anki from
            # starting. Leave the version unseen so a later launch can retry.
            return
        finally:
            self._whats_new_prompt_open = False
        if not shown:
            return
        if self._whats_new_seen_version != WHATS_NEW_VERSION:
            self._whats_new_seen_version = WHATS_NEW_VERSION
            self._save_persisted_settings()
        if destination == "settings":
            self._open_settings_dialog(settings_page="gameplay")
        elif destination == "records":
            self._open_settings_dialog(settings_page="gameplay", run_scores=True)
        elif destination == "countdown_audio":
            self._open_settings_dialog(settings_page="feedback", countdown_audio=True)
    ...
    def _open_settings_dialog(
        ...
        *,
        ...
        open_settings_dialog(
            ...
            time_boost_setting=time_boost_setting,
    ...
    def on_webview_will_set_content(self, web_content: Any, context: object | None) -> None:
        if not isinstance(context, Reviewer):
            return
        for part in ("card_timer.css", "card_timer.js", "audio_feedback.js"):
    ...
    def on_reviewer_did_show_answer(self, card: Any) -> None:
        ...
        self._last_reviewer_signature = f"{card_id}:answer"
        QTimer.singleShot(
            ANSWER_SHOWN_POST_PAINT_DELAY_MS,
            lambda current_reviewer=reviewer, expected_card_id=card_id: self._handle_answer_shown_after_paint(
                current_reviewer,
                expected_card_id,
            ),
        )

    def _handle_answer_shown_after_paint(self, reviewer: Reviewer, expected_card_id: int) -> None:
        if getattr(mw, "state", "") != "review" or getattr(mw, "reviewer", None) is not reviewer:
            return
        current_card = getattr(reviewer, "card", None)
        if current_card is None or int(getattr(current_card, "id", 0) or 0) != int(expected_card_id or 0):
            return
    ...
    def on_reviewer_will_answer_card(self, ease_tuple: tuple[bool, int], reviewer: Reviewer, card: Any) -> tuple[bool, int]:
        ...
        if self.should_block_answer_key_while_paused():
            self._push_bottom_bar_pause_guard()
            self._pending_review_undo_snapshot = None
            self._pending_review_card_id = 0
            self._pending_answer_feedback_event = ""
            return (False, ease)
        if should_continue:
            self._pending_review_undo_snapshot = (
                None if self.engine.state.no_undo_mode else self.engine.capture_review_undo_snapshot()
            )
    ...
    def on_reviewer_did_answer_card(self, reviewer: Reviewer, card: Any, ease: int) -> None:
        ...
        self._pending_review_undo_snapshot = None
        self._pending_review_card_id = 0
        if self.engine.state.no_undo_mode:
            self._clear_anki_undo_history()
    ...
    def on_state_did_undo(self, changes: Any) -> None:
        if self.engine.state.no_undo_mode:
            self.engine.notify_undo_blocked()
            self._sync_no_undo_guard()
            self._push_state(only_if_changed=False)
            return
    ...
    def on_state_did_change(self, new_state: str, old_state: str) -> None:
        self._review_entry_free_card_pending = review_entry_allowance_after_state_change(
            self._review_entry_free_card_pending,
            new_state=new_state,
            old_state=old_state,
        )
        self._sync_no_undo_guard()
    ...
    def on_state_shortcuts_will_change(self, state: Any, shortcuts: list[tuple[str, Any]]) -> None:
        if str(state) != "review":
            return
        if self.pause_shortcut_mode == PAUSE_SHORTCUT_MODE_SPLIT:
            shortcuts.append((self.pause_shortcut_display(), self._on_pause_only_shortcut))
            shortcuts.append((self.unpause_shortcut_display(), self._on_unpause_only_shortcut))
        else:
            shortcuts.append((self.pause_shortcut_display(), self._on_pause_shortcut))
        speed_streak_keys = self._configured_review_shortcut_keys()
        anki_undo_key = QKeySequence("u").toString().casefold()
        if self.engine.state.no_undo_mode:
            if anki_undo_key not in speed_streak_keys:
                shortcuts.append(("u", self._on_no_undo_shortcut))
            shortcuts.append((QKeySequence(QKeySequence.StandardKey.Undo), self._on_no_undo_shortcut))
            QTimer.singleShot(0, self._claim_no_undo_shortcuts)
        shortcuts.append((self.shortcut_label("boost"), self._on_time_boost_shortcut))
        self._schedule_time_boost_shortcut_health_check()

    def _start_timer(self) -> None:
        # This is UI state, not a collection operation. A parented QTimer stays
        # valid while the main window exists and does not ask Anki's progress
        # manager to run while the collection is still unloaded at startup.
        self._timer = QTimer(mw)
        self._timer.setInterval(100)
        self._timer.timeout.connect(self._on_tick)
        self._timer.start()

    def _countdown_audio_phase_signature(self) -> str:
        state = self.engine.state
        return "|".join(
            (
                str(state.current_card_id or ""),
                str(state.phase or "idle"),
                str(int(state.phase_start_epoch_ms or 0)),
            )
        )

    def _countdown_audio_is_active(self) -> bool:
        state = self.engine.state
        return bool(
            getattr(mw, "state", "") == "review"
            and state.enabled
            and state.visuals_enabled
            and state.audio_enabled
            and state.countdown_audio_enabled
            and state.synced
            and state.phase in {"question", "answer"}
            and state.phase_start_epoch_ms > 0
            and state.phase_limit_ms > 0
            and not state.paused
            and not state.time_drain_auto_paused
            and not (state.phase == "question" and state.first_card_free)
            and state.current_timer_policy_mode != TIMER_POLICY_NO_TIMEOUT
            and bool(state.countdown_audio_file)
        )

    def _cancel_countdown_audio(self, *, stop_sound: bool = False) -> None:
        if self._countdown_cue_timer.isActive():
            self._countdown_cue_timer.stop()
        self._countdown_cue_pending = None
        if stop_sound:
            self.audio_feedback.stop_timed()
            self._stop_web_audio_feedback("countdown")

    def _reset_countdown_audio_phase(self, phase_key: str = "") -> None:
        # A card/phase change invalidates pending ticks, but it must not clip a
        # cue that has already started. Explicit pause/disable/timeout paths
        # still call _cancel_countdown_audio(stop_sound=True).
        self._cancel_countdown_audio(stop_sound=False)
        self._countdown_cue_phase_key = str(phase_key or "")
        self._countdown_cue_fired_seconds.clear()
        self._countdown_warning_cycle_active = False

    def _sync_countdown_audio(self) -> None:
        if not self._countdown_audio_is_active():
            self._cancel_countdown_audio(stop_sound=True)
            return

        state = self.engine.state
        phase_key = self._countdown_audio_phase_signature()
        if phase_key != self._countdown_cue_phase_key:
            self._reset_countdown_audio_phase(phase_key)

        selected = self.audio_feedback.normalize_file(state.countdown_audio_file)
        if not selected:
            self._cancel_countdown_audio(stop_sound=True)
            return
        ...
        remaining_ms = self._raw_timer_remaining_ms(now_ms)
        warning_seconds = normalize_countdown_warning_seconds(state.countdown_warning_seconds)
        alignment_ms = normalize_countdown_alignment_ms(state.countdown_audio_alignment_ms)
        warning_cycle_limit = (warning_seconds * 1000) + alignment_ms
        in_warning_cycle = 0 < remaining_ms <= warning_cycle_limit
        if self._countdown_warning_cycle_active and not in_warning_cycle:
            # A boost can carry the timer back out of the warning range. The
            # next descent is a fresh warning cycle and should tick again.
            self._countdown_cue_fired_seconds.clear()
        self._countdown_warning_cycle_active = in_warning_cycle

        schedule = next_countdown_cue(
            remaining_ms=remaining_ms,
            warning_seconds=warning_seconds,
            alignment_ms=alignment_ms,
            fired_seconds=self._countdown_cue_fired_seconds,
        )
        if schedule is None:
            self._cancel_countdown_audio(stop_sound=False)
            return

        launch_epoch_ms = now_ms + schedule.delay_ms
        pending = self._countdown_cue_pending
        if (
            pending is not None
            and pending[0] == phase_key
            and pending[1].second == schedule.second
            and abs(pending[2] - launch_epoch_ms) <= 25
        ):
            return
        self._cancel_countdown_audio(stop_sound=False)
        self._countdown_cue_pending = (phase_key, schedule, launch_epoch_ms)
        self._countdown_cue_timer.start(max(0, int(schedule.delay_ms)))

    def _on_countdown_cue_due(self) -> None:
        pending = self._countdown_cue_pending
        self._countdown_cue_pending = None
        if pending is None or not self._countdown_audio_is_active():
            return
        phase_key, schedule, _launch_epoch_ms = pending
        if phase_key != self._countdown_audio_phase_signature():
            return

        state = self.engine.state
        remaining_ms = self._raw_timer_remaining_ms(int(time.time() * 1000))
        alignment_ms = normalize_countdown_alignment_ms(state.countdown_audio_alignment_ms)
        expected_launch_remaining = (schedule.second * 1000) + alignment_ms
        # Do not burst stale sounds after sleep, a debugger pause, or a busy UI.
        if remaining_ms < (schedule.second * 1000) - 140 or remaining_ms > expected_launch_remaining + 300:
            self._countdown_cue_fired_seconds.add(schedule.second)
            self._sync_countdown_audio()
            return

        self._countdown_cue_fired_seconds.add(schedule.second)
        self.preview_countdown_audio(
            state.countdown_audio_file,
            state.countdown_audio_volume,
        )
        self._sync_countdown_audio()
    ...
    def _on_pause_only_shortcut(self) -> None:
        if getattr(mw, "state", "") != "review" or self.engine.state.paused:
            return
        event = self.engine.toggle_pause()
        if event:
            self._track_manual_pause_event(event)
    ...
    def _on_unpause_only_shortcut(self) -> None:
        if getattr(mw, "state", "") != "review" or not self.engine.state.paused:
            return
        event = self.engine.toggle_pause()
        if event:
            self._track_manual_pause_event(event)
    ...
    def _on_time_boost_shortcut(self) -> None:
        if getattr(mw, "state", "") != "review":
            return
        self._use_time_boost()

    def _use_time_boost(self) -> None:
        event = self.engine.use_time_boost()
        if not event:
            self.engine.notify_time_boost_unavailable()
            self._push_state(only_if_changed=False)
            return
        self._play_feedback_event("sync")
        self._push_state(only_if_changed=False)

    def _set_focus_rule(self, rule: str, enabled: bool) -> None:
        previous_no_undo_mode = bool(self.engine.state.no_undo_mode)
        event = self.engine.set_focus_rule(rule, enabled)
        if not event:
            return
        if self.engine.state.no_undo_mode and not previous_no_undo_mode:
            self._clear_anki_undo_history()
    ...
    def _on_no_undo_shortcut(self) -> None:
        if getattr(mw, "state", "") != "review" or not self.engine.state.no_undo_mode:
            return
        self.engine.notify_undo_blocked()
        self._push_state(only_if_changed=False)

    def _clear_anki_undo_history(self) -> None:
        self.engine.clear_review_undo_history()
        collection = getattr(mw, "col", None)
        if collection is not None:
            for method_name in ("clear_undo", "clearUndo"):
                clear_undo = getattr(collection, method_name, None)
    ...
    def _sync_no_undo_guard(self) -> None:
        action = getattr(getattr(mw, "form", None), "actionUndo", None)
        active = bool(self.engine.state.no_undo_mode and getattr(mw, "state", "") == "review")
        if action is None:
            self._undo_action_guarded = active
            return
        if active and not self._undo_action_guarded:
            try:
                action.setEnabled(False)
                action.setText("Undo (No Undo mode)")
            except Exception:
                pass
            self._undo_action_guarded = True
        elif not active and self._undo_action_guarded:
            self._undo_action_guarded = False
            update_actions = getattr(mw, "update_undo_actions", None)
    ...
    def _schedule_time_boost_shortcut_health_check(self) -> None:
        self._boost_shortcut_healthy = False
        QTimer.singleShot(0, self._claim_time_boost_shortcut)
        QTimer.singleShot(120, self._claim_time_boost_shortcut)

    def _claim_time_boost_shortcut(self) -> bool:
        if getattr(mw, "state", "") != "review":
            self._boost_shortcut_healthy = False
            return False
        claimed = QKeySequence(self.shortcut_label("boost")).toString().casefold()
        if not claimed:
            self._boost_shortcut_healthy = False
            return False
        matched = 0
        for shortcut in list(getattr(mw, "stateShortcuts", []) or []):
            try:
                if shortcut.key().toString().casefold() != claimed:
                    continue
                try:
                    shortcut.activated.disconnect()
                except Exception:
                    pass
                shortcut.activated.connect(self._on_time_boost_shortcut)
                try:
                    shortcut.activatedAmbiguously.disconnect()
                except Exception:
                    pass
                try:
                    shortcut.activatedAmbiguously.connect(self._on_time_boost_shortcut)
                except Exception:
                    pass
                ...
                continue
        self._boost_shortcut_healthy = matched > 0
        self._boost_shortcut_last_key = claimed
        if matched:
            self._boost_shortcut_claim_count += matched
        return self._boost_shortcut_healthy

    def _configured_review_shortcut_keys(self) -> set[str]:
        keys = {
            QKeySequence(self.pause_shortcut_display()).toString().casefold(),
            QKeySequence(self.shortcut_label("boost")).toString().casefold(),
        }
        if self.pause_shortcut_mode == PAUSE_SHORTCUT_MODE_SPLIT:
            keys.add(QKeySequence(self.unpause_shortcut_display()).toString().casefold())
        return {key for key in keys if key}

    def _claim_no_undo_shortcuts(self) -> None:
        if getattr(mw, "state", "") != "review" or not self.engine.state.no_undo_mode:
            return
        speed_streak_keys = self._configured_review_shortcut_keys()
        claimed = {
            QKeySequence(QKeySequence.StandardKey.Undo).toString().casefold(),
        }
        anki_undo_key = QKeySequence("u").toString().casefold()
        if anki_undo_key not in speed_streak_keys:
            claimed.add(anki_undo_key)
        for shortcut in list(getattr(mw, "stateShortcuts", []) or []):
            try:
                if shortcut.key().toString().casefold() not in claimed:
                    continue
                try:
                    shortcut.activated.disconnect()
                except Exception:
                    pass
                shortcut.activated.connect(self._on_no_undo_shortcut)
                shortcut.setEnabled(True)
            except Exception:
                continue
    ...
    def _apply_anki_undo_to_speed_streak(self) -> None:
        ...
        if not self.developer_mode_enabled:
            self.run_history.observe_undo(self.engine.state)
        self._last_nonce_sent = -1
        if getattr(mw, "state", "") == "review":
            self._last_reviewer_signature = ""
            self._sync_reviewer_surface()
    ...
    def _on_tick(self) -> None:
        ...
        self._sync_countdown_audio()
        streak_before_timeout = int(self.engine.state.streak or 0)
        event = self.engine.check_timeout()
        if event:
            if event == "timeout" and streak_before_timeout > 0 and not self.developer_mode_enabled:
                self._complete_active_run("timeout")
            self._play_feedback_event("timeout")
            self._push_state(only_if_changed=False)

    def _handle_non_review_state(self) -> None:
        self._reset_countdown_audio_phase()
        self._review_entry_free_card_pending = True
        self._restore_deferred_time_drain_cards()
        self._flush_pending_review_later_sync()
        ...
        state = self.engine.state
        self.run_history.begin_review_exit()
        self.engine._clear_time_drain_auto_pause()
    ...
    def _handle_review_state(self) -> None:
        self._non_review_surface_applied = False
        self.run_history.end_review_exit()
    ...
    def _sync_live_flag_state(self) -> None:
        ...
        if next_flag != previous_flag:
            if self._time_drain_timer_override_enabled():
                self.engine.set_current_timer_policy(**self._timer_policy_for_card(card))
                time_drain_flag = int(getattr(self.engine.state, "time_drain_flag", 0) or 0)
                if time_drain_flag in {previous_flag, next_flag}:
                    self.engine.restart_current_phase_timer()
    ...
    def _reconfigure_side_panel_if_reviewing(self) -> None:
        self._last_reviewer_signature = ""
        if getattr(mw, "state", "") == "review":
            self._sync_reviewer_surface()
    ...
    def _handle_question_shown(self, reviewer: Reviewer) -> None:
        ...
        self._queue_review_later_sync_for_card(card, current_flag=card_flag)
        # A visible blue Review Later card should count immediately.  Waiting
        # for an answer/transition made the daily queue fragile when a review
        # session exited early or a later hook did not run.
        if card_flag == int(self.engine.state.review_later_flag or 0):
            self._flush_pending_review_later_sync_for_card(int(getattr(card, "id", 0) or 0))
        deck_name = self._deck_name_for_card(card)
        event = self.engine.sync_visible_question_surface(
            card_id=str(card.id),
            deck_name=deck_name,
            card_flag=card_flag,
            timer_policy=self._timer_policy_for_card(card),
            review_entry=self._review_entry_free_card_pending,
        )
        self._review_entry_free_card_pending = False
        if event == "sync":
            self._play_feedback_event("sync")
        self._sync_countdown_audio()
        self._push_state(only_if_changed=False)
    ...
    def _handle_answer_shown(self, reviewer: Reviewer) -> None:
        ...
        if event == "reveal":
            ...
            self._sync_countdown_audio()
            # Updating records, serializing the complete panel payload, and
            # evaluating it in two webviews is substantially heavier than the
            # phase/audio transition. Keep it out of the answer's paint frame.
            QTimer.singleShot(
                ANSWER_SHOWN_PANEL_UPDATE_DELAY_MS,
                lambda: self._push_state(only_if_changed=False),
            )
        self._restore_review_focus()
    ...
    def _finalize_rate(self, event: str) -> None:
        ...
        if metrics:
            ...
            if not self.developer_mode_enabled:
                self.run_history.observe_answer(
                    self.engine.state,
                    active_ms=int(metrics.get("active_ms", 0) or 0),
                )
    ...
    def _flush_pending_review_later_sync(self) -> None:
        ...
        try:
            ...
            # Never interrupt reviewing, but leave enough evidence to diagnose
            # persistence failures instead of silently losing daily entries.
            try:
                log_path = self.data_root / "review_later_sync.log"
                log_path.parent.mkdir(parents=True, exist_ok=True)
                with log_path.open("a", encoding="utf-8") as stream:
                    stream.write(
                        f"{time.strftime('%Y-%m-%d %H:%M:%S')} "
                        f"card_id={pending.get('card_id', 0)} {exc!r}\n"
                    )
    ...
    def _push_state(self, *, only_if_changed: bool = True) -> None:
        ...
        effective_scoreboard_list_mode = scoreboard_list_mode_for_layout(
            self.scoreboard_layout,
            self.scoreboard_list_mode,
        )
        run_history_payload = self.run_history.display_payload(
            list_mode=effective_scoreboard_list_mode,
            purity_mode=self.scoreboard_purity,
        )
        payload = json.dumps(
            {
                **state,
                ...
                "hapticsBackend": self.haptics.backend_name,
                "hapticsDiagnostics": self.haptics.diagnostics,
                ...
                "inlineSide": self.inline_side,
                "sidePanelEnabled": int(self.side_panel_enabled),
                "timeDrainTimerOverrideEnabled": int(self._time_drain_timer_override_enabled()),
                "windowPositionPresets": self._window_position_presets_payload(),
                "windowPositionDefaultPresetId": self._window_position_default_preset_id,
                "visualMode": self.visual_mode,
                "visualColorPalettes": {
                    visual: dict(palette)
                    for visual, palette in self.visual_color_palettes.items()
                },
                "customColors": palette_for_visual(
                    self.visual_color_palettes,
                    self.visual_mode,
                    legacy_colors=state.get("customColors", {}),
                ),
                "sphereMode": self.sphere_mode,
                "renderMode": self.render_mode,
                "crystalRotationEnabled": int(self.crystal_rotation_enabled),
                "crystalColorMode": self.crystal_color_mode,
                "shortcutBindings": self.current_shortcut_bindings(),
                "boostShortcutHealthy": int(self._boost_shortcut_healthy),
                "boostShortcutClaimCount": int(self._boost_shortcut_claim_count),
                "boostShortcutConfigured": self.shortcut_label("boost"),
                ...
                "scoreboardLayout": self.scoreboard_layout,
                "scoreboardListMode": effective_scoreboard_list_mode,
                "scoreboardOrder": self.scoreboard_order,
                "scoreboardPurity": self.scoreboard_purity,
                "scoreboardCelebrationEnabled": int(self.scoreboard_celebration_enabled),
                "scoreboardColors": resolved_scoreboard_colors(
                    self._scoreboard_color_preview
                    if self._scoreboard_color_preview is not None
                    else self.scoreboard_colors
                ),
                "runHistory": run_history_payload,
    ...
    def _uses_reduced_render_timing(self) -> bool:
        return self.visual_mode in {VISUAL_MODE_LIGHTWEIGHT_ROWS, VISUAL_MODE_NUMBER_ONLY} or self.render_mode == RENDER_MODE_ULTRA_LOW_RESOURCE
    ...
    def _timer_display_step_ms(self) -> int:
        if self.visual_mode in {VISUAL_MODE_LIGHTWEIGHT_ROWS, VISUAL_MODE_NUMBER_ONLY}:
            return 100
    ...
    def _raw_timer_remaining_ms(self, now_ms: int) -> int:
        ...
        if state.time_drain_auto_paused:
            return max(
                0,
                int(state.phase_limit_ms) - int(state.time_drain_auto_pause_elapsed_ms),
            )
    ...
    def _build_timer_display_payload(self) -> dict[str, Any]:
        ...
        signature = "|".join(
            (
                ...
                str(int(bool(state.time_drain_auto_paused))),
                str(int(state.time_drain_auto_pause_elapsed_ms or 0)),
    ...
    def _ensure_display_surface(self, reviewer: Reviewer) -> None:
        if self.display_mode == DISPLAY_MODE_COMPATIBILITY:
            ...
            return
        if not self.side_panel_enabled:
            if self._sidebar_container is not None:
                self._detach_inline_sidebar()
            self._sidebar_web = None
            self._review_web = getattr(reviewer, "web", None)
            return
    ...
    def _apply_inline_side_position(self) -> None:
        if self._sidebar_container is None or self._sidebar_web is None or self._review_web is None:
            return
        layout = self._sidebar_container.layout()
        if not isinstance(layout, QHBoxLayout):
            return
        layout.removeWidget(self._sidebar_web)
        layout.removeWidget(self._review_web)
        if self.inline_side == "right":
            layout.addWidget(self._review_web, 1)
            layout.addWidget(self._sidebar_web, 0)
        else:
            layout.addWidget(self._sidebar_web, 0)
            layout.addWidget(self._review_web, 1)
    ...
    def _set_sidebar_hidden(self, hidden: bool) -> None:
        ...
        if self._sidebar_web is None:
            return
        hidden = bool(hidden)
        if self._last_inline_sidebar_web is self._sidebar_web and self._last_inline_sidebar_hidden is hidden:
            return
        ...
        try:
            ...
            pass
        self._last_inline_sidebar_web = self._sidebar_web
        self._last_inline_sidebar_hidden = hidden
...
"""
    ...
    def _timer_policy_candidate(
        ...
        *,
        ...
        return {
            ...
            "question_limit_ms": 0 if bool(rule.get("question_untimed", False)) else max(
                1, int(round(float(rule.get("question_seconds", 1) or 1) * 1000))
            ),
            "answer_limit_ms": 0 if bool(rule.get("answer_untimed", False)) else max(
                1, int(round(float(rule.get("answer_seconds", 1) or 1) * 1000))
            ),
        }
    ...
    def _note_field_value(self, note: Any, field_name: str) -> str:
        if note is None:
            return ""
        try:
            ...
            pass
        try:
            fields = getattr(note, "items", lambda: [])()
            for name, value in fields:
                if str(name or "").strip().lower() == field_name.strip().lower():
                    return str(value or "")
        except Exception:
            pass
        try:
            keys = getattr(note, "keys", lambda: [])()
            for name in keys:
                if str(name or "").strip().lower() == field_name.strip().lower():
                    try:
                        return str(note[name] or "")
                    except Exception:
                        return ""
        except Exception:
            pass
    ...
    def _note_tags(self, note: Any) -> set[str]:
        try:
    ...
    def _note_type_name(self, card: Any, note: Any) -> str:
        for source in (note, card):
            try:
                ...
                pass
        try:
            model_id = int(getattr(note, "mid", 0) or 0)
            note_type = mw.col.models.get(model_id) if model_id > 0 else None
            if isinstance(note_type, dict):
    ...
    def _card_template_text(self, card: Any) -> str:
        parts: list[str] = []
        try:
            template = card.template()
            if isinstance(template, dict):
                parts.extend(str(template.get(key, "") or "") for key in ("qfmt", "afmt"))
        except Exception:
            pass
        try:
            note_type = card.note_type()
            templates = note_type.get("tmpls", []) if isinstance(note_type, dict) else []
            ord_value = self._card_ord_for_card(card)
            if 0 <= ord_value < len(templates):
                template = templates[ord_value]
                if isinstance(template, dict):
                    parts.extend(str(template.get(key, "") or "") for key in ("qfmt", "afmt"))
        except Exception:
            pass
    ...
    def _is_anking_one_by_one_card(self, card: Any, note: Any) -> bool:
        one_by_one = self._note_field_value(note, "One by one").strip()
        if not one_by_one:
            return False
        selected_clozes = {
            int(value)
            for value in re.findall(r"\d+", one_by_one)
            if int(value) > 0
        }
        if not selected_clozes:
            return True
        return (self._card_ord_for_card(card) + 1) in selected_clozes

    def _is_typed_answer_card(self, card: Any) -> bool:
        template_text = self._card_template_text(card).lower()
        return "{{type:" in template_text

    def _timer_policy_for_card(self, card: Any) -> dict[str, Any]:
        note = self._note_for_card(card)
        candidates: list[dict[str, Any]] = []
        time_drain_flag = int(getattr(self.engine.state, "time_drain_flag", 0) or 0)
        if time_drain_flag > 0 and self._flag_for_card(card) == time_drain_flag:
            candidate = self._timer_policy_candidate(
                source="time_drain_flag",
                rule=dict(self.special_timer_rules.get("time_drain_flag", {})),
            )
            if candidate is not None:
                candidates.append(candidate)
        if bool(self.special_timer_rules.get("tags_enabled", False)):
            note_tags = self._note_tags(note)
            for tag_rule in self.special_timer_rules.get("tags", []):
                tag = str(tag_rule.get("tag", "") or "").strip().lower()
                if not tag or tag not in note_tags:
                    continue
                candidate = self._timer_policy_candidate(
                    source=f"tag:{tag}",
                    rule=tag_rule,
                ...
                if candidate is not None:
                    candidates.append(candidate)
        if bool(self.special_timer_rules.get("note_types_enabled", False)):
            note_type_name = self._note_type_name(card, note)
            note_type_key = note_type_name.casefold()
            if note_type_key:
                for note_type_rule in self.special_timer_rules.get("note_types", []):
                    configured_names = {
                        str(name or "").strip().casefold()
                        for name in note_type_rule.get("note_types", [])
                        if str(name or "").strip()
                    }
                    if note_type_key not in configured_names:
                        continue
                    candidate = self._timer_policy_candidate(
                        source=f"note_type:{note_type_name}",
                        rule=note_type_rule,
                    )
                    if candidate is not None:
                        candidates.append(candidate)
        if self._is_anking_one_by_one_card(card, note):
            candidate = self._timer_policy_candidate(
                source="anking_one_by_one",
                rule=dict(self.special_timer_rules.get("anking_one_by_one", {})),
            )
            if candidate is not None:
                candidates.append(candidate)
        if self._is_typed_answer_card(card):
            candidate = self._timer_policy_candidate(
                source="typed_answer",
                rule=dict(self.special_timer_rules.get("typed_answer", {})),
            )
            if candidate is not None:
                candidates.append(candidate)
        ...
        if not candidates:
            return {}
        question_limits = [int(candidate["question_limit_ms"]) for candidate in candidates]
        answer_limits = [int(candidate["answer_limit_ms"]) for candidate in candidates]
        best_question = 0 if 0 in question_limits else max(question_limits)
        best_answer = 0 if 0 in answer_limits else max(answer_limits)
        source = "+".join(str(candidate["source"]) for candidate in candidates if str(candidate["source"]))
        return {
            "source": source[:80],
            "mode": TIMER_POLICY_NORMAL,
            "question_limit_ms": best_question,
            "answer_limit_ms": best_answer,
        }

    def _normalize_special_timer_rule(
        self,
        raw: object,
        *,
        ...
        return {
            ...
            "question_seconds": max(0.1, float(item.get("question_seconds", default_question_seconds) or default_question_seconds)),
            "answer_seconds": max(0.1, float(item.get("answer_seconds", default_answer_seconds) or default_answer_seconds)),
            "question_untimed": bool(item.get("question_untimed", False)),
            "answer_untimed": bool(item.get("answer_untimed", False)),
        }

    def _normalize_special_timer_rules(
        self,
        raw: object,
        *,
        general_question_seconds: float,
        general_answer_seconds: float,
    ) -> dict[str, Any]:
        data = dict(raw) if isinstance(raw, dict) else {}
        note_type_rules: list[dict[str, Any]] = []
        for item in data.get("note_types", []) if isinstance(data.get("note_types"), list) else []:
            if not isinstance(item, dict):
                continue
            names: list[str] = []
            seen_names: set[str] = set()
            for raw_name in item.get("note_types", []) if isinstance(item.get("note_types"), list) else []:
            ...
            if not names:
                continue
            note_type_rules.append({
                "note_types": names,
                **self._normalize_special_timer_rule(
                    item,
                    default_question_seconds=general_question_seconds,
                    default_answer_seconds=general_answer_seconds,
                ),
            })
        tags: list[dict[str, Any]] = []
        for item in data.get("tags", []) if isinstance(data.get("tags"), list) else []:
            if not isinstance(item, dict):
                continue
        ...
        tags_enabled = bool(data.get("tags_enabled", bool(tags)))
        note_types_enabled = bool(data.get("note_types_enabled", bool(note_type_rules)))
        return {
            "anking_one_by_one": self._normalize_special_timer_rule(
                data.get("anking_one_by_one"),
                default_question_seconds=general_question_seconds,
                default_answer_seconds=general_answer_seconds,
            ),
            "typed_answer": self._normalize_special_timer_rule(
                data.get("typed_answer"),
                default_question_seconds=general_question_seconds,
                default_answer_seconds=general_answer_seconds,
            ),
            "time_drain_flag": self._normalize_special_timer_rule(
                data.get("time_drain_flag"),
                default_question_seconds=general_question_seconds,
                default_answer_seconds=general_answer_seconds,
            ),
            "note_types_enabled": note_types_enabled,
            "note_types": note_type_rules,
            "tags_enabled": tags_enabled,
            "tags": tags,
        }

    def _migrate_legacy_special_timer_rules(
        self,
        config: dict[str, Any],
        *,
        general_question_seconds: float,
        ...
        def legacy_rule(prefix: str) -> dict[str, Any]:
            mode = self._normalize_timer_policy_mode(config.get(f"{prefix}_mode", TIMER_POLICY_NORMAL))
            return {
                "enabled": mode != TIMER_POLICY_NORMAL,
                "question_seconds": general_question_seconds + max(0.0, float(config.get(f"{prefix}_question_extra_seconds", 0) or 0)),
                "answer_seconds": general_answer_seconds + max(0.0, float(config.get(f"{prefix}_answer_extra_seconds", 0) or 0)),
                "question_untimed": mode == TIMER_POLICY_NO_TIMEOUT,
                "answer_untimed": mode == TIMER_POLICY_NO_TIMEOUT,
            }

        tags: list[dict[str, Any]] = []
        legacy_tag = str(config.get("special_timing_tag", "") or "").strip()
        if legacy_tag:
            tags.append({"tag": legacy_tag, **legacy_rule("special_timing_tag")})
        return {
            "anking_one_by_one": legacy_rule("special_timing_anking"),
            "typed_answer": legacy_rule("special_timing_typed"),
            "time_drain_flag": self._normalize_special_timer_rule(
                None,
                default_question_seconds=general_question_seconds,
                default_answer_seconds=general_answer_seconds,
    ...
    def _load_persisted_settings(self) -> None:
        ...
        feedback_preferences_migrated = False
        try:
            self._whats_new_seen_version = str(config.get("whats_new_seen_version", "") or "").strip()
            raw_render_mode = str(config.get("render_mode", RENDER_MODE_WEBGL) or RENDER_MODE_WEBGL)
            ...
            self.inline_side = "right" if str(config.get("inline_side", "left")).strip().lower() == "right" else "left"
            self.side_panel_enabled = bool(config.get("side_panel_enabled", True))
            requested_scoreboard_layout = config.get("scoreboard_layout", SCOREBOARD_LAYOUT_BEST_ONLY)
            # v2.04 initially wrote Streak Strip as the implicit default. Move
            # that one old default forward once; choices made after this build
            # are preserved normally, including Streak Strip.
            if (
                int(config.get("scoreboard_default_version", 0) or 0) < 2
                and str(requested_scoreboard_layout or "").strip().lower() == "compact"
            ):
                requested_scoreboard_layout = SCOREBOARD_LAYOUT_BEST_ONLY
            self.scoreboard_layout = normalize_scoreboard_layout(requested_scoreboard_layout)
            self.scoreboard_list_mode = normalize_scoreboard_list_mode(
                config.get("scoreboard_list_mode", SCOREBOARD_LIST_BEST)
            )
            # v2.04 used Recent 5 as its implicit default. Move that old
            # default to Best 5 once; later saved choices remain unchanged.
            if (
                int(config.get("scoreboard_default_version", 0) or 0) < 3
                and str(config.get("scoreboard_list_mode", "") or "").strip().lower() == SCOREBOARD_LIST_RECENT
            ):
                self.scoreboard_list_mode = SCOREBOARD_LIST_BEST
            self.scoreboard_list_mode = scoreboard_list_mode_for_layout(
                self.scoreboard_layout,
            ...
            self.scoreboard_order = normalize_scoreboard_order(
                config.get("scoreboard_order", SCOREBOARD_ORDER_RANKED)
            )
            self.scoreboard_purity = normalize_scoreboard_purity(
                config.get("scoreboard_purity", SCOREBOARD_PURITY_ALL)
            )
            self.scoreboard_celebration_enabled = bool(
                config.get("scoreboard_celebration_enabled", True)
            )
            self.scoreboard_colors = normalize_scoreboard_colors(config.get("scoreboard_colors", {}))
            self._scoreboard_color_preview = None
            if raw_visual_mode:
                ...
                self.visual_mode = VISUAL_MODE_SPHERE
            # Migrate the former hidden "orbit animation off" state into the
            # explicit Number Only visual without changing what the user sees.
            if (
                self.visual_mode == VISUAL_MODE_SPHERE
                and not bool(config.get("orbit_animation_enabled", True))
            ):
                self.visual_mode = VISUAL_MODE_NUMBER_ONLY
            self.sphere_mode = normalize_sphere_mode(config.get("sphere_mode", SPHERE_MODE_DEFAULT))
            ...
            self.crystal_color_mode = normalize_crystal_color_mode(
                config.get("crystal_color_mode", CRYSTAL_COLOR_MODE_CORE)
            )
            self.visual_color_palettes = normalize_visual_color_palettes(
                config.get("visual_color_palettes"),
                legacy_colors=config.get("custom_colors", {}),
            )
            if self.visual_mode == VISUAL_MODE_LIGHTWEIGHT_ROWS:
                self.render_mode = RENDER_MODE_ULTRA_LOW_RESOURCE
            elif self.visual_mode == VISUAL_MODE_NUMBER_ONLY:
            ...
            requested_default_preset = str(config.get("window_position_default_preset_id", "") or "").strip()
            valid_preset_ids = {"default"}
            valid_preset_ids.update(str(item.get("id", "")) for item in self._window_position_presets)
            self._window_position_default_preset_id = (
                requested_default_preset if requested_default_preset in valid_preset_ids else ""
            )
            ...
            self.block_answer_keys_while_paused = bool(config.get("block_answer_keys_while_paused", False))
            ...
            general_question_seconds = float(config.get("question_seconds", 12))
            general_answer_seconds = float(config.get("answer_seconds", 8))
            raw_special_timer_rules = config.get("special_timer_rules")
            legacy_special_timing_active = any(
                self._normalize_timer_policy_mode(config.get(key, TIMER_POLICY_NORMAL)) != TIMER_POLICY_NORMAL
                for key in (
                    "special_timing_anking_mode",
                    "special_timing_typed_mode",
                    "special_timing_tag_mode",
                )
            )
            new_rules_are_default = (
                isinstance(raw_special_timer_rules, dict)
                and not bool(dict(raw_special_timer_rules.get("anking_one_by_one", {}) or {}).get("enabled", False))
                and not bool(dict(raw_special_timer_rules.get("typed_answer", {}) or {}).get("enabled", False))
                and not bool(dict(raw_special_timer_rules.get("time_drain_flag", {}) or {}).get("enabled", False))
                and not bool(raw_special_timer_rules.get("note_types_enabled", False))
                and not list(raw_special_timer_rules.get("note_types", []) or [])
                and not bool(raw_special_timer_rules.get("tags_enabled", False))
                and not list(raw_special_timer_rules.get("tags", []) or [])
            )
            if not isinstance(raw_special_timer_rules, dict) or (legacy_special_timing_active and new_rules_are_default):
                raw_special_timer_rules = self._migrate_legacy_special_timer_rules(
                    config,
                    general_question_seconds=general_question_seconds,
                    general_answer_seconds=general_answer_seconds,
                )
            self.special_timer_rules = self._normalize_special_timer_rules(
                raw_special_timer_rules,
                general_question_seconds=general_question_seconds,
                general_answer_seconds=general_answer_seconds,
            )
            self.special_timing_anking_mode = self._normalize_timer_policy_mode(
                config.get("special_timing_anking_mode", TIMER_POLICY_NORMAL)
            )
            self.special_timing_anking_question_extra_ms = max(
                0,
                int(round(float(config.get("special_timing_anking_question_extra_seconds", 0) or 0) * 1000)),
            )
            self.special_timing_anking_answer_extra_ms = max(
                0,
                int(round(float(config.get("special_timing_anking_answer_extra_seconds", 15) or 0) * 1000)),
            )
            self.special_timing_typed_mode = self._normalize_timer_policy_mode(
                config.get("special_timing_typed_mode", TIMER_POLICY_NORMAL)
            )
            self.special_timing_typed_question_extra_ms = max(
                0,
                int(round(float(config.get("special_timing_typed_question_extra_seconds", 15) or 0) * 1000)),
            )
            self.special_timing_typed_answer_extra_ms = max(
                0,
                int(round(float(config.get("special_timing_typed_answer_extra_seconds", 0) or 0) * 1000)),
            )
            self.special_timing_tag = str(config.get("special_timing_tag", "") or "").strip()
            self.special_timing_tag_mode = self._normalize_timer_policy_mode(
                config.get("special_timing_tag_mode", TIMER_POLICY_NORMAL)
            )
            self.special_timing_tag_question_extra_ms = max(
                0,
                int(round(float(config.get("special_timing_tag_question_extra_seconds", 15) or 0) * 1000)),
            )
            self.special_timing_tag_answer_extra_ms = max(
                0,
                int(round(float(config.get("special_timing_tag_answer_extra_seconds", 15) or 0) * 1000)),
            )
            # Legacy values are read only for migration. New saves use
            # special_timer_rules exclusively.
            self.special_timing_anking_mode = TIMER_POLICY_NORMAL
            self.special_timing_typed_mode = TIMER_POLICY_NORMAL
            self.special_timing_tag_mode = TIMER_POLICY_NORMAL
            ...
            if reduced_motion_enabled is None:
                reduced_motion_enabled = self.visual_mode in {VISUAL_MODE_LIGHTWEIGHT_ROWS, VISUAL_MODE_NUMBER_ONLY}
            elif self.visual_mode in {VISUAL_MODE_LIGHTWEIGHT_ROWS, VISUAL_MODE_NUMBER_ONLY}:
                reduced_motion_enabled = True
            elif self.visual_mode in {VISUAL_MODE_SINGULARITY, VISUAL_MODE_PRISM_GATE}:
                reduced_motion_enabled = False
            ...
            if raw_audio_event_files is None:
                raw_audio_event_files = {event_key: selected_audio_file for event_key in default_audio_event_files()}
            persisted_audio_event_files = normalize_audio_event_files(
                raw_audio_event_files,
                fallback_file=selected_audio_file,
            )
            persisted_countdown_audio_file = str(
                config.get("countdown_audio_file", DEFAULT_COUNTDOWN_AUDIO_FILE)
                or DEFAULT_COUNTDOWN_AUDIO_FILE
            )
            ...
            self.engine.update_time_limits(
                ...
                free_first_card_on_review_entry=bool(
                    config.get("free_first_card_on_review_entry", True)
                ),
                answer_timeout_breaks_streak=bool(
                    config.get("answer_timeout_breaks_streak", True)
                ),
                time_drain_flag=int(config.get("time_drain_flag", 2)),
                time_drain_review_last=bool(config.get("time_drain_review_last", False)),
                time_drain_auto_pause_enabled=bool(config.get("time_drain_auto_pause_enabled", False)),
                review_later_flag=int(config.get("review_later_flag", 4)),
                audio_enabled=bool(config.get("audio_enabled", DEFAULT_AUDIO_ENABLED)),
                selected_audio_file=selected_audio_file,
                audio_event_files=persisted_audio_event_files,
                audio_event_volumes=normalize_audio_event_volumes(
                    config.get("audio_event_volumes", default_audio_event_volumes())
                ),
                countdown_audio_enabled=bool(
                    config.get("countdown_audio_enabled", DEFAULT_COUNTDOWN_AUDIO_ENABLED)
                ),
                countdown_audio_file=persisted_countdown_audio_file,
                countdown_audio_volume=normalize_audio_volume_percent(
                    config.get("countdown_audio_volume", DEFAULT_AUDIO_VOLUME_PERCENT)
                ),
                countdown_warning_seconds=normalize_countdown_warning_seconds(
                    config.get("countdown_warning_seconds", DEFAULT_COUNTDOWN_WARNING_SECONDS)
                ),
                countdown_audio_alignment_ms=normalize_countdown_alignment_ms(
                    config.get("countdown_audio_alignment_ms", DEFAULT_COUNTDOWN_AUDIO_ALIGNMENT_MS)
                ),
                haptics_enabled=startup_haptics_enabled,
                haptic_controller_profile=str(config.get("haptic_controller_profile", "standard")),
                ...
                orbit_animation_enabled=(self.visual_mode != VISUAL_MODE_NUMBER_ONLY),
            ...
            self.engine.update_gameplay_settings(
                gameplay_mode=str(config.get("gameplay_mode", GAMEPLAY_MODE_TIME_BOOST)),
                no_pause_mode=bool(config.get("no_pause_mode", False)),
                no_undo_mode=bool(config.get("no_undo_mode", False)),
                show_focus_mode_toggles=bool(config.get("show_focus_mode_toggles", True)),
                boost_seconds=float(config.get("boost_seconds", 10)),
                max_boost_charges=int(config.get("max_boost_charges", 5)),
                starting_boost_charges=int(config.get("starting_boost_charges", 3)),
                cards_per_boost_charge=int(config.get("cards_per_boost_charge", 10)),
            )
            self._normalize_feedback_preferences()
            feedback_preferences_migrated = (
                self.engine.state.audio_event_files != persisted_audio_event_files
                or self.engine.state.countdown_audio_file != persisted_countdown_audio_file
                or self.engine.state.selected_audio_file != selected_audio_file
            )
            if self.developer_mode_enabled:
                # Activating the developer preset initially enables the pause lock,
                # but subsequent launches must respect the user's saved choice.
                self._apply_developer_mode_settings(enable_pause_lock=False)
            ...
            self.engine.update_time_limits(
                question_seconds=12,
                answer_seconds=8,
                free_first_card_on_review_entry=True,
                answer_timeout_breaks_streak=True,
                time_drain_flag=2,
                time_drain_review_last=False,
                time_drain_auto_pause_enabled=False,
                review_later_flag=4,
                audio_enabled=DEFAULT_AUDIO_ENABLED,
                selected_audio_file=DEFAULT_AUDIO_FILE,
                audio_event_files=default_audio_event_files(),
                audio_event_volumes=default_audio_event_volumes(),
                countdown_audio_enabled=DEFAULT_COUNTDOWN_AUDIO_ENABLED,
                countdown_audio_file=DEFAULT_COUNTDOWN_AUDIO_FILE,
                countdown_audio_volume=DEFAULT_AUDIO_VOLUME_PERCENT,
                countdown_warning_seconds=DEFAULT_COUNTDOWN_WARNING_SECONDS,
                countdown_audio_alignment_ms=DEFAULT_COUNTDOWN_AUDIO_ALIGNMENT_MS,
                haptics_enabled=True,
                haptic_controller_profile="standard",
            ...
            self.engine.update_gameplay_settings(
                gameplay_mode=GAMEPLAY_MODE_TIME_BOOST,
                no_pause_mode=False,
                no_undo_mode=False,
                show_focus_mode_toggles=True,
                boost_seconds=10,
                max_boost_charges=5,
                starting_boost_charges=3,
                cards_per_boost_charge=10,
            )
            self._normalize_feedback_preferences()
            ...
            self.haptics.set_enabled(self.engine.state.haptics_enabled)
        try:
            normalized_config = self._build_persisted_settings_config()
            normalized_signature = json.dumps(
                normalized_config,
                sort_keys=True,
                separators=(",", ":"),
            )
            if feedback_preferences_migrated:
                # Persist MP3-to-WAV migrations immediately.  Previously the
                # normalized signature was only cached in memory, which made
                # later saves look unnecessary and allowed the old MP3 choice
                # to return on every Anki launch.
                self._last_saved_config_signature = ""
                self._save_persisted_settings()
            else:
                self._last_saved_config_signature = normalized_signature
        except Exception:
            self._last_saved_config_signature = ""
    ...
    def _build_persisted_settings_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "run_history_active": self.run_history.export_active(),
            "scoreboard_layout": self.scoreboard_layout,
            "scoreboard_default_version": 3,
            "scoreboard_list_mode": self.scoreboard_list_mode,
            "scoreboard_order": self.scoreboard_order,
            "scoreboard_purity": self.scoreboard_purity,
            "scoreboard_celebration_enabled": bool(self.scoreboard_celebration_enabled),
            "scoreboard_colors": dict(self.scoreboard_colors),
            "display_mode": self.display_mode,
            "inline_side": self.inline_side,
            "side_panel_enabled": bool(self.side_panel_enabled),
            "visual_mode": self.visual_mode,
            "sphere_mode": self.sphere_mode,
            "render_mode": self.render_mode,
            "crystal_rotation_enabled": bool(self.crystal_rotation_enabled),
            "crystal_color_mode": self.crystal_color_mode,
            ...
            "window_position_default_preset_id": self._window_position_default_preset_id,
            "whats_new_seen_version": self._whats_new_seen_version,
            "shortcut_bindings": self.current_shortcut_bindings(),
            "pause_shortcut_mode": self.pause_shortcut_mode,
            "block_answer_keys_while_paused": bool(self.block_answer_keys_while_paused),
            ...
            "audio_event_files": dict(state.audio_event_files),
            "audio_event_volumes": dict(state.audio_event_volumes),
            "countdown_audio_enabled": bool(state.countdown_audio_enabled),
            "countdown_audio_file": str(state.countdown_audio_file),
            "countdown_audio_volume": int(state.countdown_audio_volume),
            "countdown_warning_seconds": int(state.countdown_warning_seconds),
            "countdown_audio_alignment_ms": int(state.countdown_audio_alignment_ms),
            ...
            "question_seconds": state.question_limit_ms / 1000,
            "answer_seconds": state.review_limit_ms / 1000,
            "free_first_card_on_review_entry": bool(state.free_first_card_on_review_entry),
            "answer_timeout_breaks_streak": bool(state.answer_timeout_breaks_streak),
            "gameplay_mode": str(state.gameplay_mode),
            "no_pause_mode": bool(state.no_pause_mode),
            "no_undo_mode": bool(state.no_undo_mode),
            "show_focus_mode_toggles": bool(state.show_focus_mode_toggles),
            ...
            "time_drain_flag": state.time_drain_flag,
            "time_drain_review_last": bool(state.time_drain_review_last),
            "time_drain_auto_pause_enabled": bool(state.time_drain_auto_pause_enabled),
            "review_later_flag": state.review_later_flag,
            "review_later_today_deck_button_enabled": bool(self.review_later_today_deck_button_enabled),
            "special_timing_anking_mode": self.special_timing_anking_mode,
            "special_timing_anking_question_extra_seconds": self.special_timing_anking_question_extra_ms / 1000,
            "special_timing_anking_answer_extra_seconds": self.special_timing_anking_answer_extra_ms / 1000,
            "special_timing_typed_mode": self.special_timing_typed_mode,
            "special_timing_typed_question_extra_seconds": self.special_timing_typed_question_extra_ms / 1000,
            "special_timing_typed_answer_extra_seconds": self.special_timing_typed_answer_extra_ms / 1000,
            "special_timing_tag": self.special_timing_tag,
            "special_timing_tag_mode": self.special_timing_tag_mode,
            "special_timing_tag_question_extra_seconds": self.special_timing_tag_question_extra_ms / 1000,
            "special_timing_tag_answer_extra_seconds": self.special_timing_tag_answer_extra_ms / 1000,
            "special_timer_rules": self.special_timer_rules,
    ...
    def _toggle_default_window_position_preset(self, preset_id: str) -> None:
        normalized = str(preset_id or "").strip()
        if normalized != "default" and self._find_window_position_preset(normalized) is None:
            return
        self._window_position_default_preset_id = (
            "" if self._window_position_default_preset_id == normalized else normalized
        )
        self._save_persisted_settings()
        self._push_state(only_if_changed=False)
    ...
    def _delete_window_position_preset(self, preset_id: str) -> None:
        ...
        if self._window_position_default_preset_id == preset_id:
            self._window_position_default_preset_id = ""
        self._save_persisted_settings()
        self._push_state(only_if_changed=False)
    ...
    def _restore_or_place_floating_window(self) -> None:
        ...
        self._geometry_persistence_suspended = True
        try:
            default_preset_id = self._window_position_default_preset_id
            if default_preset_id == "default":
                self._apply_default_compatibility_layout()
                needs_save = True
                return
            if default_preset_id:
                preset = self._find_window_position_preset(default_preset_id)
                if preset is not None:
                    restored_default_main = _restore_widget_geometry(
                        mw,
                        str(preset.get("main_geometry", "") or ""),
                    )
                    restored_default_floating = self._floating_window.restore_exported_geometry(
                        str(preset.get("floating_geometry", "") or "")
                    )
                    if restored_default_main and restored_default_floating:
                        self._compatibility_main_geometry = _export_widget_geometry(mw) or self._compatibility_main_geometry
                        self._floating_geometry = self._floating_window.export_geometry() or self._floating_geometry
                        return
    ...
    def _schedule_display_mode_prompt_if_needed(self) -> None:
        ...
        QTimer.singleShot(1, self._prompt_for_display_mode)
