(function () {
  const state = {
    ...
    scoreboardActionCloseTimer: 0,
    scoreboardActionRunId: "",
    scoreboardActionAnchor: null,
    ...
    lastVisualSelectorSignature: "",
    lastScoreboardRowsSignature: "",
    lastThemeSignature: "",
    ...
    scoreboardSelectedRunId: "",
    scoreboardInitialized: false,
    lastScoreboardLiveStreak: 0,
    scoreboardCelebrationTimer: 0,
  };
  ...
  const VISUAL_MODE_ICONS = {
    prism_gate: `<svg class="acg-visual-mode-icon" viewBox="0 0 32 32" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" aria-hidden="true"><circle cx="16" cy="16" r="5" fill="currentColor" fill-opacity=".25"/><path d="M16 11C11 3 25 1 27 11M21 16c8-5 10 9 0 11M16 21C21 29 7 31 5 21M11 16C3 21 1 7 11 5M16 1v4m15 11h-4M16 31v-4M1 16h4"/></svg>`,
    sphere: `
  ...
  function timeDrainToggleMarkup() {
    return `
      ...
      <label class="acg-time-drain-toggle" for="acgTimeDrainAutoPause">
        <span class="acg-time-drain-toggle-copy">
          <span class="acg-time-drain-toggle-label">Auto-Pause Time Drains</span>
          <span class="acg-time-drain-toggle-subcopy">Stops the countdown while you work Time Drain cards.</span>
        </span>
        <span class="acg-time-drain-toggle-control">
          <input id="acgTimeDrainAutoPause" class="acg-time-drain-toggle-input" type="checkbox" />
          <span class="acg-time-drain-toggle-track" aria-hidden="true">
            <span class="acg-time-drain-toggle-knob"></span>
          </span>
        </span>
  ...
  const template = `
    <div id="speed-streak-sidebar" class="speed-streak-sidebar hidden">
      ...
      <div class="acg-inner">
        ...
        <div id="acgScoreboard" class="acg-scoreboard hidden">
        <div id="acgScoreboardTarget" class="acg-scoreboard-target" role="button" tabindex="0" aria-live="polite" aria-haspopup="dialog" aria-expanded="false" aria-label="Open Streak Records options" title="Open Streak Records options">
            <span class="acg-scoreboard-metric acg-scoreboard-best-metric"><small>ALL-TIME BEST</small><strong id="acgScoreboardBest">—</strong><span id="acgScoreboardConfetti" class="acg-scoreboard-confetti" aria-hidden="true"></span></span>
            <span class="acg-scoreboard-divider" aria-hidden="true"></span>
            <span class="acg-scoreboard-metric acg-scoreboard-live-metric"><small>LIVE</small><strong id="acgScoreboardLive">—</strong><span id="acgScoreboardLivePurity" class="acg-purity-badge hidden" aria-live="polite"></span></span>
          </div>
          <section id="acgScoreboardPanel" class="acg-scoreboard-panel" aria-label="Speed Streak records">
            <header class="acg-scoreboard-head">
              <span id="acgScoreboardTitle">RECENT 5</span>
            </header>
            <div id="acgScoreboardRows" class="acg-scoreboard-rows"></div>
          </section>
          <div id="acgScoreboardActions" class="acg-scoreboard-actions" role="dialog" aria-modal="false" aria-label="Streak Records" aria-hidden="true">
            <div class="acg-scoreboard-actions-title">Streak Records</div>
            <div class="acg-scoreboard-actions-buttons">
              <button type="button" data-scoreboard-action="stats">Stats</button>
              <button type="button" data-scoreboard-action="settings">Settings</button>
            </div>
          </div>
        </div>
        <div id="acgStage" class="acg-stage" title="Click to open visual and resource options">
          <div id="acgField" class="acg-field">
            ...
            <div id="acgPrismScene" class="acg-prism-scene">
              <canvas id="acgPrismCanvas" aria-hidden="true"></canvas>
              <div class="acg-prism-readout"><span id="acgPrismStreak">0</span></div>
              <div class="acg-prism-caption">HYPERNOVA <span>·</span> <span id="acgPrismNext">NEXT LAYER · 10</span></div>
            </div>
            ...
            <div id="acgVisualSelector" class="acg-visual-selector" aria-label="Visual options">
              ...
              <div class="acg-visual-selector-panel">
                <div class="acg-visual-choice-list" role="group" aria-label="Visual style">
                  ...
                  <button class="acg-visual-choice" type="button" data-visual-choice="prism_gate" title="Hypernova" aria-label="Hypernova">${VISUAL_MODE_ICONS.prism_gate}</button>
        ...
        <div id="acgTimeDrainOverlay" class="acg-time-drain">
          <div class="acg-time-drain-copy">
            ...
            <div class="acg-time-drain-body">This card is a time drain.</div>
  ...
  function ensureMounted() {
    ...
    const scoreboardRows = document.getElementById("acgScoreboardRows");
    scoreboardRows?.addEventListener("click", (event) => {
      const row = event.target?.closest?.("[data-scoreboard-run-id]");
      if (!row) return;
      const runId = String(row.getAttribute("data-scoreboard-run-id") || "");
      openScoreboardActions(row, runId);
    });

    const scoreboardTarget = document.getElementById("acgScoreboardTarget");
    if (scoreboardTarget) {
      const openRecordActions = (event) => {
        if (event?.type === "keydown") {
          if (!["Enter", " "].includes(event.key)) return;
          event.preventDefault();
        }
        openScoreboardActions(scoreboardTarget);
      };
      scoreboardTarget.addEventListener("click", openRecordActions);
      scoreboardTarget.addEventListener("keydown", openRecordActions);
    }

    const scoreboardActions = document.getElementById("acgScoreboardActions");
    if (scoreboardActions) {
      scoreboardActions.addEventListener("pointerenter", cancelScoreboardActionClose);
      scoreboardActions.addEventListener("pointerleave", scheduleScoreboardActionClose);
      scoreboardActions.addEventListener("click", (event) => {
        const button = event.target?.closest?.("[data-scoreboard-action]");
        if (!button || typeof pycmd !== "function") return;
        const action = String(button.getAttribute("data-scoreboard-action") || "");
        const runId = String(state.scoreboardActionRunId || "");
        closeScoreboardActions();
        if (action === "stats") {
          pycmd(runId ? `speed-streak:open-stats-run:${runId}` : "speed-streak:open-stats");
        } else if (action === "settings") {
          pycmd("speed-streak:open-settings:gameplay:run-scores");
        }
      });
    }
    document.addEventListener("pointermove", (event) => {
      if (!scoreboardActions?.classList.contains("open")) return;
      if (pointerIsNearScoreboardActions(event.clientX, event.clientY)) {
        cancelScoreboardActionClose();
      } else if (!state.scoreboardActionCloseTimer) {
        scheduleScoreboardActionClose();
      }
    });
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape" && scoreboardActions?.classList.contains("open")) {
        closeScoreboardActions();
      }
    });
    ...
    if (sidebarRoot && visualSelector && visualSelectorToggle) {
      const syncSidebarWidth = () => {
        ...
        syncPauseOverlayPlacement(state.data || {});
      };
      syncSidebarWidth();
  ...
  function ensureTimeDrainToggleControl() {
    if ($("acgTimeDrainReviewLast") && $("acgTimeDrainAutoPause")) {
      return;
    }
  ...
  function bindTimeDrainToggleInput() {
    [$("acgTimeDrainReviewLast"), $("acgTimeDrainAutoPause")].forEach((input) => {
  ...
  function getVisualMode(data) {
    ...
    if (["prism_gate", "prism", "hypernova"].includes(normalized)) return "prism_gate";
  ...
  function computeTimer(data) {
    ...
    const autoPaused = Boolean(data.timeDrainAutoPaused);

    if (phase === "idle" || !Number(data.phaseStartEpochMs || 0)) {
      return { phase, free, untimed: false, paused, autoPaused: false, remaining: 0, total: 0, baseTotal: 0, secondsText: "0.0" };
    }
    if (free) {
      return { phase, free: true, untimed: false, paused, autoPaused: false, remaining: 0, total: 0, baseTotal: 0, secondsText: "0.0" };
    }
    if (untimed && !limit) {
      return { phase, free: false, untimed: true, paused: false, autoPaused: false, remaining: 0, total: 0, baseTotal: 0, secondsText: "0.0" };
    }
    if (!limit) {
      return { phase, free: true, untimed: false, paused, autoPaused: false, remaining: 0, total: 0, baseTotal: 0, secondsText: "0.0" };
    }
    const remaining = paused || autoPaused
    ...
    const boostRemaining = paused || autoPaused || !boostAnchor
    ...
    if (paused) {
      return { phase, free: false, untimed: false, paused: true, autoPaused: false, remaining, total: limit, baseTotal: baseLimit, baseRemaining, boostRemaining, boostActive, boostRatio, boostAnchor, storedBoostRemaining, totalRatio, baseProgress, overflowTurns, overflowProgress, secondsText: formatTimerSecondsText(remaining) };
    }
    if (autoPaused) {
      return { phase, free: false, untimed: false, paused: false, autoPaused: true, remaining, total: limit, baseTotal: baseLimit, baseRemaining, boostRemaining, boostActive, boostRatio, boostAnchor, storedBoostRemaining, totalRatio, baseProgress, overflowTurns, overflowProgress, secondsText: formatTimerSecondsText(remaining) };
    }
    return {
      phase,
      free: false,
  ...
  function saveSettings(overrides = {}) {
    ...
    const timeDrainAutoPauseEnabled = Boolean($("acgTimeDrainAutoPause")?.checked);
  ...
  function syncSettingsFields(data) {
    ...
    const signature = JSON.stringify({
      ...
      timeDrainAutoPauseEnabled: Boolean(data.timeDrainAutoPauseEnabled),
    ...
    const timeDrainAutoPauseInput = $("acgTimeDrainAutoPause");
    if (timeDrainAutoPauseInput && document.activeElement !== timeDrainAutoPauseInput) {
      timeDrainAutoPauseInput.checked = Boolean(data.timeDrainAutoPauseEnabled);
    }
  ...
  function visualResourceLevels(visualMode) {
    if (visualMode === "prism_gate") return [
      {label: "Still", tick: "Still", description: "A frozen stellar core that updates with your streak. No continuous or event motion."},
      {label: "Smooth", tick: "Smooth", description: "60 FPS plasma arms, streaming stars, and milestone eruptions. The everyday setting; 1× pixel density."},
      {label: "Rich", tick: "Rich", description: "60 FPS with more star trails and sharper glow detail. Up to 1.5× pixel density; maximum 960 pixels per edge."},
    ];
  ...
  function defaultVisualResourceLevel(visualMode) {
    if (visualMode === "prism_gate") return 1;
  ...
  function currentVisualResourceLevel(data, visualMode) {
    if (visualMode !== getVisualMode(data)) return defaultVisualResourceLevel(visualMode);
    if (visualMode === "prism_gate") return getRenderMode(data) === "ultra_low_resource" ? 0 : getRenderMode(data) === "low_resource" ? 1 : 2;
  ...
  function applyVisualResourceLevel(visualMode, rawLevel) {
    ...
    if (visualMode === "prism_gate") {
      saveSettings({visualMode: "prism_gate", renderMode: level === 0 ? "ultra_low_resource" : level === 1 ? "low_resource" : "webgl", orbitAnimationEnabled: true});
      return;
    }
    if (visualMode === "crystal_reactor") {
      saveSettings({
  ...
  function renderVisualResourceSelector(data, requestedLevel = null) {
    ...
    const choice = ["sphere", "prism_gate", "singularity", "crystal_reactor", "lightweight_rows", "number_only"].includes(state.visualSelectorChoice)
    ...
    // Answer/phase updates do not change this settings UI. Replacing its SVG
    // and tick markup on every answer creates needless DOM/style/GC work.
    const selectorSignature = `${actualVisualMode}|${choice}|${level}`;
    if (selectorSignature === state.lastVisualSelectorSignature) return;
    state.lastVisualSelectorSignature = selectorSignature;
    ...
    if (name) {
      name.textContent = choice === "sphere"
        ? "Satellite style"
        : choice === "prism_gate" ? "Hypernova motion"
        : choice === "singularity"
          ? "Singularity detail"
  ...
  function applyScoreboardColors(sidebar, colors) {
    if (!sidebar) return;
    const label = normalizeHexColor(colors?.label) || "#8ea0cc";
    const value = normalizeHexColor(colors?.value) || "#65f0c2";
    const newRecord = normalizeHexColor(colors?.new_record) || "#f5aa41";
    sidebar.style.setProperty("--acg-record-label", label);
    sidebar.style.setProperty("--acg-record-value", value);
    sidebar.style.setProperty("--acg-record-new", newRecord);
  }
  ...
  function webglContextIsLost(renderer) {
  ...
  function disposeWebglRenderer(renderer = state.webgl, deleteResources = true) {
    if (!renderer) {
      return;
    }
    ...
    if (deleteResources && !renderer.contextLost && !webglContextIsLost(renderer)) {
      try {
        renderer.gl.deleteBuffer(renderer.buffer);
        renderer.gl.deleteBuffer(renderer.debrisBuffer);
        renderer.gl.deleteProgram(renderer.program);
        renderer.gl.deleteProgram(renderer.debrisProgram);
      } catch (_error) {}
    }
  ...
  function ensureWebglRenderer() {
    ...
    if (state.webgl?.canvas === canvas && state.webgl.gl) {
      if (!state.webgl.contextLost && !webglContextIsLost(state.webgl)) {
        return state.webgl;
      }
      // Keep the lost renderer and its context-restored listener in place so
      // the browser can notify us when the shared canvas becomes usable again.
      return null;
    }
    try {
      ...
      renderer.contextLostHandler = (event) => {
        event.preventDefault();
        if (state.webgl !== renderer) return;
      ...
      renderer.contextRestoredHandler = () => {
        if (state.webgl !== renderer) return;
        const shouldResume = Boolean(state.data) && getVisualMode(state.data) === "sphere";
        disposeWebglRenderer(renderer, false);
        if (shouldResume && !document.hidden && state.data) {
          window.requestAnimationFrame(() => render(state.data));
        }
      };
      ...
      initializeWebglCanvasSizing(renderer, () => {
        if (!renderer.running && !renderer.contextLost && !webglContextIsLost(renderer) && getVisualMode(state.data || {}) === "sphere") {
  ...
  function cancelScoreboardActionClose() {
    if (!state.scoreboardActionCloseTimer) return;
    window.clearTimeout(state.scoreboardActionCloseTimer);
    state.scoreboardActionCloseTimer = 0;
  }

  function closeScoreboardActions() {
    cancelScoreboardActionClose();
    const menu = $("acgScoreboardActions");
    const target = $("acgScoreboardTarget");
    if (state.scoreboardActionAnchor) {
      state.scoreboardActionAnchor.classList?.remove?.("action-source");
      state.scoreboardActionAnchor.setAttribute?.("aria-expanded", "false");
    }
    target?.setAttribute("aria-expanded", "false");
    menu?.classList.remove("open");
    menu?.setAttribute("aria-hidden", "true");
    state.scoreboardActionRunId = "";
    state.scoreboardActionAnchor = null;
  }

  function positionScoreboardActions(anchor) {
    const sidebar = $("speed-streak-sidebar");
    const scoreboard = $("acgScoreboard");
    const menu = $("acgScoreboardActions");
    if (!sidebar || !scoreboard || !menu || !anchor) return;
    const sidebarRect = sidebar.getBoundingClientRect();
    const scoreboardRect = scoreboard.getBoundingClientRect();
    const anchorRect = anchor.getBoundingClientRect();
    const menuRect = menu.getBoundingClientRect();
    const halfWidth = Math.max(1, menuRect.width / 2);
    const left = clamp(
      anchorRect.left + (anchorRect.width / 2) - scoreboardRect.left,
      halfWidth + 4,
      Math.max(halfWidth + 4, scoreboardRect.width - halfWidth - 4),
    );
    let top = anchorRect.bottom - scoreboardRect.top + 7;
    if (scoreboardRect.top + top + menuRect.height > sidebarRect.bottom - 10) {
      top = anchorRect.top - scoreboardRect.top - menuRect.height - 7;
    }
    menu.style.left = `${Math.round(left)}px`;
    menu.style.top = `${Math.max(4, Math.round(top))}px`;
  }

  function openScoreboardActions(anchor, runId = "") {
    const menu = $("acgScoreboardActions");
    if (!menu || !anchor) return;
    closeScoreboardActions();
    state.scoreboardActionRunId = String(runId || "");
    state.scoreboardActionAnchor = anchor;
    anchor.classList.add("action-source");
    anchor.setAttribute("aria-expanded", "true");
    menu.classList.add("open");
    menu.setAttribute("aria-hidden", "false");
    positionScoreboardActions(anchor);
  }

  function scheduleScoreboardActionClose() {
    const menu = $("acgScoreboardActions");
    if (!menu?.classList.contains("open")) return;
    cancelScoreboardActionClose();
    state.scoreboardActionCloseTimer = window.setTimeout(() => {
      state.scoreboardActionCloseTimer = 0;
      closeScoreboardActions();
    }, 520);
  }

  function pointerIsNearScoreboardActions(x, y) {
    const menu = $("acgScoreboardActions");
    const anchor = state.scoreboardActionAnchor;
    if (!menu || !anchor?.isConnected) return false;
    return Math.min(
      distanceFromPointToRect(x, y, anchor.getBoundingClientRect()),
  ...
  function uploadWebglSatellites(renderer, satellites, signature) {
    if (renderer.demolitionComplete && !state.fusionDemolitionActive) {
      // A completed loss sequence must not make the next live run invisible.
      // This is a cheap state reset; it does not add any per-frame work.
      renderer.demolitionComplete = false;
  ...
  function drawWebglFrame(renderer, timestamp = performance.now()) {
    if (!renderer.running || renderer.contextLost) {
      return;
    }
    renderer.frameId = 0;
  ...
  // Hypernova: one bounded Canvas 2D surface plus two tiny cached glow sprites.
  // Keep the original prism_gate storage key so existing selections and palettes migrate.
  // Geometry and upload-sized storage are fixed regardless of streak length.
  const PRISM_LIMITS = Object.freeze({rings: 6, segments: 64, stars: 72, edge: 960});
  const HYPERNOVA_UNLOCKS = ["FIRST RING", "STAR FIELD", "PLASMA SPIRAL", "SECOND ARM", "SUPERNOVA", "POLAR JETS", "THIRD ARM", "OUTER HALO", "FOURTH ARM", "HYPERNOVA"];

  function stopPrismGate(dispose = false) {
    const renderer = state.prismGate;
    if (!renderer) return;
    window.clearTimeout(renderer.wakeId);
    window.cancelAnimationFrame(renderer.frameId);
    renderer.wakeId = renderer.frameId = 0;
    renderer.lastAt = 0;
    renderer.nextFrameAt = 0;
    if (dispose) {
      renderer.observer?.disconnect();
      renderer.motionQuery?.removeEventListener?.("change", renderer.motionChanged);
      renderer.canvas.width = renderer.canvas.height = 1;
      state.prismGate = null;
    }
  }

  function prismCanMove(renderer) {
    const data = renderer.data;
    return !document.hidden && !data.paused && !state.settingsOpen
      && Boolean(data.enabled && data.visualsEnabled)
      && !(data.displayMode !== "compatibility" && data.sidebarCollapsed)
      && getVisualMode(data) === "prism_gate" && renderer.level > 0
      && data.orbitAnimationEnabled !== false && !data.reducedMotion
      && !renderer.motionQuery?.matches;
  }

  function renderPrismGate(data) {
    let renderer = state.prismGate;
    const canvas = $("acgPrismCanvas");
    if (!canvas || document.hidden) { stopPrismGate(); return; }
    if (!renderer) {
      const ctx = canvas.getContext("2d", {alpha: false});
      if (!ctx) {
        setText("acgPrismStreak", String(data.streak || 0));
        return;
      }
      renderer = state.prismGate = {
        canvas, ctx, data, frameId: 0, wakeId: 0, lastAt: 0, clock: 0,
        nextFrameAt: 0, sourceStreak: Number(data.streak || 0), lastStreak: Number(data.streak || 0), level: 1, signature: "", needsResize: true, nonce: Number(data.eventNonce || 0),
        eventAt: -10, eventKind: "", eventStrength: 1,
        points: new Float32Array(PRISM_LIMITS.rings * (PRISM_LIMITS.segments + 1) * 3),
        colors: [], paletteKey: "", width: 1, height: 1,
        motionQuery: window.matchMedia?.("(prefers-reduced-motion: reduce)"),
      };
      renderer.observer = new ResizeObserver(() => {
        renderer.needsResize = true;
        if (state.prismGate === renderer) renderPrismGate(renderer.data);
      });
      renderer.observer.observe(canvas);
      renderer.motionChanged = () => renderPrismGate(renderer.data);
      renderer.motionQuery?.addEventListener?.("change", renderer.motionChanged);
    }
    renderer.data = data;
    renderer.level = getRenderMode(data) === "ultra_low_resource" ? 0 : getRenderMode(data) === "low_resource" ? 1 : 2;
    const palette = resolveCustomColors(data.customColors, data.appearanceMode || "midnight");
    const paletteKey = JSON.stringify(palette);
    if (renderer.paletteKey !== paletteKey) {
      renderer.paletteKey = paletteKey;
      renderer.palette = palette;
      const [red, green, blue] = hexToRgb(palette.core);
      // Rotate the spectrum around the user's own base color, keeping luminous edges.
      const hue = Math.atan2(Math.sqrt(3) * (green - blue), 2 * red - green - blue) * 180 / Math.PI;
      renderer.colors = Array.from({length: 8}, (_, i) => `hsl(${hue + i * 16 - 70}, 95%, ${64 + i % 3 * 7}%)`);
      renderer.glows = [renderer.colors[1], renderer.colors[5]].map(color => {
        const sprite = document.createElement("canvas");sprite.width = sprite.height = 96;
        const paint = sprite.getContext("2d");
        const glow = paint.createRadialGradient(48,48,0,48,48,48);
        glow.addColorStop(0,"#ffffff");glow.addColorStop(.07,"#e5ffff");
        glow.addColorStop(.18,color);glow.addColorStop(1,"transparent");
        paint.fillStyle=glow;paint.fillRect(0,0,96,96);return sprite;
      });
    }
    const streak = Math.max(0, Number(data.streak || 0));
    const nonce = Number(data.eventNonce || 0);
    if (renderer.nonce !== nonce) {
      renderer.nonce = nonce;
      const kind = String(data.lastEventType || "");
      if (["again", "timeout", "reset", "hard", "good", "easy", "time-boost"].includes(kind)) {
        renderer.sourceStreak = Math.max(streak, renderer.lastStreak || 0);
        renderer.eventAt = renderer.clock;
        renderer.eventKind = kind;
        renderer.eventStrength = streak > 0 && streak % 100 === 0 ? 3 : streak > 0 && streak % 50 === 0 ? 2.5 : streak > 0 && streak % 10 === 0 ? 1.7 : 1;
        renderer.eventColor = palette[{again: "red", timeout: "red", reset: "red", hard: "yellow", good: "green", easy: "blue", "time-boost": "yellow"}[kind]];
        if (String(data.lastEventText || "").includes("Boost earned")) {
          renderer.eventStrength = Math.max(2, renderer.eventStrength);
          renderer.eventColor = "#ffe59b";
        }
      }
    }
    renderer.lastStreak = streak;
    renderer.earnedColors = Array.isArray(data.satelliteColors) ? data.satelliteColors.slice(-100) : [];
    renderer.earnedStateKey = `${streak}|${renderer.earnedColors.join(",")}`;
    setText("acgPrismStreak", String(streak));
    setText("acgPrismNext", `${HYPERNOVA_UNLOCKS[Math.floor(streak / 10)] || "RING UPGRADE"} · ${(Math.floor(streak / 10) + 1) * 10}`);
    const moving = prismCanMove(renderer);
    const signature = `${renderer.earnedStateKey}|${nonce}|${paletteKey}|${renderer.level}|${moving}`;
    const dirty = signature !== renderer.signature || renderer.needsResize;
    renderer.signature = signature;
    if (!moving) {
      stopPrismGate();
      if (dirty) drawPrismGate(renderer, performance.now());
    } else if (!renderer.frameId && !renderer.wakeId) {
      renderer.frameId = window.requestAnimationFrame((now) => drawPrismGate(renderer, now));
    }
  }

  function drawPrismGate(renderer, now) {
    renderer.frameId = 0;
    if (state.prismGate !== renderer || document.hidden) return;
    const moving = prismCanMove(renderer);
    // Vsync scheduling with a deadline accumulator. A timeout followed by rAF adds
    // an extra refresh interval; never put a timer in front of this animation.
    if (moving && now + .8 < renderer.nextFrameAt) {
      renderer.frameId = window.requestAnimationFrame(timestamp => drawPrismGate(renderer, timestamp));
      return;
    }
    if (moving) {
      renderer.nextFrameAt = Math.max(renderer.nextFrameAt || now, now - 1000 / 60) + 1000 / 60;
      if (renderer.lastAt) renderer.clock += Math.min(.1, (now - renderer.lastAt) / 1000);
    }
    renderer.lastAt = moving ? now : 0;
    const {ctx, canvas, data, points} = renderer;
    if (renderer.needsResize || renderer.sizeLevel !== renderer.level) {
      const width = Math.max(1, canvas.clientWidth), height = Math.max(1, canvas.clientHeight);
      const density = Math.min(window.devicePixelRatio || 1, renderer.level === 2 ? 1.5 : 1, PRISM_LIMITS.edge / Math.max(width, height));
      const bufferWidth = Math.max(1, Math.round(width * density));
      const bufferHeight = Math.max(1, Math.round(height * density));
      // A redundant resize clears the surface and invalidates its GPU backing.
      // Layout notifications may change subpixels without changing buffer size.
      if (canvas.width !== bufferWidth) canvas.width = bufferWidth;
      if (canvas.height !== bufferHeight) canvas.height = bufferHeight;
      renderer.width = width; renderer.height = height;
      ctx.setTransform(canvas.width / width, 0, 0, canvas.height / height, 0, 0);
      renderer.needsResize = false; renderer.sizeLevel = renderer.level;
    }
    const w = renderer.width, h = renderer.height, cx = w / 2, cy = h / 2;
    const radius = Math.min(w * .47, h * .39);
    const streak = Math.max(0, Number(data.streak || 0)), t = renderer.clock;
    const age = t - renderer.eventAt;
    const loss = ["again", "timeout", "reset"].includes(renderer.eventKind);
    const reaction = moving && age >= 0 && age < 1.9 ? Math.sin(Math.min(1, age / .12) * Math.PI / 2) * Math.pow(1 - age / 1.9, 2) : 0;
    const visualStreak = loss && reaction > .01 ? renderer.sourceStreak : streak;
    const stage = Math.min(10, Math.floor(visualStreak / 10));
    const growth = stage / 10;
    const pulse = reaction * renderer.eventStrength * (.18 + growth * .82);
    const arms = stage < 3 ? 0 : stage < 4 ? 1 : stage < 7 ? 2 : stage < 9 ? 3 : stage < 10 ? 4 : 6;
    const segments = PRISM_LIMITS.segments;
    const spin = t * (.12 + growth * .58);
    ctx.globalAlpha = 1; ctx.globalCompositeOperation = "source-over";
    ctx.fillStyle = "#020611"; ctx.fillRect(0, 0, w, h);
    // Cached glow sprites replace live blur filters. They are regenerated only on palette changes.
    const haloSize = radius * (.6 + growth * 2.2 + pulse * .15);
    ctx.globalAlpha = .08 + growth * .37;
    ctx.drawImage(renderer.glows[0], cx - haloSize / 2, cy - haloSize / 2, haloSize, haloSize);
    ctx.globalCompositeOperation = "lighter";
    // Streaming star field: fixed seeds, analytic lifetimes, no spawned particle lists.
    const starCount = stage < 2 ? 0 : Math.min(renderer.level === 2 ? 72 : 48, (stage - 1) * (renderer.level === 2 ? 8 : 6));
    for (let i = 0; i < starCount; i++) {
      const seed = ((i * 137.508 + 23) % 997) / 997;
      const life = (seed + t * (.19 + (i % 5) * .018)) % 1;
      const a = i * 2.39996 + spin * .12;
      const travel = .27 + life * life * 1.35;
      const r = radius * travel;
      const stretch = (4 + life * life * 24) * (1 + pulse * .4);
      const x = cx + Math.cos(a) * r, y = cy + Math.sin(a) * r * 1.3;
      const alpha = Math.sin(life * Math.PI) * (.3 + life * .6);
      ctx.strokeStyle = renderer.colors[i % 8];ctx.globalAlpha = alpha;ctx.lineWidth = i % 7 === 0 ? 1.8 : .8;
      ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x-Math.cos(a)*stretch,y-Math.sin(a)*stretch*1.3);ctx.stroke();
      if (i % 4 === 0) {
        ctx.globalAlpha = alpha * .65;
        ctx.drawImage(renderer.glows[i % 2], x-5, y-5, 10,10);
      }
    }
    // Opposed polar jets: sweeping, tapered flame ribbons behind the stellar disc.
    ctx.save();ctx.translate(cx,cy);ctx.rotate(-.4 + Math.sin(t * .22) * .18);
    for (const direction of stage >= 6 ? [-1,1] : []) {
      ctx.save();ctx.scale(direction,direction);
      ctx.fillStyle = renderer.colors[2];ctx.globalAlpha = .1 + pulse * .025;
      ctx.beginPath();ctx.moveTo(-radius*.14,0);
      ctx.bezierCurveTo(-radius*.32,-radius*.42,radius*.19,-radius*.78,0,-radius*1.5);
      ctx.bezierCurveTo(radius*.04,-radius*.82,radius*.48,-radius*.42,radius*.14,0);ctx.fill();
      ctx.strokeStyle = renderer.colors[4];ctx.globalAlpha = .42;ctx.lineWidth = 1.2;
      ctx.beginPath();ctx.moveTo(0,0);ctx.bezierCurveTo(radius*.34,-radius*.55,-radius*.09,-radius*.85,0,-radius*1.4);ctx.stroke();
      ctx.restore();
    }
    ctx.restore();
    // Cache the luminous spiral once per geometry/palette/size change. Rotate one
    // sprite at 60 FPS, then animate its travelling knots independently.
    const discPixels = Math.min(640, Math.max(128, Math.ceil(radius * 2.5 * (renderer.level === 2 ? 1.5 : 1))));
    const discKey = `${arms}|${renderer.paletteKey}|${discPixels}`;
    if (arms && renderer.discKey !== discKey) {
      renderer.discKey = discKey;
      renderer.plasmaDisc = document.createElement("canvas");
      renderer.plasmaDisc.width = renderer.plasmaDisc.height = discPixels;
      const paint = renderer.plasmaDisc.getContext("2d");
      const scale = discPixels / 2.5;
      paint.translate(discPixels / 2, discPixels / 2);
      paint.scale(scale, scale);
      paint.globalCompositeOperation = "lighter";
      for (let arm = 0; arm < arms; arm++) {
        const offset = arm * (segments + 1) * 3;
        for (let i = 0; i <= segments; i++) {
          const u = i / segments, a = arm * Math.PI * 2 / arms + u * 3.7;
          const r = .24 + u * .77, at = offset + i * 3;
          points[at] = Math.cos(a) * r;points[at + 1] = Math.sin(a) * r;
        }
        for (let part = 0; part < 8; part++) {
          const start = part * 8, u = (part + .5) / 8;
          paint.beginPath();
          for (let i = start; i <= start + 8; i++) {
            const at = offset + i * 3;
            if (i === start) paint.moveTo(points[at], points[at + 1]);
            else paint.lineTo(points[at], points[at + 1]);
          }
          paint.strokeStyle = renderer.colors[(arm + Math.floor(part / 2)) % 8];
          paint.globalAlpha = (1-u*.7)*.16;paint.lineWidth = .12-u*.07;paint.stroke();
          paint.globalAlpha = (1-u*.6)*.68;paint.lineWidth = .027-u*.017;paint.stroke();
          paint.strokeStyle = "#dbfcff";paint.globalAlpha = (1-u)*.8;paint.lineWidth = .006;paint.stroke();
        }
      }
      renderer.lossDisc = document.createElement("canvas");
      renderer.lossDisc.width = renderer.lossDisc.height = discPixels;
      const lossPaint = renderer.lossDisc.getContext("2d");
      lossPaint.drawImage(renderer.plasmaDisc, 0, 0);
      lossPaint.globalCompositeOperation = "source-in";
      lossPaint.fillStyle = renderer.palette.red;lossPaint.fillRect(0, 0, discPixels, discPixels);
    }
    const expansion = (1 + pulse * .07) * (loss ? 1 - reaction * .55 : 1);
    const squash = .83 + Math.sin(t*.31) * .08;
    const discGrowth = .45 + growth * .55;
    const discSize = radius * 2.5 * expansion * discGrowth;
    ctx.save();ctx.translate(cx,cy);ctx.scale(1,squash);ctx.rotate(spin);
    ctx.globalAlpha = .22 + growth * .4;
    if (arms) ctx.drawImage(loss && reaction > .1 ? renderer.lossDisc : renderer.plasmaDisc, -discSize/2, -discSize/2, discSize, discSize);
    ctx.restore();
    for (let arm = 0; arm < arms; arm++) {
      for(let knot=0;knot<3;knot++) {
        const u = (t*.38+arm*.17+knot/3)%1;
        const a = arm*Math.PI*2/arms+spin+u*3.7;
        const r = radius*(.24+u*.77)*expansion*discGrowth;
        const x = cx+Math.cos(a)*r, y = cy+Math.sin(a)*r*squash;
        const size = (1-u)*13+5;
        ctx.globalAlpha=.3+growth*.4;ctx.drawImage(renderer.glows[arm%2],x-size/2,y-size/2,size,size);
      }
    }
    // Background layers are earned by decades; the seed has none of them.
    const coreRadius = Math.max(23, radius * .19);
    ctx.strokeStyle=renderer.colors[1];
    for(let i=0;i<(stage >= 5 ? Math.min(18, (stage-4)*3) : 0);i++) {
      const a=i*Math.PI/9-spin*.6;
      const energy=.5+.5*Math.sin(t*2.4+i*1.9);
      const inner=coreRadius*(1.08+energy*.12);
      const outer=coreRadius*(1.4+energy*.8+pulse*.22);
      ctx.globalAlpha=.22+energy*.3;ctx.lineWidth=1+energy*2;
      ctx.beginPath();ctx.moveTo(cx+Math.cos(a)*inner,cy+Math.sin(a)*inner);
      ctx.quadraticCurveTo(cx+Math.cos(a+.24)*outer,cy+Math.sin(a+.24)*outer,cx+Math.cos(a+.45)*inner,cy+Math.sin(a+.45)*inner);ctx.stroke();
    }
    for(let i=0;i<(stage >= 8 ? stage-7 : 0);i++) {
      const r=radius*(.89+i*.12);
      const a=-spin*(i%2?1:-.7)+i*2.1;
      ctx.globalAlpha=visualStreak >= 50 ? .55 : .22;ctx.strokeStyle=renderer.colors[(i*2+4)%8];ctx.lineWidth=i===1?2:1;
      ctx.beginPath();ctx.ellipse(cx,cy,r,r*.83,0,a,a+1.5+Math.min(visualStreak,100)*.005);ctx.stroke();
    }
    // Every answer leaves a permanent segment. Cache this construction separately
    // from the backdrop. Two cached images let neighboring rings counter-rotate
    // without rebuilding their paths or allocating a surface for every ring.
    const earnedStreak = Math.floor(streak);
    const recentColors = renderer.earnedColors;
    const earnedKey = `${renderer.earnedStateKey}|${renderer.paletteKey}|${discPixels}`;
    if (renderer.earnedKey !== earnedKey) {
      renderer.earnedKey = earnedKey;
      const discs = renderer.earnedDiscs || (renderer.earnedDiscs = [document.createElement("canvas"), document.createElement("canvas")]);
      const paints = discs.map(earned => {
        if (earned.width !== discPixels) earned.width = earned.height = discPixels;
        const paint = earned.getContext("2d");
        paint.setTransform(1,0,0,1,0,0);paint.clearRect(0,0,discPixels,discPixels);
        paint.translate(discPixels/2,discPixels/2);paint.scale(discPixels/2.5,discPixels/2.5);
        return paint;
      });
      const count = Math.min(100, earnedStreak);
      const current = (earnedStreak-1)%100;
      for (let i=0;i<count;i++) {
        const ring=Math.floor(i/10);
        // The ring direction alternates by layer. Add counter-clockwise layers
        // in reverse slot order so the newest answer remains at the leading edge
        // of its motion instead of looking like a trailing tail.
        const slot=ring%2 ? 9-(i%10) : i%10;
        const paint=paints[ring%2];
        const r=.32+ring*.069, a=-Math.PI/2+slot*Math.PI/5;
        // Past 100, completed rings remain. Each answer reforges one segment in
        // a contrasting generation, keeping work and storage bounded forever.
        const generation=Math.floor((earnedStreak-1-i)/100);
        const answerIndex=i+generation*100;
        const historyIndex=answerIndex-(earnedStreak-recentColors.length);
        const rating=renderer.palette[recentColors[historyIndex]] || renderer.palette.green;
        const color=generation%2 ? "#f1ddff" : rating;
        const active=ring===Math.floor(current/10);
        paint.beginPath();paint.arc(0,0,r,a+.075,a+Math.PI/5-.075);
        paint.strokeStyle=color;paint.globalAlpha=active?.95:.56;
        paint.lineWidth=active?.032:.015;paint.stroke();
        const mid=a+Math.PI/10,x=Math.cos(mid)*r,y=Math.sin(mid)*r;
        paint.globalAlpha=1;paint.fillStyle=color;
        paint.beginPath();paint.arc(x,y,active?.021:.012,0,Math.PI*2);paint.fill();
        if(i===current) {
          // An outward diamond marks the newest piece after its pulse has faded.
          const tip=r+.048;paint.save();paint.rotate(mid);paint.translate(tip,0);
          paint.fillStyle="#ffffff";paint.beginPath();paint.moveTo(.024,0);
          paint.lineTo(0,.018);paint.lineTo(-.024,0);paint.lineTo(0,-.018);paint.closePath();paint.fill();paint.restore();
        }
      }
    }
    ctx.globalCompositeOperation="source-over";ctx.globalAlpha=loss?1-reaction*.75:1;
    // The core count stays steady; alternating layers turn in opposite directions.
    // Clock freezes for Still, pause, hidden scenes, and reduced motion.
    const earnedSize=radius*2.5;
    for(let parity=0;parity<Math.min(2,Math.ceil(earnedStreak/10));parity++) {
      ctx.save();ctx.translate(cx,cy);ctx.rotate(t*.28*(parity===0?1:-1));
      ctx.drawImage(renderer.earnedDiscs[parity],-earnedSize/2,-earnedSize/2,earnedSize,earnedSize);
      ctx.restore();
    }
    ctx.globalCompositeOperation="lighter";
    // Early answers get a small ripple; late milestones earn the full eruption.
    if(reaction>.001) {
      ctx.strokeStyle=renderer.eventColor||"#c5fcff";
      for(let shell=0;shell<(stage < 2 ? 1 : 3);shell++) {
        const r=radius*(loss?Math.max(.12,1.35-age*.7):.25+age*(.55+shell*.12))*(.35+growth*.65);
        ctx.globalAlpha=reaction*(shell===0?.7:.22);ctx.lineWidth=shell===0?1.5:3;
        ctx.beginPath();ctx.ellipse(cx,cy,r,r*.86,0,0,Math.PI*2);ctx.stroke();
      }
      const sprayCount=stage < 2 ? 0 : renderer.eventStrength>=2?32:8+stage;
      for(let i=0;i<sprayCount;i++) {
        const a=i*Math.PI*2/sprayCount+i*.07;
        const r=radius*(.3+age*(.6+(i%4)*.09));
        const tail=(10+(i%5)*5)*reaction*renderer.eventStrength;
        ctx.globalAlpha=reaction*.8;ctx.lineWidth=i%3===0?2:1;
        ctx.beginPath();ctx.moveTo(cx+Math.cos(a)*r,cy+Math.sin(a)*r*.9);
        ctx.lineTo(cx+Math.cos(a)*(r+tail),cy+Math.sin(a)*(r+tail)*.9);ctx.stroke();
      }
    }
    // The readout remains steady inside a hot, cached core bloom.
    const bloom=coreRadius*(2+growth*2+pulse*.35);
    ctx.globalAlpha=.15+growth*.65;ctx.drawImage(renderer.glows[1],cx-bloom/2,cy-bloom/2,bloom,bloom);
    ctx.globalCompositeOperation="source-over";ctx.globalAlpha=1;ctx.fillStyle="#071223";
    ctx.beginPath();ctx.arc(cx,cy,coreRadius,0,Math.PI*2);ctx.fill();
    ctx.strokeStyle=loss&&reaction>.1?renderer.eventColor:"#9ac5d0";ctx.lineWidth=1+growth;ctx.stroke();
    ctx.globalAlpha=.12+growth*.38;ctx.strokeStyle=renderer.colors[3];ctx.lineWidth=2+growth*2;
    ctx.beginPath();ctx.arc(cx,cy,coreRadius+4,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1;
    if(moving && (earnedStreak > 0 || stage >= 2 || (age >= 0 && age < 1.9))) {
      renderer.frameId=window.requestAnimationFrame(timestamp=>drawPrismGate(renderer,timestamp));
    } else {
      // Only the empty seed is static; the first earned piece starts rotating.
      // Answer updates wake this same loop; no idle polling timer is needed.
      renderer.lastAt=renderer.nextFrameAt=0;
    }
  }
  ...
  function handleStateEffects(data) {
    ...
    const singularity = isSingularityMode(data) || getVisualMode(data) === "prism_gate";
    const sphereMode = getSphereMode(data);
  ...
  function renderLiveTimerState(data) {
    ...
    const animationSignature = [
      ...
      Number(Boolean(timer.autoPaused)),
    ...
    if (!enabled) {
      ...
      timerHero.classList.remove("danger");
      timerValue.classList.remove("danger");
    } else if (timer.autoPaused) {
      const timerRamp = getTimerRampColors(data);
      const ratio = clamp(Number(timer.totalRatio || 0), 0, 1);
      setText("acgTimer", "Time Drain");
      setText(phaseLabel, "Time Drain");
      setText(timerValue, "--");
      setStyleProperty(timerHero, "--timer-progress", `${ratio}turn`);
      setStyleProperty(timerHero, "--timer-base-progress", `${ratio}turn`);
      setStyleProperty(timerHero, "--timer-color", timerRamp.yellow);
      syncTimerRingWebgl(
        timerHero,
        timer,
        timerRamp.yellow,
      ...
      const warningMs = Math.max(1000, Number(data?.countdownWarningSeconds || 3) * 1000);
      const danger = timer.remaining > 0 && timer.remaining <= warningMs;
    ...
    if (timeDrainOverlay && timeDrainTimer) {
      ...
      if (sidebar) sidebar.classList.toggle("time-drain-active", activeTimeDrain);
      const scoreboardPanel = $("acgScoreboardPanel");
      if (scoreboardPanel) {
        scoreboardPanel.hidden = activeTimeDrain;
        scoreboardPanel.setAttribute("aria-hidden", activeTimeDrain ? "true" : "false");
      }
      setText(timeDrainTimer, timer.autoPaused ? "--" : timer.untimed ? "UNTIMED" : timer.free ? "FREE" : timer.phase === "idle" ? "--" : timer.secondsText);
    }
  }
  ...
  function formatRunTimestamp(value) {
    ...
    const prefix = sameDay ? "Today" : (date.toDateString() === yesterday.toDateString() ? "Yesterday" : date.toLocaleDateString([], { month: "short", day: "numeric" }));
  ...
  function formatRunEndReason(value) {
    const labels = {
      ...
      "anki-closed": "Anki closed without restore",
  ...
  function scoreboardRecords(data) {
    const history = data?.runHistory || {};
    const completed = Array.isArray(history.runs) ? history.runs.map((run) => ({ ...run, live: false })) : [];
    const purity = String(data?.scoreboardPurity || history.purityMode || "all");
    const order = String(data?.scoreboardOrder || "ranked");
    const records = purity === "pure" ? completed.filter(isPureRun) : completed;
    if (order === "chronological") {
      records.sort((left, right) => {
        return Date.parse(String(right.endedAt || "")) - Date.parse(String(left.endedAt || ""));
      });
    } else {
      records.sort((left, right) => {
        const scoreDifference = Number(right.streak || 0) - Number(left.streak || 0);
  ...
  function renderScoreboardDetail(record) {
    const detail = $("acgScoreboardDetail");
    if (!detail) return;
    if (!record) {
    ...
    const pauseCount = Math.max(0, Number(record.pauseCount || 0));
    const exitCount = Math.max(0, Number(record.reviewExitCount || 0));
    const title = record.live ? "LIVE RUN" : formatRunTimestamp(record.endedAt);
    detail.classList.remove("hidden");
    detail.innerHTML = `
      <div class="acg-scoreboard-detail-head">
        <strong>${escapeHtml(title)}</strong>
        <span>${Number(record.streak || 0)} cards</span>
      </div>
      <dl>
        <div><dt>First → last streak number</dt><dd>${formatRunDuration(record.runSpanMs)}</dd></div>
        <div><dt>Total time (in-game)</dt><dd>${formatRunDuration(record.activeMs)}</dd></div>
        <div><dt>Manual pauses</dt><dd>${pauseCount} · ${formatRunDuration(record.pausedMs)}</dd></div>
        <div><dt>Left Review</dt><dd>${exitCount} · ${formatRunDuration(record.reviewExitMs)}</dd></div>
        <div><dt>Restart continuation</dt><dd>${record.resumedAfterRestart ? "Yes" : "No"}</dd></div>
        <div><dt>Undo used</dt><dd>${Math.max(0, Number(record.undoCount ?? (record.usedUndo ? 1 : 0)))}</dd></div>
        ${record.live ? "" : `<div><dt>Run ended</dt><dd>${escapeHtml(formatRunEndReason(record.endReason))}</dd></div>`}
      </dl>
      ${record.live ? "" : `<button class="acg-scoreboard-delete" type="button" data-delete-scoreboard-run="${Number(record.id || 0)}">Delete this run</button>`}
    `;
  }

  function burstScoreboardConfetti() {
    const container = $("acgScoreboardConfetti");
    if (!container) return;
    container.replaceChildren();
    const palette = ["#ff6f96", "#ffd978", "#65f0c2", "#7fb0ff", "#f097e4", "#9f63d3"];
    const pieceCount = 22;
    for (let index = 0; index < pieceCount; index += 1) {
      const piece = document.createElement("i");
      const angle = (-168 + ((336 / (pieceCount - 1)) * index)) * (Math.PI / 180);
      const distance = 34 + ((index % 5) * 7);
      piece.style.setProperty("--acg-confetti-x", `${Math.cos(angle) * distance}px`);
      piece.style.setProperty("--acg-confetti-y", `${Math.sin(angle) * distance}px`);
      piece.style.setProperty("--acg-confetti-turn", `${180 + ((index * 83) % 360)}deg`);
      piece.style.setProperty("--acg-confetti-delay", `${(index % 4) * 18}ms`);
      piece.style.backgroundColor = palette[index % palette.length];
      piece.classList.toggle("round", index % 4 === 0);
      container.appendChild(piece);
    }
    window.setTimeout(() => container.replaceChildren(), 1500);
  }

  function celebrateScoreboard(root) {
    if (!root) return;
    root.classList.remove("new-best");
    void root.offsetWidth;
    root.classList.add("new-best");
    burstScoreboardConfetti();
    if (state.scoreboardCelebrationTimer) window.clearTimeout(state.scoreboardCelebrationTimer);
    state.scoreboardCelebrationTimer = window.setTimeout(() => {
      state.scoreboardCelebrationTimer = 0;
      root.classList.remove("new-best");
    }, 1800);
  }

  function renderScoreboard(data) {
    const root = $("acgScoreboard");
    const target = $("acgScoreboardTarget");
    const panel = $("acgScoreboardPanel");
    const rowsNode = $("acgScoreboardRows");
    if (!root || !target || !panel || !rowsNode) return;
    const layout = String(data?.scoreboardLayout || "best_only");
    const listMode = String(data?.scoreboardListMode || "recent");
    const order = String(data?.scoreboardOrder || "ranked");
    const purity = String(data?.scoreboardPurity || "all") === "pure" ? "pure" : "all";
    const history = data?.runHistory || {};
    const todayMode = String(history.benchmarkMode || "all_time") === "today" || listMode === "today";
    const best = Math.max(0, Number(history.bestStreak || 0));
    const savedBest = Math.max(0, Number(history.savedBestStreak ?? history.bestStreak ?? 0));
    const active = history.active && Number(history.active.streak || 0) > 0 ? history.active : null;
    const liveStreak = active ? Math.max(0, Number(active.streak || 0)) : 0;
    const records = scoreboardRecords(data);
    const recordBreaking = savedBest > 0
      && liveStreak > savedBest
      && (purity !== "pure" || isPureRun(active));
    root.classList.toggle("hidden", layout === "off");
    root.classList.toggle("compact", layout === "compact");
    root.classList.toggle("ladder", layout === "ladder");
    root.classList.toggle("best-only", layout === "best_only");
    root.classList.toggle("open", layout === "ladder");
    root.classList.toggle("record-breaking", recordBreaking);
    const bestLabel = target.querySelector(".acg-scoreboard-metric small");
    setText(bestLabel, recordBreaking
      ? "NEW RECORD"
      : (purity === "pure"
        ? (todayMode ? "PURE TODAY'S BEST" : "PURE BEST")
        : (todayMode ? "TODAY'S BEST" : "ALL-TIME BEST")));
    setText("acgScoreboardBest", best > 0 ? String(best) : "—");
    target.setAttribute(
      "aria-label",
      recordBreaking
        ? `New record: ${best}. Open Streak Records options.`
        : `Open Streak Records options. ${todayMode ? "Today's best" : "All-time best"}: ${best > 0 ? best : "none yet"}.`,
    );
    setText("acgScoreboardTitle", purity === "pure"
      ? (todayMode ? "PURE TODAY'S BEST 5" : (listMode === "best" ? "PURE BEST 5" : "PURE RECENT 5"))
      : (todayMode ? "TODAY'S BEST 5" : (listMode === "best" ? "ALL-TIME BEST 5" : "RECENT 5")));

    if (!active) {
      setText("acgScoreboardLive", "—");
      target.title = best > 0
        ? `${todayMode ? "Today's best" : "Best streak"}: ${best}. Click for Stats or Settings.`
        : `Complete cards to set your first${todayMode ? " streak today" : " streak record"}. Click for Stats or Settings.`;
    } else if (savedBest <= 0) {
      setText("acgScoreboardLive", String(liveStreak));
      target.title = `Live streak: ${liveStreak}. This is your first${todayMode ? " streak today" : " streak record"}. Click for Stats or Settings.`;
    } else if (liveStreak > savedBest) {
      setText("acgScoreboardLive", String(liveStreak));
      target.title = `New best by ${liveStreak - savedBest}. Click for Stats or Settings.`;
    } else if (liveStreak === savedBest) {
      setText("acgScoreboardLive", String(liveStreak));
      target.title = `Your live streak is tied with your saved best${todayMode ? " today" : ""}. Click for Stats or Settings.`;
    } else {
      setText("acgScoreboardLive", String(liveStreak));
      target.title = `${savedBest - liveStreak} cards from your saved best${todayMode ? " today" : ""}. Click for Stats or Settings.`;
    }
    const livePurity = $("acgScoreboardLivePurity");
    if (livePurity) {
      const liveIsPure = Boolean(active) && isPureRun(active);
      livePurity.classList.toggle("hidden", !active);
      livePurity.classList.toggle("pure", liveIsPure);
      livePurity.classList.toggle("breaks", Boolean(active) && !liveIsPure);
      setText(livePurity, active ? (liveIsPure ? "Pure" : "Breaks") : "");
      livePurity.setAttribute("aria-label", active ? `Current run: ${liveIsPure ? "Pure" : "Breaks"}.` : "");
      livePurity.setAttribute(
        "title",
        active
          ? (liveIsPure
            ? "Pure run: no pauses, Review departures, restart continuation, or undo."
            : "Run with breaks: this run includes a pause, Review departure, restart continuation, or undo.")
          : "",
      );
    }

    // Completed records normally stay identical across hundreds of live answers.
    // Preserve their nodes (and focus) and avoid formatting dates/rebuilding hidden
    // rows in Best Only mode. Include only fields that affect row presentation.
    const rowsSignature = JSON.stringify([order, new Date().toDateString(), records.map(record => [
      record.id, record.endedAt, record.streak, isPureRun(record),
    ])]);
    if (rowsSignature !== state.lastScoreboardRowsSignature) {
      state.lastScoreboardRowsSignature = rowsSignature;
      rowsNode.innerHTML = records.length
        ? records.map((record, index) => {
          const timestamp = formatRunTimestamp(record.endedAt);
          return `
            <button class="acg-scoreboard-row" type="button" data-scoreboard-run-id="${escapeHtml(String(record.id))}" aria-haspopup="dialog" aria-expanded="false" title="Open this streak record's Stats or Settings">
              <span class="acg-scoreboard-rank">${order === "ranked" ? `#${index + 1}` : "•"}</span>
              <span class="acg-scoreboard-row-copy"><strong>${escapeHtml(timestamp)}</strong><small class="acg-scoreboard-row-purity ${isPureRun(record) ? "pure" : "breaks"}">${isPureRun(record) ? "Pure" : "Breaks"}</small></span>
              <b>${Number(record.streak || 0)}</b>
            </button>
          `;
        }).join("")
        : `<div class="acg-scoreboard-empty">No finished runs yet.</div>`;
    }

    if (!state.scoreboardInitialized) {
      state.scoreboardInitialized = true;
    } else if (
      Boolean(data?.scoreboardCelebrationEnabled)
      && recordBreaking
      && state.lastScoreboardLiveStreak <= savedBest
    ) {
      celebrateScoreboard(root);
    }
    state.lastScoreboardLiveStreak = liveStreak;
  }

  function syncPauseOverlayPlacement(data = state.data || {}) {
    const sidebar = $("speed-streak-sidebar");
    const scoreboard = $("acgScoreboard");
    if (!sidebar || !scoreboard) return;
    const layout = String(data?.scoreboardLayout || "best_only");
    const placeBelowScoreboard = layout === "ladder"
      && !scoreboard.classList.contains("hidden");
    sidebar.classList.toggle("pause-below-scoreboard", placeBelowScoreboard);
    if (!placeBelowScoreboard) {
      sidebar.style.removeProperty("--acg-pause-top");
      return;
    }
    const sidebarRect = sidebar.getBoundingClientRect();
    const scoreboardRect = scoreboard.getBoundingClientRect();
    if (sidebarRect.height <= 0 || scoreboardRect.height <= 0) return;
    const top = Math.max(12, Math.round(scoreboardRect.bottom - sidebarRect.top + 6));
    sidebar.style.setProperty("--acg-pause-top", `${top}px`);
  }

  function syncTimeDrainOverlayLayout() {
    const sidebar = $("speed-streak-sidebar");
    const overlay = $("acgTimeDrainOverlay");
    const target = $("acgScoreboardTarget");
    const scoreboard = $("acgScoreboard");
    if (!sidebar || !overlay || !target || !scoreboard) return;
    if (!sidebar.classList.contains("time-drain-active") || scoreboard.classList.contains("hidden")) {
      overlay.style.removeProperty("--acg-time-drain-target-top");
      overlay.style.removeProperty("--acg-time-drain-target-bottom");
      return;
    }
    const overlayRect = overlay.getBoundingClientRect();
    const targetRect = target.getBoundingClientRect();
    if (overlayRect.height <= 0 || targetRect.height <= 0) return;
    overlay.style.setProperty(
      "--acg-time-drain-target-top",
      `${Math.round(targetRect.top - overlayRect.top)}px`,
    );
    overlay.style.setProperty(
      "--acg-time-drain-target-bottom",
      `${Math.round(targetRect.bottom - overlayRect.top)}px`,
    );
  }
  ...
  function render(data) {
    ...
    const scoreboardOff = String(data?.scoreboardLayout || "best_only") === "off";
    ...
    sidebar.classList.toggle("prism-gate-mode", enabled && visualsEnabled && visualMode === "prism_gate");
    ...
    sidebar.classList.toggle("scoreboard-off", scoreboardOff);
    ...
    applyScoreboardColors(sidebar, data.scoreboardColors || {});
    ...
    syncTimeDrainOverlayLayout();
    syncPauseOverlayPlacement(data);
    ...
    if (!fusionLossIncoming) handleStateEffects(data);
    if (!(enabled && visualsEnabled && !sidebarCollapsed && visualMode === "prism_gate")) stopPrismGate(true);
    if (enabled && visualsEnabled && !sidebarCollapsed && visualMode === "prism_gate") {
      stopSingularityRenderer();
      stopCrystalReactor();
      clearOrbitScene();
      clearRowsScene();
      renderPrismGate(data);
    } else if (enabled && visualsEnabled && !sidebarCollapsed && lightweightRows) {
      stopSingularityRenderer();
      stopCrystalReactor();
      clearOrbitScene();
  ...
  window.SpeedStreak = {
    ...
    previewHaptics(sequence) {
      if (!Array.isArray(sequence) || sequence.length === 0 || !getGamepadActuator()) {
        return false;
      }
      playBrowserHaptics(sequence);
      return true;
  ...
  window.addEventListener("pagehide", () => { stopPrismGate(true); disposeSingularityRenderer(); });
