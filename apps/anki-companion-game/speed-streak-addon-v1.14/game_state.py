...
class CompanionGameEngine:
    ...
    def update_time_limits(
        ...
        *,
        ...
        self._publish(
            "settings",
            f"Timers updated: question {question_seconds:.1f}s, answer {answer_seconds:.1f}s, time drain flag {s.time_drain_flag}, review later flag {s.review_later_flag}, visuals {'on' if s.visuals_enabled else 'off'}, orbit animation {'on' if s.orbit_animation_enabled else 'off'}, appearance {s.appearance_mode}.",
        )
        return "settings"
