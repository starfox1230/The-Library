...
DEFAULT_BOOST_SECONDS = 5.0
DEFAULT_MAX_BOOST_CHARGES = 3
DEFAULT_STARTING_BOOST_CHARGES = 1
DEFAULT_CARDS_PER_BOOST_CHARGE = 5
...
class CompanionGameEngine:
    ...
    def on_rate(self, ease: int) -> Optional[str]:
        ...
        completed_on_active_timer = bool(
            not self.card.question_free
            and s.phase_limit_ms > 0
            and answer_elapsed_ms < s.phase_limit_ms
        ...
        if s.visuals_enabled:
            ...
            if s.gameplay_mode == GAMEPLAY_MODE_TIME_BOOST:
                s.streak_multiplier = 1.0
                charge_earned = self._advance_boost_charge_progress() if completed_on_active_timer else False
                charge_detail = (
                    f"charge earned ({s.boost_charges}/{s.max_boost_charges})"
                    if charge_earned
                    else (
                        f"charges full ({s.boost_charges}/{s.max_boost_charges})"
                        if s.boost_charges >= s.max_boost_charges
                        else (
                            f"charge {s.boost_charge_progress}/{s.cards_per_boost_charge}"
                            if completed_on_active_timer
                            else "charge meter unchanged (untimed/expired)"
                        )
                    )
                )
    ...
    def time_boost_unavailable_reason(self) -> str:
        ...
        if s.paused:
            return "Resume the timer before using a charge."
        if s.first_card_free:
            return "The first card is free—no charge needed."
        ...
        if s.boost_charges <= 0:
            return "No Time Boost charges are available."
    ...
    def use_time_boost(self) -> Optional[str]:
        ...
        self._publish(
            "time-boost",
            f"Time Boost: +{s.boost_seconds:g}s | {s.boost_charges} charge{'s' if s.boost_charges != 1 else ''} left.",
        )
        return "time-boost"
