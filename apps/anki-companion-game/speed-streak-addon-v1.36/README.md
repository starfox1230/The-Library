...
This folder contains Speed Streak v1.36, built directly from v1.35 without modifying the frozen v1.35 folder.
...
<ul>
  <li>Added <strong>Time Boost</strong>, which replaces score with earnable charges that can add time before the timer expires</li>
...
- Uses cropped screenshots rendered from the real v1.36 interface in both the in-add-on guide and the paste-ready AnkiWeb description.
...
- Sends a short-lived yellow electric shock, expanding energy ring, and capped lightning fan across the entire panel whenever Time Boost earns a new charge.
...
- Offers Time Boost mode, where staying on pace earns limited charges that add time, with optional No Pause and No Undo focus rules
...
3. Close Anki.
4. In the folder that opens, create a new folder named `speed_streak_v1_36`.
...
`v1.36` keeps the v1.35 visuals and gameplay while adding the progressive Singularity mode.
...
- In Time Boost mode, press the configured boost shortcut (default `C`) to spend one charge and add time.
