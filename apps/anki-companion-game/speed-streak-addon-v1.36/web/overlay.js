(function () {
  ...
  function computeTimer(data) {
    ...
    const totalRatio = boostActive ? boostRatio : baseProgress;
    const overflow = Math.max(0, boostRatio - 1);
  ...
  function createTimerRingProgram(gl) {
    ...
    const fragment = createShader(gl, gl.FRAGMENT_SHADER, `
      ...
      void main() {
        ...
        float overflowArc = u_overflow_turns >= 1.0 ? 1.0 : clamp(u_overflow_progress, 0.0, 1.0);
  ...
  function drawTimerRingFrame(renderer) {
    ...
    if (timer.active && timer.total > 0) {
      ...
      const boostActive = boostRemaining > 0;
      progress = clamp(boostActive ? boostRatio : normalRatio, 0, 1);
      baseProgress = boostActive ? 0 : clamp(normalRatio, 0, 1);
      const overflow = Math.max(0, boostRatio - 1);
  ...
  function syncTimerRingWebgl(timerHero, timer, color, progress, active, animationSignature, deadlineEpochMs) {
    ...
    renderer.timer = {
      ...
      progress,
      baseProgress: Boolean(timer?.boostActive) ? 0 : Number(timer?.baseProgress ?? progress),
  ...
  function renderLiveTimerState(data) {
    ...
    if (!enabled) {
      ...
      const displayBaseProgress = timer.boostActive ? 0 : normalRatio;
  ...
  function renderGameplayEconomy(data) {
    ...
    if (boostZone) {
      boostZone.setAttribute("title", canUseBoost ? "Click the charge bank to edit Time Boost settings" : unavailableReason);
    }
    ...
    setText("acgBoostProgressText", charges >= maxCharges ? "" : `Next charge ${progress} / ${required}`);
    if (progressFill) {
