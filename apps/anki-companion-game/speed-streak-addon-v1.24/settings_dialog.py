...
from .game_state import TIMER_POLICY_EXTRA_TIME, TIMER_POLICY_NORMAL, TIMER_POLICY_NO_TIMEOUT
...
class SettingsDialog(QDialog):
    def __init__(self, controller: Any) -> None:
        ...
        self.special_timer_tag_controls: list[dict[str, QWidget]] = []
        ...
        self.setStyleSheet(
            ...
            QDoubleSpinBox:hover, QComboBox:hover, QLineEdit:hover {
            ...
            QDoubleSpinBox:disabled, QComboBox:disabled, QLineEdit:disabled {
        ...
        self.developer_status_label.setStyleSheet(
            """
            color: #f4f7ff;
            background: rgba(48, 68, 104, 0.96);
    ...
    def _build_special_timing_card_panel(
        ...
        *,
        ...
        if card_type == "anking":
            ...
            self.special_timing_tag_input.textChanged.connect(self.persist_settings)
    ...
    def _build_absolute_special_timers(self, parent: QWidget, layout: QVBoxLayout) -> None:
        ...
        tags_panel = QFrame(parent)
        tags_panel.setProperty("class", "settingRow")
        tags_panel_layout = QVBoxLayout(tags_panel)
        tags_panel_layout.setContentsMargins(12, 12, 12, 12)
        ...
        add_button = QPushButton("Add Tag Rule", tags_panel)
    ...
    def _build_absolute_timer_rule_panel(
        ...
        *,
        ...
        panel = QFrame(parent)
        panel.setProperty("class", "settingRow")
        ...
        controls: dict[str, QWidget] = {"enabled": enabled}
        if rule_key == "tag":
            tag_input = QLineEdit(panel)
            tag_input.setPlaceholderText("speedstreak::long-card")
            tag_input.setText(tag)
            tag_input.textChanged.connect(self.persist_settings)
            controls["tag"] = tag_input
            panel_layout.addWidget(self._build_setting_row(panel, "Exact tag", "Matches this complete Anki tag only.", tag_input, control_width=300))
        ...
        if rule_key == "tag":
            remove_button = QPushButton("Remove Tag Rule", panel)
    ...
    def _remove_special_timer_tag_rule(self, controls: dict[str, QWidget]) -> None:
    ...
    def _build_audio_event_settings(self, parent: QWidget, layout: QVBoxLayout) -> None:
        for item in HAPTIC_EVENT_OPTIONS:
            ...
            upload_button = QPushButton("Upload", parent)
            ...
            preview_button = QPushButton("Play", parent)
    ...
    def _build_haptic_event_settings(self, parent: QWidget, layout: QVBoxLayout) -> None:
        for item in HAPTIC_EVENT_OPTIONS:
            ...
            preview_button = QPushButton("Test", parent)
            preview_button.setProperty("class", "secondaryAction")
    ...
    def _build_shortcuts_section(self, parent: QWidget) -> QWidget:
        ...
        self.block_answer_keys_while_paused_check = QCheckBox("Block answer keys while paused", frame)
        self.block_answer_keys_while_paused_check.toggled.connect(self.persist_settings)
        layout.addWidget(
            self._build_setting_row(
                frame,
                "Pause Screen Key Guard",
                "When Speed Streak is paused during review, block Anki's normal Space, Enter, and 1-4 answer shortcuts.",
    ...
    def _sync_single_special_timer_controls(
        ...
        if isinstance(tag_input, QLineEdit):
            tag_input.setText(str(rule.get("tag", "") or ""))
    ...
    def _special_timer_rule_from_controls(self, controls: dict[str, QWidget]) -> dict[str, Any]:
        enabled = controls.get("enabled")
        question_spin = controls.get("question_spin")
        answer_spin = controls.get("answer_spin")
        question_untimed = controls.get("question_untimed")
        ...
        tag_input = controls.get("tag")
        if isinstance(tag_input, QLineEdit):
            result["tag"] = tag_input.text().strip()
        return result
