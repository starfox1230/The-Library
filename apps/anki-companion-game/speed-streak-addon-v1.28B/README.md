...
This folder contains the Speed Streak v1.28B version of the project packaged as an Anki add-on.
...
3. Close Anki.
4. In the folder that opens, create a new folder named `speed_streak_v1_28b`.
...
`v1.28B` keeps the `Brick Layout` visual mode, the optional deck-page Review Later count button, the WebGL sphere renderer, the improved Windows haptics path, and the Haptics Lab.
