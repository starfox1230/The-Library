...
This folder contains Speed Streak v1.32, built directly from v1.31 without modifying the frozen v1.31 folder.
...
- Adds `No Undo mode`, which clears Anki's undo history during a committed review run, plus compact No Pause/No Undo toggles above the charge bank.
- Collapses the No Pause/No Undo controls and shortcut keycap until the charge bank is hovered or keyboard-focused. The single control row expands in normal layout between the timer and charges, temporarily pushing the charge bank downward without a panel background. A Gameplay option controls whether inactive focus-mode toggles are included; active toggles are always included.
- Opens Settings directly at the Time Boost capacity and earning controls when the charge bank or its progress area is clicked, and releases mouse-created focus so the control row still collapses when the pointer leaves.
...
3. Close Anki.
4. In the folder that opens, create a new folder named `speed_streak_v1_32`.
...
`v1.32` keeps the v1.31 Sphere, Brick, and Crystal Reactor visuals.
